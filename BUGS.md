...
**1210 filed · 1199 fixed · 13 open · 2 false positives.**
...
| 1197 | P0 | AWS simulator foundational event routing | 9 (missing cloud slice) | The AWS simulator has SQS/SNS and CloudWatch Events-adjacent logs/metrics, but no EventBridge `events` API slice. Foundational EventBridge operations (`PutRule`, `PutTargets`, `PutEvents`, list/describe/delete rules and targets, event bus policy where providers exercise it) must be implemented and covered by SDK/CLI/Terraform tests. |
| 1198 | P0 | GCP simulator foundational event routing | 9 (missing cloud slice) | The GCP simulator has Pub/Sub but no Eventarc slice. Eventarc trigger/channel/provider operations used by SDK/CLI/Terraform flows must be implemented instead of treating Pub/Sub as a substitute for event routing. |
| 1199 | P0 | Azure simulator foundational event routing | 9 (missing cloud slice) | The Azure simulator has Service Bus and Storage Queue but no Event Grid slice. Event Grid topics/domains/system topics/event subscriptions and publish/delivery behaviors needed by official SDK/CLI/Terraform flows must be implemented as Azure public APIs. |
