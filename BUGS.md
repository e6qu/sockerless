...
- BUG-1301: Firecracker VM rootfs image creation used `os.Truncate` as if it created `rootfs.ext4`, but Go's truncate call fails when the file does not already exist. Deterministic old workdirs could mask the bug when a previous image file was present; unique launch workdirs exposed it reliably as `truncate .../rootfs.ext4: no such file or directory`. Rootfs image creation now opens the image with create/truncate flags before sizing it, and default Firecracker launches allocate a unique hash-prefixed workdir under the common temp root so the Firecracker Unix socket path stays short. Explicit caller-provided workdirs remain explicit and isolated by the caller.
