...
**746 total — 743 fixed, 3 open, 1 false positive.**
...
| ID | Sev | Area | Summary |
|----|-----|------|---------|
| 736 | H | cloudrun/aca | Cloud Run Jobs/Services and ACA Jobs/Apps silently dropped `HostConfig.Binds` on the floor — the bind-mount specs never reached the jobspec/servicespec/appspec builders, so `docker run -v /h:/c` completed with an appearing-to-work container that had no mount. Now `ContainerCreate` on both backends rejects `Binds` up-front with an `InvalidParameterError` that points at Phase 92 / Phase 93 (real GCS / Azure Files provisioning). Real mount support lands when those phases implement the cloud-resource wiring documented in `specs/CLOUD_RESOURCE_MAPPING.md`. |
| 735 | H | ecs | `backends/ecs/taskdef.go::buildContainerDef` silently built a Fargate scratch volume when `Binds` was set but `SOCKERLESS_ECS_AGENT_EFS_ID` was unset — docker clients saw the mount path appear but it was empty and non-persistent. `ContainerCreate` now rejects `Binds` with a clear config error when AgentEFSID isn't set; `buildContainerDef` trusts the caller (no silent scratch fallback) and emits a real `EFSVolumeConfiguration` pointing at `/sockerless/binds/<container-short>/<host-path>`. Test `TestBuildContainerDef_MountPoints` updated to set `AgentEFSID="fs-test"` to match the new invariant. |
