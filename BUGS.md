...
**1216 filed · 1202 fixed · 16 open · 2 false positives.**
...
| 1212 | P1 | Azure Terraform local macOS execution path | 10 (testability gap) | Issue #248: the Azure Terraform simulator test fails locally on macOS because Go's cgo-backed SystemCertPool ignores `SSL_CERT_FILE`, so Terraform cannot validate the simulator's generated CA. The harness needs a non-interactive real-provider path, likely by running Terraform in Linux container mode on macOS. |
| 1213 | P1 | AWS EventBridge remaining API parity | 7 (partial implementation) | Issue #249: the foundational EventBridge rule/target/PutEvents flow is implemented, but event-bus lifecycle/policy operations and advanced EventBridge resources such as archives/replays remain missing. Implement the remaining public EventBridge APIs with SDK/CLI/Terraform coverage. |
| 1214 | P1 | Azure Event Grid remaining API parity | 7 (partial implementation) | Issue #250: the foundational custom-topic flow is implemented, but Event Grid domains, domain topics, system topics, partner topics where relevant, and their event subscriptions remain missing. Implement the remaining public Microsoft.EventGrid APIs with SDK/CLI/Terraform coverage. |
| 1215 | P1 | GCP Eventarc remaining API parity | 7 (partial implementation) | Issue #251: the foundational Eventarc trigger lifecycle is implemented, but channel, channel-connection, provider discovery/listing, and sibling Eventarc surfaces remain missing where official clients expose them. Implement the remaining public Eventarc APIs with SDK/CLI/Terraform coverage. |
| 1216 | P1 | simulator local Docker test harness | 10 (testability gap) | Issue #253: the simulator `docker-test` targets for AWS, GCP, and Azure point at missing `Dockerfile.test` files even though docs reference them as the local Docker-based test path. Add real Docker test images that run the existing SDK/CLI/Terraform tests, or replace the dead targets/docs with the actual supported local execution path. |

## Recently closed (last phase only — older history lives in PR descriptions + `git log`)

This phase closed BUG-1197, BUG-1198, and BUG-1199 for the foundational event-routing flows. The AWS simulator now has an EventBridge JSON-protocol slice for rules, targets, tags, and `PutEvents`, including SQS/SNS target delivery and SDK/CLI/Terraform coverage. The GCP simulator now has Eventarc v1 trigger lifecycle routes with AIP-style long-running operations and SDK/gcloud/Terraform coverage. The Azure simulator now has Microsoft.EventGrid topic and event-subscription ARM routes plus a real topic publish endpoint that fans out to webhook subscriptions, covered by the Azure management SDK, Azure CLI, and Terraform topic coverage. Remaining advanced/sibling parity is tracked explicitly as BUG-1213, BUG-1214, and BUG-1215.
