...
**854 total — 853 fixed, 1 open, 1 false positive.** Three sections: [Open](#open) / [False positives](#false-positives) / [Resolved](#resolved). Per-project rule: no bug deferral, no fakes, no fallbacks — every filed bug ships a fix in the same round. Fix detail beyond the one-liner: see `git log <commit>` or the linked PR.
...
|----|-----|------|-----------|
| 854 | M | ecs / image-resolve | sha256-only digest refs route to `public.ecr.aws/docker/library/sha256:...` which 404s. Surfaced by gitlab-runner's permission-helper container in cell 3 (GitLab × ECS). Architecture otherwise works. Fix queued in DO_NEXT.md. |
...
### Phase 110 prep (PR #122 — open) (BUG-845..854)

| ID | Sev | Area | One-liner |
|----|-----|------|-----------|
| 854 | M | ecs / image-resolve | gitlab-runner's `setVolumePermissions` step (a small helper container that fixes permissions on the cache volume before each job) references its image by sha256 digest only (e.g. `sha256:50d8f7aab0bdec1b0a39ef47c23983d1ab0089e57cea32549b4bd80919ee8610`). Sockerless's `resolveImageURI` (BUG-846) routes Docker Hub library refs to `public.ecr.aws/docker/library/<name>:<tag>` — but for a digest-only ref with no name, the routing produces `public.ecr.aws/docker/library/sha256:50d8...` which doesn't exist, and the ECS task pull retries 7× then fails with `CannotPullContainerError: pull image manifest has been retried 7 time(s)`. Surfaces in cell 3 (GitLab × ECS) on the gitlab-runner cache-permission helper. **Pending fix** (recorded in DO_NEXT.md): in `backends/ecs/image_resolve.go`, detect digest-only refs (`^sha256:[0-9a-f]{64}$`) and either resolve them via local image-store digest lookup, or surface a clear error pointing the operator at the docker-volume cache-disable workaround. Cell 3 architecturally works otherwise — gitlab-runner registered, pipeline triggered, runner picked it up, only the cache-helper image pull fails. |
