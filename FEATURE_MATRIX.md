...
| **Memory** | `backends/memory` | WASM sandbox (Core + wazero process execution) |
...
| Command | API Route | Core | Docker | ECS | CloudRun | ACA | Lambda | GCF | AZF | Memory |
|---------|-----------|------|--------|-----|----------|-----|--------|-----|-----|--------|
...
- **Cloud backends (ECS, CloudRun, ACA)**: Exec uses the agent driver chain. If an agent is connected to the container, exec runs on the real container. Otherwise, falls back to synthetic exec.
- **FaaS backends (Lambda, GCF, AZF)**: Exec uses the core synthetic driver. Real exec inside a function invocation is not possible.
- **Memory**: Exec runs inside the WASM sandbox via wazero when `SOCKERLESS_SYNTHETIC=1` is not set.
...
- **Core/Memory**: Build processes the Dockerfile and creates an image record in the store.
...
- **Memory**: Full synthetic network driver with IP allocation (IPAM). On Linux, optional platform network driver provides real network namespace isolation.
...
| Operation | API Route | Core | Docker | ECS | CloudRun | ACA | Lambda | GCF | AZF | Memory |
|-----------|-----------|------|--------|-----|----------|-----|--------|-----|-----|--------|
