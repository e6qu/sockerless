...
#
# Every persistent background loop or listener added to simulators/aws/*.go
...
for bf in "$repo_root"/simulators/aws/sdk-tests/*behavioral*_test.go; do
    [[ -f "$bf" ]] || continue
    rel="${bf#"$repo_root"/}"
...
# 4. Detect newly-added persistent background loops / listeners in simulators/aws/*.go
...
changed_go=$(git diff --name-only "$staged_range" 2>/dev/null \
    | grep -E '^simulators/aws/[^/]+\.go$' \
    | grep -vE '(_test\.go|/docs/|/README\.md)$' \
    || true)

exempt_file="$repo_root/simulators/aws/behavioral-exempt.txt"
