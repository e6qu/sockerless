...
- Auth: the user interface presents a login form on first visit — paste the `BLEEPHUB_ADMIN_TOKEN` value. The token is verified against `GET /api/v3/user`, kept in browser localStorage, and sent on every user-interface request. The `/internal/*` dashboard endpoints enforce it server-side (any valid personal access token, including the admin token); `/health` stays open for liveness probes. It is a single-token operator surface — for multi-user access control, front Bleephub with a reverse proxy.
...
**OAuth Apps.** Distinct entity from GitHub Apps. Bleephub still has operator-only `POST/GET /internal/oauth-apps` setup routes because GitHub has no public Representational State Transfer application programming interface to create OAuth Apps; those routes are not part of the GitHub-compatible `/api/v3/` surface. OAuth web flow (`/login/oauth/authorize`) requires the real login session plus CSRF consent POST before issuing an authorization code; device flow lives at `/login/device/code`. Token-management family on the real `/api/v3/applications/{client_id}/{token,grant}` (check / reset / revoke / scope).
The browser settings surface owns normal OAuth App setup: `GET /settings/oauth-apps` lists the signed-in user's OAuth Apps, and `POST /settings/oauth-apps/new` creates one from form data and returns the one-time client secret. The Bleephub user interface uses those settings routes, not the operator-only internal setup routes.
