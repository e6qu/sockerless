...
The object-byte tests also drive a real `simulator-aws` S3 endpoint: artifact upload, cache upload, and runner log upload assert the expected S3 objects are written, so these paths do not rely on fake S3 or memory-only assertions.
