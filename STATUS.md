...
| | |
|---|---|
| Active branch | `feat/azure-ratchet-3` — **third Azure service ratchet (BUG-2229).** Four agents (one service-set each, isolated worktrees, integrated by merge + one combined measured pass), every op spec-validated against vendored ARM Swagger (0 new violations) + real azure-sdk-for-go: **Cosmos DB** (2024-08-15 25→103/124, 2021-10-15 25→100/121 — MongoDB/Cassandra/Gremlin/SQL-scripts families, throughput migration, account ops), **Event Grid** (both docs 100% — 61/61 + 127/127, incl. partner family), **API Management** (apis 9→52/91, products 4→25/31, five docs 100%; fixed BUG-2228 named-value secret leak), **PostgreSQL Flexible Server** 15→37/66 **+ ARM Resources core** 15→32/40 **+ subscriptions 100% + App Insights components 100%**. **Azure 1000→1409/2597 (38%→54%).** azure build/lint(0)/deadcode(0)/dupl(0) + route-validity + doc/spec-consumption + coverage-floor + spec-validator(0) + contract-hook gates all pass on the merged sim. |
...
| Bugs | See [BUGS.md](BUGS.md) header (2230 filed · 2186 fixed · 2 open · 16 FP). 2 open: BUG-1075 (live-cloud), BUG-1345 (azuread upstream) — both externally gated. |
| Live infra | None up. |
