...
| | |
|---|---|
| Active branch | `feat/gcp-ratchet-3` — **GCP service ratchet round 3 (BUG-2235).** Pivoted from Azure to GCP. Four agents; two targets (Bigtable Admin v2, Cloud Resource Manager v3, Logging v2, Cloud Functions v2) were independently re-verified already at their faithful ceiling (all remaining Discovery methods are `{+name}`/`{+resource}` template alternates the route-validity gate forbids). The two with real gaps: **Compute Engine v1** 174→440/1994 (images/snapshots/disks, the load-balancing control plane, instance-group managers, instance actions, aggregated lists — metadata-only/host-agnostic) and **Cloud Run v2** 62→89/116 (worker pools, instances, UpdateJob/DeleteExecution/Tasks + a jobs getIamPolicy colon-split fix). **GCP 3180→3473/5244 (61%→66%).** gcp build/lint(0)/deadcode(0)/dupl(0) + route-validity + doc/spec-consumption + coverage-floor + contract-hook all green; full-suite spec-validator confirmation before push. |
...
| Bugs | See [BUGS.md](BUGS.md) header (2236 filed · 2192 fixed · 2 open · 16 FP). 2 open: BUG-1075 (live-cloud), BUG-1345 (azuread upstream) — both externally gated. |
| Live infra | None up. |
