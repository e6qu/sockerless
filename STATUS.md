...
| **8** GL × gcf | sockerless-gcf | 🟡 **v16 root cause pinned** | All architectural fixes verified. Materialize completes in 13 s. **Root cause**: gcf's `invokePodServiceMain` POSTs the build container's user CMD immediately on materialize completion. The CMD `[/usr/bin/dumb-init /entrypoint gitlab-runner-build]` exits 0 with no work (gitlab-runner-helper's "build" subcommand is a no-op without CI env). invokePodServiceMain then closes WaitChs and PutInvocationResult, so docker reports the container as "exited" — but gitlab-runner expected the container to STAY ALIVE for `docker attach` script delivery. **Fix path**: port the cloudrun `stdinPipe` + `attachStream` pattern (`backends/cloudrun/attach_stream.go` + `backends/cloudrun/start_service.go::invokeServiceDefaultCmd`) to gcf. Materialize creates the Service; the invoke goroutine WAITS for stdinPipe data (via attach); on receive, POSTs the script as envelope; publishes response back via attachStream. |
