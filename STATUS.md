...
| Bugs | See [BUGS.md](BUGS.md) header (2207 filed · 2163 fixed · 2 open · 16 FP). 2 open: BUG-1075 (live-cloud), BUG-1345 (azuread upstream) — both externally gated. |
| Live infra | None up. |

## What's next

- **AWS service-conformance ratchet (active arc).** The gate measures ~37 AWS services against the vendored `aws-sdk-go-v2` Smithy models; twenty-one are at 100% (Batch, CloudTrail, CodeBuild, WAFv2, ECR, KMS, ELBv2, Lambda, EFS, STS, EventBridge Scheduler, Cloud Map, ElastiCache, Auto Scaling, Route 53, API Gateway v1, API Gateway v2, RDS, CloudFront, EC2, SSM) — plus DynamoDB + Amplify (gated complete). With EC2 + SSM closed, the next big floors are SSM-adjacent mid-size services: CloudWatch Logs (111/113, the rest is HTTP/2 streaming), S3 (105/107, the rest is event-stream/Object-Lambda), and unmeasured/low services — Glue is 264/267 (its faithful max). Resume steps in [DO_NEXT.md](DO_NEXT.md).
