...
| | |
|---|---|
| Active branch | `feat/bleeplab-artifacts` (Arc 3 Phase 4; PR pending). |
| In-flight | **bleeplab object-store-backed CI artifacts (cross-stage passing).** The GitLab ECS cell now does real cross-stage work: build-job compiles `calc` and publishes it as an artifact; test-job (no toolchain) downloads it and runs it — no recompile (`make bleeplab-runner-docker-test-ecs` → TEST 1/2/3 PASS). bleeplab gained an artifact store (S3 > `BLEEPLAB_ARTIFACTS_DIR` > memory, mirroring the git storage), the runner artifact upload/download endpoints, `artifacts:`/`dependencies:` CI parsing, and `dependencies` advertised in the job response. Coordinate fix: the runner's config `url` (used by the in-container artifacts-uploader, not `CI_API_V4_URL`) must be container-reachable → set to `host.docker.internal:8929`. Completes the "storage based on object store, like bleephub" ask (git + artifacts). |
...
| Bugs | See [BUGS.md](BUGS.md) header. 4 open: BUG-1075 (live-cloud), BUG-1345 (azuread upstream), BUG-1781 (FaaS multi-container pods), BUG-1803 (flaky azf reverse-agent attach test). |
| Live infra | None up. |
