...
| Branch purpose | Follow-up Bleephub GitHub-fidelity hardening after #783. The branch made repository and deployment deletion purge persisted child state, repository-ID keyed rows, selected-repository references, deployment rows, and Pages deployment rows so deleted resources do not leave durable state that can attach to later ID reuse. |
| Current fixes | BUG-2457, BUG-2458, BUG-2459, and BUG-2460 were fixed on this branch. Repository deletion now purges persisted issue comments, issue events, sub-issue links, issue dependency links, pull request reviews, pull request review comments, artifact attestations, repository activity, clone traffic, watch subscriptions, selected-repository allowlists, GitHub App selected repositories, installation token repository scopes, code-security attachments, deployments, deployment statuses, environments, environment policies, protection rules, and Pages deployment records for the deleted repository. Deployment deletion now purges status rows with the deployment record. The branch also carried the deterministic README badge refresh generated by the pre-push hook. |
...
| Bugs | See [BUGS.md](BUGS.md) header: 2460 filed, 2416 fixed, 3 open, 16 false positives. Open: BUG-2441, BUG-1345, and BUG-1075. |
