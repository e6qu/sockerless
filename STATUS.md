...
| Current fixes | BUG-2506 and BUG-2507 were fixed on this branch. Manual legacy builds now validate real Pages configuration, resolve the configured git source, directly publish `.nojekyll` trees or run the pinned `github-pages` 232/Jekyll 3.10.0 toolchain in safe production mode, publish real output through the shared S3-compatible transaction, and persist terminal build/site metadata and errors. The release image ships Ruby, Bundler, the exact GitHub Pages gem, and the command wrapper; real release-image, git, and Amazon Simple Storage Service simulator coverage verifies generated Markdown/Liquid output. |
...
| Bugs | See [BUGS.md](BUGS.md) header: 2508 filed, 2463 fixed, 4 open, 16 false positives. Open: BUG-2508, BUG-2441, BUG-1345, and BUG-1075. |
...
- Fix BUG-2508 by routing smart-HTTP pushes, Contents API commits, and Git Database branch-reference writes through one post-ref-update fan-out that automatically rebuilds configured legacy Pages sources.
