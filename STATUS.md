...
| Bugs | See [BUGS.md](BUGS.md) header (1835 filed · 1793 fixed · 2 open · 7 FP). 2 open: BUG-1075 (live-cloud), BUG-1345 (azuread upstream). |
| Live infra | None up. |
...
- **B. GitHub `actions/runner` cells on aca + azf — DONE (current branch).** Both backends green on the full bleephub container-job topology (container job + `services:` + dispatcher). GitHub+GitLab parity reached on all six container-capable backends.
- **C. Standing (NEXT)** — live-cloud pass (BUG-1075, biggest open gap), releases (#363), fresh sim fidelity audits.
