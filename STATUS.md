...
**104 phases closed. Phase 110 closed — all 4 cells GREEN. PR #123 (`phase-118-faas-pods`) bundles Phase 118 + 120 + 121 + 122. Phase 118 (FaaS pod overlays for gcf + lambda) code-complete. Phase 120 (4 GCP runner cells, docker executor — no k8s/GKE/ARC) code-complete; live runs pending operator. Phase 121 (cloud-faithful sim hardening — Cloud Build + IAM/token endpoints in the GCP sim, Azure mirror) NEXT — gates gcf integration tests passing in CI. Phase 122 (per-cloud github-runner-dispatcher: GCP + Azure variants) queued. Phase 119 (k8s shim) discarded. CI surfaced + closed BUG-887 (sim parseClause `:` operator) and BUG-891 (reverted backend EndpointURL bypasses BUG-888/889/890 — those violated the no-fakes rule; the right fix is on the simulator side per Phase 121).**
