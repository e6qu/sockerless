...
| | |
|---|---|
| Active branch | `feat/ratchet-up-7` — **ratchet up EC2 + Glue, the two biggest floors (BUG-2200).** ~248 ops in two rounds (4+3 agents over isolated pre-wired sub-registrar files), all spec-validated against the vendored `aws-sdk-go-v2` Smithy models (0 divergences). **EC2 122→270:** Transit Gateway (42), IPAM (31, real CIDR carving), Site-to-Site + Client VPN (35), Verified Access + Traffic Mirroring (41). **Glue 102→194:** ML Transforms + Data Quality (34), Sessions/Statements/Dev Endpoints/Blueprints (26), Catalog/Table Optimizer/BatchGets/Integrations (32). Fixed a validator false positive: the spec-shape validator wrongly applied the Smithy `jsonName` trait for awsJson1.1 (verified vs the aws-sdk-go-v2 Glue deserializer — awsJson ignores jsonName); it now accepts a field under the member name OR its jsonName. aws sim/sdk/cli build/lint(0)/unit green; contract + cli-shard + all conformance tests pass. |
...
| Bugs | See [BUGS.md](BUGS.md) header (2200 filed · 2156 fixed · 2 open · 14 FP). 2 open: BUG-1075 (live-cloud), BUG-1345 (azuread upstream) — both externally gated. |
| Live infra | None up. |

## What's next

- **AWS service-conformance ratchet (active arc).** The gate measures ~37 AWS services against the vendored `aws-sdk-go-v2` Smithy models; seven are at 100% (Batch, CloudTrail, CodeBuild, WAFv2, ECR, KMS, ELBv2). Keep ratcheting the big floors — **EC2 (270/769) and Glue (194/267)** have the most headroom — and drive more mid-size services to 100%. Resume steps in [DO_NEXT.md](DO_NEXT.md).
