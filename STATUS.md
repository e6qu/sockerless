...
| Branch purpose | Follow-up Bleephub GitHub-fidelity hardening after merged #784. The branch made repository deletion clean real Codespace runtime/workspace state before deleting repository records, and it hardened the AWS simulator SDK CI shard against hosted-runner disk exhaustion. |
| Current fixes | BUG-2468 and BUG-2469 were fixed on this branch. Repository deletion now removes backing Codespace containers and workspace directories through the same fail-loud path as direct Codespace deletion before it deletes repository state. Public REST, GraphQL, internal organization deletion, repository creation rollback, and template-generation rollback callers now surface cleanup failures instead of deleting only the SQLite row or silently orphaning runtime state. The `sim (aws sdk)` CI job now frees regenerable Go/Docker/apt caches before execution and runs the prebuilt AWS SDK test binary against the prebuilt simulator binary instead of rebuilding during the heavy shard. The branch also carried the deterministic README badge refresh commit generated by the pre-push hook. |
...
| Bugs | See [BUGS.md](BUGS.md) header: 2469 filed, 2425 fixed, 3 open, 16 false positives. Open: BUG-2441, BUG-1345, and BUG-1075. |
