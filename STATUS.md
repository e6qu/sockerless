...
| In-flight | None. |
| Planned next | Migrate the first public VPC/NIC or VM family onto the real-execution substrate for BUG-1267 / issues #332-#336. |
| Last merged | Real-execution network namespaces for issue #336. |
...
The real-execution network namespace PR advanced issue #336. The shared [simulators/realexec](simulators/realexec) network object now creates a dedicated Linux network namespace per simulated cloud network/VPC implementation object. Its bridge and gateway live inside that namespace, and attached NIC veth peers are moved into that namespace before being enslaved to the bridge. Guest namespaces still receive lease-based private IPs and default routes. The mandatory realexec host-network smoke test verifies the bridge is inside the network namespace, proves guest-to-gateway and guest-to-guest reachability with real packets, exercises nftables cleanup, and verifies guest and network namespaces are removed. Issues #332-#336 remain open because public EC2/GCE/Azure VM, VPC, security, NAT, and load-balancer API paths have not yet been migrated onto the substrate.
