...
| Branch purpose | Continued the Bleephub UI/API completeness audit after merged #791 across the release provider, parity audit, shared UI themes, repository organization/actions, clean browser viewer-state reads, a retained GitHub Classroom transition product, and real fine-grained personal access token settings and enforcement. |
| Current fixes | BUG-2512 through BUG-2522 and BUG-2524 through BUG-2534 were fixed on this branch except the still-open aggregate BUG-2523. Bleephub retained GitHub Classroom and added durable fine-grained personal access token creation, one-time credential disclosure, organization approval, GitHub App-only administration, resource/permission enforcement, secret scanning, official-client coverage, and saturated light/dark settings. The release editor also preserved uploaded assets across saved edits. BUG-2523 remained open for three operator-ingested domains. |
...
| Bugs | See [BUGS.md](BUGS.md) header: 2534 filed, 2489 fixed, 4 open, 16 false positives. Open: BUG-2523, BUG-2441, BUG-1345, and BUG-1075. |
...
- Replace BUG-2523's remaining three operator-ingested domains with real GitHub-compatible browser or producer workflows: GitHub Marketplace purchases, CodeQL database production, and hosted-compute network onboarding.
