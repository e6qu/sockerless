...
| | |
|---|---|
| Active branch | `feat/azure-ratchet-1` — **first Azure service ratchet (BUG-2224).** Now that the Azure coverage gate exists (#689), drive the first wave of Azure services up against it (the sim had a gate but no ratcheted services). Four agents, spec-validated against vendored ARM Swagger (0 new violations) + real azure-sdk-for-go: **Container Apps** (containerApps/jobs/managedEnvironments all 100%), **Container Registry** (2025-11-01 + 2023-07-01 + registrytasks all 100%), **Networking** (NSG/NAT-GW/PubIP-Prefix/RouteTable 100%; vnet 18/21, LB 22/27, NIC 14/15), **Service Bus + Event Hubs** (all 8 docs 100%; latent PascalCase casing bug fixed). **Azure 630→857/2597 (24%→33%); all three sims now actively ratcheting.** azure build/lint(0)/dupl(0) + route-validity + doc/spec-consumption + coverage-floor gates pass. Plus a CI-caught boyscout fix (BUG-2225): EC2 `RunInstances` now honors `ClientToken` idempotency so an SDK retry replays the original reservation instead of doubling the instance batch. |
...
| Bugs | See [BUGS.md](BUGS.md) header (2225 filed · 2181 fixed · 2 open · 16 FP). 2 open: BUG-1075 (live-cloud), BUG-1345 (azuread upstream) — both externally gated. |
| Live infra | None up. |
...
- **Coverage ratchets — all three sims now gated.** AWS (`serviceCoverageFloor`, Smithy), GCP (`gcpMethodFloor`, Discovery docs), and now Azure (`azureMethodFloor`, ARM Swagger) each have an operation-coverage ratchet. GCP is at 61% (3180/5244) with ~22 services at 100%; Azure ratcheting has started — 630→857/2597 (24%→33%) with Container Apps/Container Registry/Service Bus/Event Hubs at 100%. Next candidates: keep ratcheting Azure (Storage, Cosmos DB, Redis, Key Vault, remaining networking, EventGrid, APIM) and GCP big surfaces (Logging, Bigtable/Cloud Run/CRM remainders, Compute selectively). Live-cloud (BUG-1075) remains the biggest externally-gated gap.
