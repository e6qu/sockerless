...
| Branch purpose | Follow-up Bleephub GitHub-fidelity hardening after merged #784. The branch made repository deletion clean real Codespace runtime/workspace state before deleting repository records, hardened the AWS simulator SDK CI shard against hosted-runner disk exhaustion, and made persisted Bleephub require object storage for durable service byte content. |
| Current fixes | BUG-2468, BUG-2469, BUG-2470, and BUG-2471 were fixed on this branch. Repository deletion now removes backing Codespace containers and workspace directories through the same fail-loud path as direct Codespace deletion before it deletes repository state. Public REST, GraphQL, internal organization deletion, repository creation rollback, and template-generation rollback callers now surface cleanup failures instead of deleting only the SQLite row or silently orphaning runtime state. The `sim (aws sdk)` CI job now frees regenerable Go/Docker/apt caches before execution and runs the prebuilt AWS SDK test binary against the prebuilt simulator binary instead of rebuilding during the heavy shard. Persisted Bleephub startup now requires initialized S3-compatible object storage for GitHub Actions artifacts, dependency caches, runner logs, release assets, GitHub Packages files, and GitHub Container Registry blobs instead of accepting memory or local files as a durable byte backend. Release asset object-delete failures now surface as API/repository-delete errors. The branch also carried the deterministic README badge refresh commit generated by the pre-push hook. |
...
| Bugs | See [BUGS.md](BUGS.md) header: 2471 filed, 2427 fixed, 3 open, 16 false positives. Open: BUG-2441, BUG-1345, and BUG-1075. |
