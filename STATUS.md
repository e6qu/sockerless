...
| | |
|---|---|
| Active branch | `feat/ratchet-up-12` — **drive EC2 + SSM to 100% + ratchet CloudWatch Logs + S3, gate DynamoDB (BUG-2207).** ~291 ops in two rounds (6+5 agents), all spec-validated against the vendored `aws-sdk-go-v2` Smithy models (0 divergences). **EC2 600→769/769 (100%)** and **SSM 43→146/146 (100%)** — the two biggest floors fully closed. **CloudWatch Logs 104→111** (2 streaming ops left), **S3 91→103** (2 event-stream/Object-Lambda left). **DynamoDB gated 57/57** + **Amplify 37/37** confirmed complete. **Twenty-one AWS services now at 100%.** Integration: added the EC2 `DescribeMacModificationTasks` op (both owning agents had deferred it to each other), wired the S3 method×subresource measurement matrix into `s3ImplementedOps`, implemented SSM's final `DeleteOpsItem`. aws sim/sdk/cli build/lint(0)/unit green; contract + cli-shard + all conformance tests pass. |
...
| Bugs | See [BUGS.md](BUGS.md) header (2208 filed · 2164 fixed · 2 open · 16 FP). 2 open: BUG-1075 (live-cloud), BUG-1345 (azuread upstream) — both externally gated. |
| Live infra | None up. |

## What's next

- **AWS service-conformance ratchet (active arc).** The gate measures ~37 AWS services against the vendored `aws-sdk-go-v2` Smithy models; twenty-one are at 100% (Batch, CloudTrail, CodeBuild, WAFv2, ECR, KMS, ELBv2, Lambda, EFS, STS, EventBridge Scheduler, Cloud Map, ElastiCache, Auto Scaling, Route 53, API Gateway v1, API Gateway v2, RDS, CloudFront, EC2, SSM) — plus DynamoDB + Amplify (gated complete). With EC2 + SSM closed, the next big floors are SSM-adjacent mid-size services: CloudWatch Logs (111/113, the rest is HTTP/2 streaming), S3 (103/107, the rest is event-stream/Object-Lambda), and unmeasured/low services — Glue is 264/267 (its faithful max). Resume steps in [DO_NEXT.md](DO_NEXT.md).
