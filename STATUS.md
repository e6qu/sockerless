...
| | |
|---|---|
| Active branch | `feat/gcp-console-real-federation` |
| Branch purpose | The Google Cloud console stopped reading the sockerless-invented `/sim/v1/cloudrun/jobs` endpoint and read the real Cloud Run Admin API instead — the `/v2/.../jobs` list and a job detail page — rendering the true resource. A real console reaches those APIs with a credential federated from the signed-in session, so the simulator gained the Security Token Service token exchange (Workforce Identity Federation) and a console credential broker that exchanges the operator's Shauth assertion for a federated access token. The invented endpoint was deleted; the credential issuance advanced BUG-2625. |
