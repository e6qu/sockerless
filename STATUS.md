...
|---|---|
| Active branch | `main` |
| Branch purpose | Bleephub deployed as an independently managed, production-like Amazon Elastic Container Service on AWS Fargate stack with private networking, Amazon Simple Storage Service git/object persistence, native dqlite voters, an internal Network Load Balancer, Amazon API Gateway wake routing, and real GitHub-compatible UI/API/Git transports. CI validated pull requests and post-merge `main` commits. |
| Current fixes | Bleephub now has a fully baked ARM64 release image, GitHub OAuth and local administrator identity, production Git Smart HTTP and SSH transports, a private administrator origin, and an idle controller that safely returns application and dqlite tasks to zero after five minutes without requests. The Amazon Simple Storage Service git bucket has versioning suspended and removes noncurrent versions. The real-browser harness starts an SSH listener with an ephemeral host key and validates port-aware clone URLs; the embedded user interface serves its branded favicon. CI runs on pull requests and `main`; repository Go modules meet the dependency-freshness gate. |
| Last merged (#793) | **Deploy Bleephub on ECS Fargate**. It deployed the standalone Bleephub production stack, real Git transports and browser identity, scale-to-zero dqlite wake/recovery, the private administrator origin, and CI/runtime hardening. |
...
| Bugs | See [BUGS.md](BUGS.md) header: 2578 filed, 2532 fixed, 9 open, 16 false positives. The local Amazon Elastic Container Service Terraform simulator apply/destroy hang remained tracked as BUG-2569. |
