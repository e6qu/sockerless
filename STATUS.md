...
- **Phase 104** — cross-backend driver framework. **Framework migration complete + cloud-native typed drivers shipped across every backend.** All 13 adapters; every dispatch site flows through TypedDriverSet. Cloud-native: 6 cloud backends use typed Logs + Attach (CloudWatch / Cloud Logging / Azure Monitor); ECS has typed ProcList/FSDiff/FSRead/FSWrite/FSExport/Signal via SSM; FaaS+CR+ACA (5 backends) have typed ProcList/FSDiff/FSRead/FSWrite/FSExport via reverse-agent + typed Exec via WrapLegacyExec wrapping ReverseAgentExecDriver. Remaining cloud-native slots: Commit/Build/Registry/Stats typed cloud paths. Wrapper-removal pass + interface tightening lined up post-migration.
