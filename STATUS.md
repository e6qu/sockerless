...
| Branch purpose | Follow-up Bleephub storage hardening after merged #786. The branch moved CodeQL variant-analysis query-pack tarballs out of SQLite metadata and into the configured S3-compatible object byte store, made controller-repository deletion fail loudly before metadata deletion when required query-pack object cleanup fails, and made GitHub Actions runner-log upload/deletion keep live process state consistent when required object-store operations fail. |
| Current fixes | BUG-2489 and BUG-2490 were fixed on this branch. CodeQL variant-analysis rows now persist controller, actor, language, target, status, query-pack size, and object-key metadata while uploaded query-pack tarballs live in object storage whenever object storage is configured. Public query-pack downloads read the object store, persistent stores fail loudly without `BLEEPHUB_OBJECT_S3_BUCKET`, and controller-repository deletion purges query-pack objects before deleting repository metadata. Runner-log upload and deletion now complete required object-store writes/deletes before changing in-memory log, console, or timeline state, so fail-loud object-store errors preserve the previous live state. |
...
| Bugs | See [BUGS.md](BUGS.md) header: 2490 filed, 2446 fixed, 3 open, 16 false positives. Open: BUG-2441, BUG-1345, and BUG-1075. |
