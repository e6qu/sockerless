...
| | |
|---|---|
| Active branch | `fix/publish-arch-image-manifests` |
| Branch purpose | Native release jobs published each `-arm64` and `-amd64` tag as a direct OCI image manifest without provenance attestations, verified both child media types, and assembled a generic short-SHA OCI index containing exactly Linux ARM64 and AMD64. |
...
- Merge the direct architecture-manifest publication fix, publish the corrected short-SHA images, and deploy the updated Admin and simulator images in the shared development environment. Register each callback, post-logout, and back-channel logout coordinate in Shauth while preserving the simulators' native cloud protocol contracts.
