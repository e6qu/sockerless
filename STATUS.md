...
**Date: 2026-05-04**. **CELL 7 GREEN** — gitlab-runner × Cloud Run Service. Pipeline https://gitlab.com/e6qu/sockerless/-/pipelines/2496721473 (job 14192454186, 1020s, status: success). Real workload proven: postgres TCP probe + psql SELECT version() returned PostgreSQL 16.13, all 6 environment probes ran, git clone succeeded, golang:1.25-alpine compiled eval-arithmetic, all 5 arithmetic results correct (11/14/21/13/6.5).

12-step BUG-925 fix shipped: docker-network → Cloud Run multi-container Service mapping with sidecar persistence across per-stage script-runners. Pure standard-Docker signal: `NetworkingConfig.EndpointsConfig.<net>.Aliases` + `Container.Config.OpenStdin` (no runner-specific labels). Runner-side knobs: `FF_NETWORK_PER_BUILD=true` + `wait_for_services_timeout=-1` in gitlab-runner config.toml.

Remaining cells:
- **Cell 5/6 (github runner)**: blocked on GitHub PAT refresh — dispatcher polls return 403 Forbidden since `2026-05-04T00:15Z`. Workflow_run was queued (run 25294748922) but dispatcher can't see it. User action needed: `gh auth refresh` + `gcloud secrets versions add github-pat --data-file=-` with new PAT.
- **Cell 8 (gitlab × gcf)**: blocked on BUG-923 — gcf ContainerCreate calls CreateFunction.Wait synchronously which exceeds gitlab-runner's 120s docker daemon timeout. Fix needs deferring CreateFunction to ContainerStart (parallels cloudrun model); requires moving 100+ lines of overlay+function-create logic. Attempted but reverted due to cell 7 priority. Re-apply after cell 5 unblocks (since cell 5 uses same backend as cell 7 = should work immediately with PAT refresh).
