...
| | |
|---|---|
| Active branch | `feat/gcp-console-remaining-real-apis` |
| Branch purpose | The Google Cloud console reached the cloud only through real cloud APIs at configured coordinates, with no simulator-served endpoint on the data plane. The `/auth/cloud-token` credential broker and its sim-side workforce-provider auto-provisioning were deleted; the browser now reads the operator's Shauth assertion from the console's own auth layer (`/auth/federation-subject`) and federates it at the real Security Token Service (`POST /v1/token`), then calls the real cloud APIs — Security Token Service, cloud API base, and workforce audience all coordinates that are the console's own origin when embedded and Google Cloud otherwise. The Shauth auth layer stayed server-side (session, front/back-channel logout, marker contract), and the workforce provider was provisioned the administrator's way through the real Identity and Access Management API. So a simulator console UI differs from a real-cloud console only in coordinates. |
