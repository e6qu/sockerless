...
| **8** GL × gcf | sockerless-gcf | 🟡 **v20 = VpcAccess + ALL_TRAFFIC fix in flight** | digest `sha256:72d6cd93`. Today's iteration log: v9-v18 surfaced + fixed AR HEAD precheck, multi-container Service direct deploy, PendingCreates speculative-running marker, resolvePodServiceFromCloud GetService follow-up, stdinPipe + attachStream pattern, relaxed ContainerAttach gating, 5 s pre-check window, OpenStdin runner-pattern (skip default-invoke, keep container alive). **v20 hypothesis** (post-state-save): gcf's materializePodService + deployContainerService were missing the VpcAccess + ALL_TRAFFIC block that cloudrun has — without it Cloud Run rejects gitlab-runner-gcf's POST to sockerless-svc-* as external (BUG-933 documented this exact failure for cloudrun). Added `Config.VPCConnector` field + wired to RevisionTemplate.VpcAccess; YAML gets `SOCKERLESS_GCF_VPC_CONNECTOR=projects/.../connectors/sockerless-connector`. v20 trigger pending — quota-permitting (today's iterations consumed regional CPU). |
