...
| | |
|---|---|
| Active branch | `feat/gcp-simulator-console-parity` |
| Branch purpose | The Google Cloud simulator presented as the Google Cloud console: the light global header with a project chip and a wide central search, a product navigation whose active item is a filled pill, inline text actions beside the page title, a filter chip above a table whose headers carry inline help, and an illustrated empty state that names the side effect of its primary action. With this the three simulator interfaces each present as their own cloud's console. Both themes were carried with the control in the top right, and contrast was measured against the rendered surfaces by a test that holds it. |
