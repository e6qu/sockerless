...
| | |
|---|---|
| Active branch | `fix/shauth-validation-contract` |
| Branch purpose | Sockerless Admin and all three simulator dashboards implemented Shauth's exact release-aware authentication validation contract, and a clean real PostgreSQL/Ory Hydra/Shauth/Chromium matrix proved direct and catalog entry, relying-party and provider logout, local signed-out return, reauthentication, anonymous fail-closed behavior, and validator-credential isolation in eight serialized flows. |
...
| Operator identity | `sockerless-admin` supports optional Shauth OpenID Connect browser sign-in, `client_secret_post`, exact issuer validation, bounded cached provider discovery, server-tracked relying-party sessions signed by an independent 32-byte-or-longer secret, front- and back-channel logout, and RP-initiated global logout to its public no-cache signed-out page. Only a session carrying the Shauth `admin` role enters the operator UI or APIs; authenticated developers receive an explicit no-cache `403` and can sign out. Its standard `/auth/validation` surface reports verified username, email, role, and immutable release while anonymous or bearer-only requests fail closed. Explicit HTTP development coordinates are limited to loopback hosts. Its unauthenticated `GET /healthz` liveness route supports infrastructure probes without exposing protected console state. |
...
| Simulator identity | The three dashboards share first-party OpenID Connect authorization-code + PKCE, exact issuer validation, signed server-tracked sessions, identity, RP-Initiated Logout, strict same-origin controls, front-channel revocation, and form-only signed back-channel revocation with atomic replay rejection. Each standard `/auth/validation` surface reports verified username, email, role, and immutable release, rejects bearer credentials, and returns anonymous browsers to its own signed-out page. Their HTML and dashboard-data routes share the same authorization boundary while health checks and native cloud API slices remain unchanged. |
...
| Nested workload host callbacks | A containerized AWS simulator propagated the outer runtime's resolved `host.docker.internal` or `host.containers.internal` IPv4 address into nested workloads and used the default route only when neither alias existed. The real Bleeplab Amazon ECS runner reached its Git origin from a nested task and completed its compiled job, artifact, and Redis service-network pipeline. |
...
| Dependency freshness | The required freshness gate inspects only Git-tracked Go modules, Terraform providers, and GitHub Actions; the current AWS Organizations SDK, Google Cloud Secret Manager, and `actions/checkout` releases were pinned across every consumer. Shauth's standalone validator was built with its own module and the Go toolchain declared by the pinned Shauth `go.mod`, even when continuous integration checked it out beneath Sockerless's workspace. |
...
| Latest merged pull request | [#811 Make signed-out Shauth re-entry explicit](https://github.com/e6qu/sockerless/pull/811) exposed an accessible, first-party `Sign in with Shauth` action on every Sockerless relying party's terminal signed-out page. |
...
- Merge this branch, publish its Admin and simulator images, deploy them in the shared development environment, and run the same eight-direction release-aware validation matrix against the live origins.
