...
| | |
|---|---|
| Active branch | `feat/gcp-ratchet-2-azure-gate` — **GCP coverage ratchet round 2 + build the Azure operation-coverage gate (BUG-2220).** Built `azureMethodFloor` (Swagger-spec analogue of the AWS/GCP coverage ratchets — the last sim without one; Azure measured at 630/2597, 24%, 90 swagger files locked). **All three sims now have an operation-coverage ratchet.** GCP round: Spanner/Cloud SQL v1/VPC Access/ServiceUsage/IAM Credentials to 100%; API Gateway 57/60, Cloud Functions 31/42, SQL v1beta4 146/148, CRM v3 11→105/124, Cloud Run v1 61→98 + v2 51→62; plus the big-surface wave: Dataflow 8→84/84, Bigtable 65→136/162, Logging 170→480/508. **GCP 2413→3180/5244 (46%→61%); ~22 services at 100%.** Every service spec-validated (0 new violations) + real Google Cloud Go SDK. gcp+azure build/lint(0) green; route-validity + doc/spec-consumption + coverage-floor gates pass. Plus a CI-resilience fix (BUG-2221): the three smoke Dockerfiles now retry `go mod download` (a transient proxy.golang.org INTERNAL_ERROR had failed the smoke ecs build). |
...
| Bugs | See [BUGS.md](BUGS.md) header (2221 filed · 2177 fixed · 2 open · 16 FP). 2 open: BUG-1075 (live-cloud), BUG-1345 (azuread upstream) — both externally gated. |
| Live infra | None up. |
