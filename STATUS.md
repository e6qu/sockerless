...
| Last merged | Firecracker-backed VM lifecycle for AWS EC2, GCP Compute Engine, and Azure VMs, plus the CI/ASG follow-up that kept VM-backed simulator jobs on KVM-capable runners. |
...
| Bugs | 1301 filed - 1300 fixed - 4 open - 2 false positives. |
...
The CI follow-up for that phase fixed the failed PR run without skips, fallbacks, or weakened tests. The simulator SDK/CLI/Terraform jobs that exercise VM-backed public APIs now run on `ubuntu-latest`, the hosted runner class that exposes `/dev/kvm` and already passed the mandatory Firecracker arithmetic smoke. The prior `ubuntu-24.04-arm` simulator runners installed Firecracker successfully but lacked `/dev/kvm`, so EC2/GCP/Azure VM public API tests failed loudly or retried through Terraform until the step timeout. The GCP simulator job now installs the matching x86_64 gcloud CLI package. Simulator test harnesses build their workload images for the runner-native Linux Docker platform (`linux/amd64` on the KVM CI runner, `linux/arm64` on arm hosts) instead of forcing arm64 images onto x64 runners without emulation. Simulator workload execution paths no longer hardcode arm64 capacity: GCP Cloud Functions/Cloud Run Jobs, AWS ECS, and Azure Functions use the resolved local image platform, while AWS Lambda translates the public `Architectures` field to Docker's platform strings. Firecracker rootfs image creation now creates and sizes `rootfs.ext4` explicitly, and default workdirs are unique per launch under the instance ID. AWS Auto Scaling Groups also stopped creating metadata-only `running` EC2 records: ASG scale-out now starts the same real EC2 Firecracker guest lifecycle as `RunInstances`, and scale-in stops the guest before terminating the instance/ENI/volume state.
