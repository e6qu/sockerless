...
| | |
|---|---|
| Active branch | `feat/gcp-ratchet-1` — **GCP: build the operation-coverage gate + ratchet 12 mid-size services (BUG-2214).** GCP had route-validity + doc-consumption gates but **no operation-coverage ratchet** (the AWS-style `serviceCoverageFloor` for Discovery docs). Built `gcpMethodFloor` (per-Discovery-doc implemented-method count, exact-equality ratchet) and ratcheted 12 mid-size services across 2 rounds (one service file per agent): **Cloud Build, Redis, Firestore admin, Cloud Storage JSON all to 100%**; Cloud KMS 122→157 (real Go-stdlib crypto), IAM admin 204→264, Artifact Registry 97→144, Eventarc 97→124, BigQuery 38→86, Cloud DNS 24→74 (real DNSSEC), Pub/Sub 77→86, Secret Manager 50→60. **GCP coverage 1986→2413/5244 (38%→46%); 6 GCP services at 100%.** Every service spec-validated against its Discovery schema (0 new violations) + real Google Cloud Go SDK round-trips. gcp sim build/lint(0) green; route-validity + doc-consumption + coverage-floor gates pass. |
...
| Bugs | See [BUGS.md](BUGS.md) header (2214 filed · 2170 fixed · 2 open · 16 FP). 2 open: BUG-1075 (live-cloud), BUG-1345 (azuread upstream) — both externally gated. |
| Live infra | None up. |
