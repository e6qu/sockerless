...
| 7 GL × cloudrun | sockerless-cloudrun | 🟡 OLD-arch v49 GREEN 2026-05-03 (pipeline 2496721473, custom runner image — **rejected by user 2026-05-04 pivot**); NEW-arch v50 today (pipeline 2498952453) fetched gitlab.com OK after connector scale-up 2→4, then **hung at git checkout** because GCSFuse `/builds` mount lacks POSIX hard-link / flock / cross-dir rename → BUG-947. | Replace GCSFuse for `/builds` per BUG-947 — recommended path A: in-memory `emptyDir` + one Cloud Run Service revision per gitlab-runner job. |
