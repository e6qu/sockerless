...
| **8** GL × gcf | sockerless-gcf | 🟡 **v17 stdinPipe shipped, gitlab-runner silent post-start** | v17 (rev `00050-p25`, digest `sha256:37e65d40`): stdinPipe + attachStream pattern ported from cloudrun. HTTP middleware bumped to Info to capture every request. **Confirmed**: ContainerStart(build) blocks 19.5 s through materialize, returns 204; invokePodServiceMain goroutine enters; **NO stdinPipe registered** at that moment so default-invoke fires (POSTs user entrypoint+cmd). After ContainerStart returns, gitlab-runner makes ZERO HTTP calls to sockerless for 15+ min. Heartbeats to gitlab.com continue. **Open**: cell 7 (cloudrun) has identical stdinPipe code yet works GREEN — gitlab-runner must call ContainerAttach BEFORE start in cell 7 but not in cell 8. Need to inspect cell 7 logs to find the difference, OR investigate whether the v17 ContainerAttach delegate registers correctly when called pre-start (we set `hasSockerlessOverlayRepoGCF` check — image at attach time may not yet be the overlay URI since materialize hasn't run). |
