...
| In-flight | **Phases 167 + 168 on the same branch.** P168.1–.8 landed; P168.9/CI hardening is in progress. Latest pushed CI run `26016144638` is green. Current local chunk: AZF's bootstrap now accepts exec envelopes, AZF attach is wired through the backend-specific handler, and `TestAZFContainerExec` plus `TestAZFGitLabRunnerAttachStdin` verify real reverse-agent exec + attach-before-start stdin delivery against the simulator. Previous pushed chunk fixed BUG-1067/1071 with `agent/cmd/sockerless-azf-bootstrap`, ACR overlay materialization, simulator HTTP Function invocation, and SIGTERM lifetime-expired signaling. Remaining: continue with runner/live/test-pyramid follow-ups 1072–1076. External Claude review through the local CLI was denied by the escalation reviewer; user can run it locally and paste results. |
