...
| | |
|---|---|
| Active branch | `fix/admin-role-gate` |
| Branch purpose | Sockerless Admin required the Shauth administrator role at its shared UI and API boundary, returned explicit no-cache denials to authenticated developers, and proved both roles through focused real topology persistence plus the full Shauth/PostgreSQL/Ory Hydra/Chromium matrix. |
...
| Latest merged pull request | [#809 Enforce Shauth SSO and harden ECS task generations](https://github.com/e6qu/sockerless/pull/809) completed the first-party relying-party matrix and corrected reused Amazon ECS attached-container generations. |
...
- Merge this branch, publish its administrator-gated Admin image, and deploy the updated image in the shared development environment before re-running the live role matrix.
