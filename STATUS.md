...
| Branch purpose | Continued the Bleephub UI/API completeness audit after merged #791 across the release provider, parity audit, shared UI themes, repository organization/actions, clean browser viewer-state reads, a retained GitHub Classroom transition product, fine-grained personal access token settings, and the real GitHub CodeQL producer and Security workflow. |
| Current fixes | BUG-2512 through BUG-2522 and BUG-2524 through BUG-2547 were fixed on this branch except the still-open aggregate BUG-2523. Bleephub retained GitHub Classroom; added durable fine-grained personal access tokens; implemented official CodeQL Action database upload/finalization, GitHub App producer authorization, real SARIF coordinates, atomic object-backed replacement, and complete multi-run/zero-finding analyses; and organized the account and Code Security browser workflows with saturated, legible light/dark themes. BUG-2523 remained open for two operator-ingested domains. |
...
| Bugs | See [BUGS.md](BUGS.md) header: 2547 filed, 2502 fixed, 4 open, 16 false positives. Open: BUG-2523, BUG-2441, BUG-1345, and BUG-1075. |
...
- Replace BUG-2523's remaining two operator-ingested domains with real GitHub-compatible browser or producer workflows: GitHub Marketplace purchases and hosted-compute network onboarding.
