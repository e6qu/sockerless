...
| | |
|---|---|
| Active branch | `feat/bleeplab-cloudrun-cell` (PR pending). |
| In-flight | **bleeplab GitLab cell on the Cloud Run backend — GREEN.** Extended the one-image, `BLEEPLAB_BACKEND`-switched harness (bleeplab/Dockerfile + run-integration.sh + Makefile) to run the full gitlab-runner docker-executor flow on cloudrun: a 3-stage pipeline (build → artifact → `services:`) passes — gcc-compiled `calc.c` (self-test, 6×7=42, sum 5050), cross-stage artifact passing, and redis reached by alias over the per-build pod network. Validation surfaced + fixed 3 real bugs: **BUG-1808** cloudrun reported a hardcoded `amd64` arch (gitlab-runner picked the wrong helper on arm64) → derive from `BuildPlatform`; **BUG-1809** the gcp sim's AR pull-through only hydrated `docker-hub`, not `gitlab-registry` (the gitlab-runner-helper 404'd) → added the `gitlab-registry`→`registry.gitlab.com` mapping; **BUG-1810** the gcp sim's Cloud Run Service invoke dialed `127.0.0.1:hostPort`, unreachable when the sim runs inside the harness container → reach the workload by bridge container IP, fall back to the host port. The CI markers are now backend-neutral so one harness covers all backends. |
...
- **A. bleeplab GitLab cells on gcf + aca (NEXT, own PRs).** The ECS **and cloudrun** cells are green. **gcf** reuses the gcp sim + the same `provision_*` skeleton (watch **BUG-964** — the gcf default-invoke gating for the multi-container pod-Service, which the redis `services:` job exercises); **aca** adds the azure sim + ACR + the `*.blob.localhost` hosts alias. Each is its own validation-heavy PR (overlay build→push→pull→reverse-agent).
