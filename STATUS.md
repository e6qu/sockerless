...
| Branch purpose | Follow-up Bleephub GitHub-fidelity hardening after #783. The branch made repository deletion purge persisted issue and pull request child state, repository-ID keyed rows, and selected-repository references so deleted repositories do not leave durable rows or access grants that can attach to later ID reuse. |
| Current fixes | BUG-2457 and BUG-2458 were fixed on this branch. Repository deletion now purges persisted issue comments, issue events, sub-issue links, issue dependency links, pull request reviews, pull request review comments, artifact attestations, repository activity, clone traffic, watch subscriptions, selected-repository allowlists, GitHub App selected repositories, installation token repository scopes, and code-security attachments for the deleted repository. The branch also carried the deterministic README badge refresh generated by the pre-push hook. |
...
| Bugs | See [BUGS.md](BUGS.md) header: 2458 filed, 2414 fixed, 3 open, 16 false positives. Open: BUG-2441, BUG-1345, and BUG-1075. |
