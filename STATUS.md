...
| 7 GL × cloudrun | sockerless-cloudrun | 🟡 OLD-arch v49 GREEN 2026-05-03 (pipeline 2496721473, custom runner image — **rejected by user 2026-05-04 pivot**); NEW-arch v50 (pipeline 2498952453) connector fix unblocked git fetch but **GCSFuse is ~200× slower than tmpfs for git ops** (211s vs 1s for sockerless clone, verified 2026-05-04 22:42 UTC) → 10-min HTTP exec timeout fires → `Job failed`. **BUG-947 only practical fix is Path B: Cloud Filestore (NFS).** Path A (emptyDir + single-Service-per-job) ruled out — Cloud Run revisions are immutable, each modification spawns a new instance with fresh emptyDir. | Provision 1 TB Filestore Zonal (~$160/mo / $5.30/day for spot tests) + swap `Volume_Gcs` → `Volume_Nfs` in `backends/cloudrun/{jobspec.go,servicespec.go}` and `backends/cloudrun-functions/volumes.go`. Awaiting $$ approval. |
