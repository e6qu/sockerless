...
| | |
|---|---|
| Active branch | `fix/elbv2-tcp-healthcheck-fields` — **ELBv2 TCP target group still returned HealthCheckPath (#688, BUG-2222).** Follow-up to #685: that fix omitted the HTTP-only `Matcher` for TCP health checks, but `HealthCheckPath` is the same HTTP/HTTPS-only class and was still leaking (`CreateTargetGroup` defaulted it to `/` for every protocol; the XML emitted it unconditionally) → perpetual `terraform-provider-aws` `health_check.path` drift on TCP target groups. Generalized the predicate (`elbv2MatcherApplies` → `elbv2HTTPHealthCheck`) + a `elbv2DefaultedHealthCheckPath` helper: path defaults to `/` only for HTTP/HTTPS, the XML emits it only for HTTP/HTTPS, and Modify clears both Matcher + path on a change to a non-HTTP health check. SDK + CLI fidelity tests extended; the idempotency-fidelity TF stack's TCP target group gained a TCP `health_check` block (real apply + clean plan, 127s). Boyscout: confirmed `ProtocolVersion` already omitted for TCP. aws build/lint(0) + conformance + spec-validator(0) green. Closes #688. |
...
| Bugs | See [BUGS.md](BUGS.md) header (2222 filed · 2178 fixed · 2 open · 16 FP). 2 open: BUG-1075 (live-cloud), BUG-1345 (azuread upstream) — both externally gated. |
| Live infra | None up. |
