...
| Branch purpose | Continued the Bleephub UI/API completeness audit after merged #791 across the release provider, parity audit, shared UI themes, repository organization/actions, and clean browser viewer-state reads. |
| Current fixes | BUG-2512 through BUG-2522 and BUG-2524 through BUG-2526 were fixed on this branch. Release workflows became routed, git/object-backed, repository-scoped, event-complete, race-safe, and official-client covered. The parity specification now states the verified proof boundary and remaining gaps. Shared light/dark chrome uses GitHub/Primer structure with saturated brand colors; repository pages use full-width context chrome, real Watch/Star toggles, and owner-selecting Fork creation; expected viewer-state absence no longer emits browser errors; and light/dark browser tests became independent. BUG-2523 remained open for five operator-ingested producer/browser workflows. |
...
| Bugs | See [BUGS.md](BUGS.md) header: 2526 filed, 2481 fixed, 4 open, 16 false positives. Open: BUG-2523, BUG-2441, BUG-1345, and BUG-1075. |
...
- Replace BUG-2523's five operator-ingested domains with real GitHub-compatible browser or producer workflows: GitHub Classroom, GitHub Marketplace purchases, CodeQL database production, hosted-compute network onboarding, and fine-grained personal access token creation.
