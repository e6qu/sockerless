...
| | |
|---|---|
| Active branch | `feat/azure-ratchet-2` — **second Azure service ratchet (BUG-2226).** Four agents (one service-set each, isolated worktrees, integrated by merge + one combined measured pass), every op spec-validated against vendored ARM Swagger (0 new violations) + real azure-sdk-for-go: **Storage ARM** (storage 6→44/49; blob/file/queue/table all 100%; fixed an undeclared `provisioningState` on FileShareProperties + a `KeyPermission` enum casing bug), **Networking** (DNS/Private DNS/Load Balancer/NIC/Public IP/VNet all 100%), **Redis / Key Vault / Managed Identity** (all 100%; fixed Redis CheckNameAvailability shape + a patch-schedule schema leak), **Container Instances** (100%) **+ RBAC** (role assignments/definitions up). **Azure 857→1000/2597 (33%→38%).** azure build/lint(0)/deadcode(0)/dupl(0) + route-validity + doc/spec-consumption + coverage-floor + spec-validator(0) + contract-hook gates all pass on the merged sim. |
...
| Bugs | See [BUGS.md](BUGS.md) header (2227 filed · 2183 fixed · 2 open · 16 FP). 2 open: BUG-1075 (live-cloud), BUG-1345 (azuread upstream) — both externally gated. |
| Live infra | None up. |
