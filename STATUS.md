...
| | |
|---|---|
| Active branch | `fix/elbv2-nlb-stable-dnsname` — **ELBv2 NLB DescribeLoadBalancers returned proxy host:port as DNSName (#691, BUG-2223).** Regression from #683: making the NLB raw-TCP data plane reachable, `elbv2ReportedDNSName` overrode the stable AWS-shaped `DNSName` with the proxy's ephemeral `host:port` → perpetual `terraform-provider-aws` `dns_name` drift + invalid Route53 alias target. Fixed by making reachability faithful instead of hijacking DNSName: DescribeLoadBalancers always returns the stable `<name>-<id>.elb.<region>.amazonaws.com` hostname; the NLB proxy binds the **listener port** on a stable host (`127.0.0.1`, or a per-NLB `127.0.0.0/8` IP on Linux via the new `realexec.ReserveNLBLoopbackIPv4`), and the hostname resolves to it via injected workload `ExtraHosts` (the production consumer of `elbv2NLBProxyAddress`, keeping deadcode green). CreateListener best-effort on bind. #691 contract + #683 capability (faithful path) + a `network` LB with Route53 alias in the idempotency TF stack (real apply + clean plan, 151s). aws+realexec build/lint(0)/deadcode(0) + conformance + spec-validator(0) green. Plus a CI flakiness fix: the AWS CLI `appdata` shard (~20 service prefixes in a 20-min budget — it timed out once on this PR) was split into two balanced 10-prefix shards (appdata + appdata2) for headroom + parallelism; the cli-shard-coverage guard confirms every test still maps to exactly one shard. |
...
| Bugs | See [BUGS.md](BUGS.md) header (2223 filed · 2179 fixed · 2 open · 16 FP). 2 open: BUG-1075 (live-cloud), BUG-1345 (azuread upstream) — both externally gated. |
| Live infra | None up. |
...
- **Coverage ratchets — all three sims now gated.** AWS (`serviceCoverageFloor`, Smithy), GCP (`gcpMethodFloor`, Discovery docs), and now Azure (`azureMethodFloor`, ARM Swagger) each have an operation-coverage ratchet. GCP is at 61% (3180/5244) with ~22 services at 100%; Azure is freshly gated at 630/2597 (24%). Next candidates: keep ratcheting GCP big surfaces (Logging 508, Bigtable 162, Dataflow 84, Cloud Run remainder, CRM remainder) and **start ratcheting Azure services** now that its gate exists (Container Apps/Jobs, Service Bus, Event Hubs, Storage, Cosmos DB, networking). Live-cloud (BUG-1075) remains the biggest externally-gated gap.
