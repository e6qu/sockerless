...
| Current fixes | BUG-2502 was fixed on this branch. GitHub Pages deployments now retrieve the supplied artifact URL or repository-owned GitHub Actions artifact bytes before reporting success, require configured object storage for object-backed artifacts, reject metadata/byte-size mismatches without mutating Pages state, and retain the deployed byte count and SHA-256 digest. Coverage drove both accepted artifact forms through Bleephub's real artifact download data plane and S3-compatible object storage. |
...
| Bugs | See [BUGS.md](BUGS.md) header: 2502 filed, 2458 fixed, 3 open, 16 false positives. Open: BUG-2441, BUG-1345, and BUG-1075. |
