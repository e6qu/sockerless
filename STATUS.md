...
| Branch purpose | Follow-up Bleephub fidelity hardening after merged #787. The branch made persisted repository ownership strict, removed internal runner-submission image fallbacks, kept container-package coverage on the real GitHub Container Registry-compatible data plane, and made Projects v2 GraphQL owner resolution fail loudly. |
| Current fixes | BUG-2491, BUG-2492, BUG-2493, and BUG-2494 were fixed on this branch. Persisted repository reload now requires valid `owner_type` and `owner_id`, validates organization-owned repositories against loaded organization state, and no longer treats empty owner types as user repositories in public listing/event paths. Internal job and workflow submission routes now require either explicit `image` or `hostMode`, and tests pass explicit images when they intend container execution. Container package fixtures now publish through the GitHub Container Registry-compatible OCI/Docker Registry HTTP API v2 routes; source coverage rejects new internal container-package seed calls; and `/internal/packages` rejects `container` package creation so container packages have one real publish path. GraphQL `createProjectV2` now requires `ownerId` to resolve to a real user or organization GitHub node ID and preserves project state when owner resolution fails. |
...
| Bugs | See [BUGS.md](BUGS.md) header: 2494 filed, 2450 fixed, 3 open, 16 false positives. Open: BUG-2441, BUG-1345, and BUG-1075. |
