...
| | |
|---|---|
| Active branch | `feat/azure-simulator-portal-parity` |
| Branch purpose | The Azure simulator presented as the Azure portal: the blue global header with its central search, a breadcrumb, a resource title carrying type and directory, a horizontal command bar, a service menu with its own search and collapsible groups, and Essentials as a two-column key/value grid. Both themes were carried with the control in the top right, and contrast was measured against the rendered surfaces by a test that holds it. |
