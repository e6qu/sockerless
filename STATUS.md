...
| | |
|---|---|
| Active branch | `feat/azure-ratchet-4` — **fourth Azure service ratchet (BUG-2234).** Four agents (isolated worktrees, merged with zero conflicts + one combined measured pass), every op spec-validated against vendored ARM Swagger (0 new violations, list-by-sub/RG paths exercised) + real azure-sdk-for-go: **Logic Apps** 13→106/106 (100%), **App Service / Web Apps** 37→161/692 (core surface; fixed BUG-2233 FunctionEnvelope name leak), **Cosmos DB** both versions to 100% (124/124, 121/121) + private-endpoint-connections 4/4 each + **Log Analytics query** 1→5/7, **API Management** apis+products to 100% (91/91, 31/31) + **PostgreSQL** to 100% (66/66) + **Resources** 32→36/40. **Azure 1409→1758/2597 (54%→68%).** azure build/lint(0)/deadcode(0)/dupl(0) + route-validity + doc/spec-consumption + coverage-floor + contract-hook all green; full-suite spec-validator confirmation before push. |
...
| Bugs | See [BUGS.md](BUGS.md) header (2234 filed · 2190 fixed · 2 open · 16 FP). 2 open: BUG-1075 (live-cloud), BUG-1345 (azuread upstream) — both externally gated. |
| Live infra | None up. |
