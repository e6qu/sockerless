...
| | |
|---|---|
| Active branch | `feat/gcp-console-visual-fidelity` |
| Branch purpose | The Google Cloud console was rebuilt to the console's real visual language after a side-by-side against the live console showed it was a rough silhouette. Design tokens were read from the live console's computed styles (its blue-tinted background, the left-anchored active pill, the primary blue); Roboto was vendored as a woff2 subset for body text and Material Symbols Outlined as inline SVG paths for icons — a real icon on every navigation item, the header tool cluster, the filter and column controls — both Apache-2.0 and self-contained. The account became an avatar with a menu holding the identity and sign-out, showing a neutral avatar rather than an error string when unauthenticated, and the empty state was completed. The browser suite pins the visual work structurally so it cannot regress unseen. |
