...
| Branch purpose | Follow-up Bleephub fidelity hardening after merged #787. The branch made persisted repository ownership strict and removed internal runner-submission image fallbacks. |
| Current fixes | BUG-2491 and BUG-2492 were fixed on this branch. Persisted repository reload now requires valid `owner_type` and `owner_id`, validates organization-owned repositories against loaded organization state, and no longer treats empty owner types as user repositories in public listing/event paths. Internal job and workflow submission routes now require either explicit `image` or `hostMode`, and tests pass explicit images when they intend container execution. |
...
| Bugs | See [BUGS.md](BUGS.md) header: 2492 filed, 2448 fixed, 3 open, 16 false positives. Open: BUG-2441, BUG-1345, and BUG-1075. |
