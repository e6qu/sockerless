...
| Branch purpose | Continued Bleephub GitHub Pages fidelity after merged #790 by implementing real branch publication and unified committed-reference eventing. |
| Current fixes | BUG-2506 through BUG-2511 were fixed on this branch. Legacy builds now publish configured git sources through direct static archiving or the pinned `github-pages` 232/Jekyll 3.10.0 runtime. Smart HTTP, Contents API, and Git Database branch writes share one race-safe fan-out for repository activity, push webhooks, GitHub Actions workflows, pull-request synchronization, and automatic matching Pages builds. Workflow-run actors use complete GitHub simple-user shapes, eventing resolves canonical repository identity, and Bleephub uses current `github.com/yuin/goldmark` 1.8.3. |
...
| Bugs | See [BUGS.md](BUGS.md) header: 2511 filed, 2467 fixed, 3 open, 16 false positives. Open: BUG-2441, BUG-1345, and BUG-1075. |
