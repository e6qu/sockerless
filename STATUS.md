...
| **8** GL × gcf | sockerless-gcf | 🟡 **v18 stdinPipe + relaxed attach + 5s pre-check window — STILL HANGS** | v18 (rev 00051, digest `sha256:5fc5c398`): full stdinPipe pattern, ContainerAttach gating dropped (any stdin attach allowed regardless of image URI), 5 s pre-check window for late-arriving stdinPipe registration. **Definitive evidence**: gitlab-runner makes ZERO `/containers/{id}/attach` calls — the 5 s pre-check times out, default-invoke fires. After ContainerStart returns, gitlab-runner makes NO HTTP calls of ANY kind to sockerless for 15+ min. The hang is internal to gitlab-runner — most likely a TCP probe from gitlab-runner's own process to the postgres service container's IP. In a multi-container Cloud Run Service revision, postgres is reachable at 127.0.0.1:5432 from inside that revision, but NOT from gitlab-runner's host (a different Cloud Run Service instance entirely). gitlab-runner's `waitForServices` retries forever. **Next investigation**: compare cell 7's `serviceToPodMemberContainer`/`NetworkSettings` response shape to cell 8's; cloudrun cell 7 GREEN means there's some way it dodges the TCP probe (maybe IPAddress/Aliases differ, maybe the cloud_state row reports postgres as exited so gitlab-runner skips the wait). |
