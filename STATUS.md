...
| | |
|---|---|
| Active branch | `fix/protect-simulator-dashboard-api` |
| Branch purpose | The first-party simulator identity boundary protected both the rendered operator interface and its `/sim/v1/*` dashboard data, while health checks and native cloud API routes remained available under their own protocol-specific authentication contracts. |
...
| Simulator identity | The three dashboards share first-party OpenID Connect authorization-code + PKCE, exact issuer validation, signed server-tracked sessions, identity, RP-Initiated Logout, strict same-origin controls, and form-only signed back-channel revocation with atomic replay rejection. Their HTML and dashboard-data routes share the same authorization boundary while health checks and native cloud API slices remain unchanged. |
...
| Latest merged pull request | [#806 Polish consoles and harden OIDC/browser validation](https://github.com/e6qu/sockerless/pull/806) completed the first-party simulator and Admin identity, logout, interface, browser-validation, and image-build foundation. |
...
- Merge and publish the dashboard-data authorization fix, then deploy the updated operator-console and simulator images in the shared development environment. Register each Admin and simulator callback, post-logout, and back-channel logout coordinate in Shauth, and preserve the simulators' native cloud protocol contracts.
