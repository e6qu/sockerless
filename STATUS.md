...
| Current fixes | BUG-2457 through BUG-2467 were fixed on this branch. Repository deletion now purges persisted issue comments, issue field values, issue events, notification read/done/subscription thread state, sub-issue links, issue dependency links, pull request reviews, pull request review comments, Projects v2 items, artifact attestations, repository activity, clone traffic, watch subscriptions, selected-repository allowlists, GitHub App selected repositories, installation token repository scopes, code-security attachments, deployments, deployment statuses, environments, environment policies, protection rules, Pages deployment records, Copilot coding agent tasks, CodeQL variant-analysis target rows, team grants, artifact metadata rows, source import records, dependency snapshots, SBOM exports, enterprise Dependabot repository-access IDs, repository labels, repository milestones, and reactions on deleted issue, pull request, comment, release, pull request review-comment, and commit-comment parents. Project deletion also clears its in-memory Projects v2 content indexes. Repository rename and transfer now move team grants, notification repo read markers, and artifact metadata repository references with the rest of the repo-full-name state. Deployment deletion and parent deletion paths now purge status rows and reaction rows with their parent records. The branch also carried deterministic README badge refresh commits generated by the pre-push hook. |
...
| Bugs | See [BUGS.md](BUGS.md) header: 2467 filed, 2423 fixed, 3 open, 16 false positives. Open: BUG-2441, BUG-1345, and BUG-1075. |
