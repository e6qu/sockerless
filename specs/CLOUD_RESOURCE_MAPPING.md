...
> - [SIMULATOR_RECOVERY.md](https://github.com/e6qu/sockerless-cloud/blob/main/specs/SIMULATOR_RECOVERY.md) (sockerless-cloud repository) — recovery on restart, PID re-attachment, simulator-side tag handling
...
8. **Every Docker abstraction is assembled from cloud primitives on every backend — FaaS included — for experiential parity with local Docker/Podman.** The goal: a user's experience with containers, pods, networks, and volumes inside any backend is *the same* as with regular Docker/Podman; the cloud is the implementation, never a visible seam, and each backend does its actual execution inside its own model (rule 10). Networks, multi-container pods, and volumes are *composed*, not declared unsupported. There is no "this Docker concept has no cloud analog": if a cloud lacks a first-class equivalent, sockerless builds one from the primitives it *does* have (Private DNS / Cloud Map / Cloud DNS zones, multi-container Service revisions / ACA Apps / Fargate tasks, EFS / GCS / Azure-Files shares) plus the sockerless **agent** (direct or reverse). The agent fills gaps in the cloud's API surface — but it must **never break or bypass** the abstractions sockerless establishes (stateless, cloud-is-source-of-truth, Docker-API-faithful). **A FaaS platform that doesn't run multiple containers inside one function is *our job to assemble*, not to reject:** compose the container-pod execution model (with its docker network) from native sidecar containers where the platform offers them, else **a pod assembled from multiple functions (one per container)** wired by cloud DNS/service-mesh + a shared workspace volume — staged across [PLAN.md](../PLAN.md) phases. The pod abstraction is delivered in full regardless of backend — including **localhost / shared-loopback networking between members** (a sibling reachable on `localhost:<port>`, matching Docker/Podman pod + Compose semantics): intrinsic where the cloud primitive shares a network namespace (multi-container revision / App / task, native sidecars), and assembled by the sockerless **agent** (proxying `localhost:<port>` to the sibling member over the cloud network) where separate functions don't share one. Every backend now assembles the pod (see the libpod pod table below); should a future surface arrive unbuilt, a fail-fast rejection is honest interim behavior tracked as *not-yet-assembled*, never the intended end state. Name the primitive + agent path that would compose any unbuilt piece; never "impossible."

9. **FaaS backends run user-supplied container images, never the native runtime.** Lambda, GCF (Cloud Run Functions gen2), and AZF (Azure Functions) deploy OCI images chosen by the operator. Sockerless never targets the platforms' "function-as-code" runtime contracts (Node/Python/Go handlers in a managed sandbox). Container deployment is what lets sockerless put its bootstrap at the entrypoint, which is the prerequisite for the reverse-agent, agent-as-handler, and overlay-rootfs (opt-in via `SOCKERLESS_OVERLAY_ROOTFS=1`) patterns. ACA and Cloud Run are native container services, so this distinction is automatic — every deployment is a container.

10. **Backend ↔ host primitive must match (CRITICAL).** When a sockerless backend is deployed *as part of a workload running on a cloud* (e.g. baked into a CI runner image), the backend must match that cloud's primitive: ECS backend in ECS, Lambda backend in Lambda, Cloud Run backend in Cloud Run, CRF in CRF, ACA in ACA, AZF in AZF. Cross-pollination ("bake the ECS backend into a Lambda image to dispatch sub-tasks via Fargate and avoid Lambda-in-Lambda recursion") is a class of architectural error tracked at top of [BUGS.md](../BUGS.md). Each cloud's own dispatch primitives are the answer for sub-task workloads on that cloud, even when 15-min caps or concurrency limits make it harder. The **runner-on-FaaS dispatch table** below gives the per-cloud primitive used for `container:` sub-tasks:
...
- **Workloads always run on Docker hosts.** No sim handler may `os/exec` a workload as a host process of the sim itself. Enforcing test: `simulator-<cloud>/sdk-tests/host_dispatch_test.go` (each sim ships one; all `simulator-*` paths in this spec refer to the [sockerless-cloud repository](https://github.com/e6qu/sockerless-cloud)).
...
|---|---|---|---|---|
| AWS EC2/ECS | IMDSv2 at `169.254.169.254/latest/meta-data/...` | `simulator-aws/metadata.go::registerHostMetadata` | `AWS_EC2_METADATA_SERVICE_ENDPOINT` | ✅ 135c-2 |
| AWS ECS task | `${ECS_CONTAINER_METADATA_URI_V4}/task` | `simulator-aws/metadata.go` (`/v4/{id}/task`) | `ECS_CONTAINER_METADATA_URI_V4` | ✅ 135c-2 |
| AWS Lambda | Runtime API `${AWS_LAMBDA_RUNTIME_API}/2018-06-01/runtime/...` | `simulator-aws/lambda_runtime.go` (per-invocation sidecar) | `AWS_LAMBDA_RUNTIME_API` | ✅ pre-existing |
| GCP GCE/CR/GCF | `metadata.google.internal/computeMetadata/v1/...` | `simulator-gcp/metadata.go::registerComputeMetadata` | `GCE_METADATA_HOST/IP/ROOT` | ✅ 135c-1 |
| Azure App Service / ACA / AZF | IMDS `169.254.169.254/metadata/instance` + `/metadata/identity/oauth2/token` | `simulator-azure/metadata.go` + `simulator-azure/managedidentity.go` | `IDENTITY_ENDPOINT` + `IDENTITY_HEADER` + `AZURE_INSTANCE_METADATA_ENDPOINT` | ✅ 135c-3 |
...
A cloud build (ACR Tasks / Cloud Build, with `IsPushEnabled`) **pushes** the built image to the registry (ACR / Artifact Registry) and the cloud's compute (ACA App / Cloud Run Service / Function) later **pulls** it from there over the public OCI `/v2/` API — there is no privileged channel from compute into registry storage. The simulators are faithful to this: the build slice does a real `docker push <ref>` + `docker rmi` (it never leaves the image in the local daemon as a shortcut), and the run pulls `<ref>` over `/v2/`. The sims' `/v2/` is the same shared `OCIRegistry` (`simulator-<cloud>/shared/oci.go`) ECR/AR/ACR are built on.
...
- **GCP:** `SOCKERLESS_GCP_AR_ENDPOINT` overrides `<region>-docker.pkg.dev`. The backend reads it once into `Config.ARRegistryEndpoint`, and that one value names the host image references carry (`gcpcommon.OverlayRegistryHost`) and the URL registry HTTP is dialed at (`gcpcommon.RegistryEndpointURL`; a bare host means `https://`) — for the auth provider, the tag-existence probe, the multi-arch index, and the image resolver alike. A harness names the simulator with its scheme (`http://127.0.0.1:<port>`).

A harness pointed at a simulator sets this coordinate to the sim's own `/v2/` address (e.g. `127.0.0.1:<port>`, which Docker auto-trusts as insecure — a local Podman host needs an `insecure-registries` drop-in for that host); a real deployment leaves it default (the real registry). The registry at either coordinate refuses an anonymous push and a push into a repository nobody created, so a harness provisions the way Terraform and the operator do against the real service — repositories through the Artifact Registry API, `docker login -u oauth2accesstoken` with an access token minted from the service-account key (`gcp-common/registrytest`) — and hands the simulator process the Docker configuration that login lands in, the way a Cloud Build worker and the Cloud Run service agent carry the project's registry credential. The backend code and the integration tests are **identical** against cloud and sim — they differ **only in this coordinate value**, exactly like `SOCKERLESS_ENDPOINT_URL` for the control-plane APIs. **Never** add an `if sim` / `if target == "sim"` branch or any sim-only behaviour in backend or test code to route the registry — that is a fake test (see [AGENTS.md](../AGENTS.md) § "Simulators are real implementations" and § "No stubs… no synthetic behavior").
...
`ContainerCreate` kicks `deployFunctionAsync` immediately (BUG-923 fix — eager async deploy so the 200ms ContainerCreate stays inside gitlab-runner's 120s docker-daemon timeout). But `network_pod.go::shouldDeferOrMaterializeNetworkPod` in `ContainerStart` later tries to defer the deploy until a script-runner triggers materialization of all pod members as a multi-container revision. **These conflict**: by the time ContainerStart runs, deployFunctionAsync has already deployed each container as its own single-container function. The defer path in ContainerStart is dead for any container that already finished its async deploy.
...
| Pod | **Assembled as ONE App Service site with sitecontainers** (`Microsoft.Web/sites/{name}/sitecontainers` — the native Azure multi-container primitive: one `isMain` container + N sidecars sharing a network namespace). A pod of containers on a user-defined network materializes a single Function App site whose IsMain container runs the reverse-agent overlay; members share one loopback (`localhost:<port>`) and the main bootstrap writes `SOCKERLESS_HOST_ALIASES` to `/etc/hosts` so siblings resolve by name. **Sidecars** run the overlay in *sidecar mode* (`SOCKERLESS_SIDECAR=1`): the bootstrap dials its own reverse-agent (so `docker exec <sidecar>` works, like Cloud Run) and execs the service without binding the function HTTP port; a raw image is the no-overlay fallback (no per-sidecar exec). **Shared workspace**: every member's named-volume binds attach to the site once (`UpdateAzureStorageAccounts`), and each sitecontainer mounts it via `SiteContainerProperties.VolumeMounts` — the pod analog of an ECS task's shared task-level Volumes. The Azure analog of an ECS multi-container task / Cloud Run multi-container revision. See `backends/azure-functions/pod_site.go::materializePodSite` + `network_pod.go`. | site name `skls-<mainID[:12]>` + sitecontainer names (`main`, sidecar names) | + tag `sockerless-pod=net-<networkID>`, `sockerless-pod-members=<base64 manifest>` |
| Image | ACR | `<acrName>.azurecr.io/<repo>:<tag>` | (registry-managed) |
...
| `cloudrun` | **GCS bucket** per volume, mounted via Cloud Run Service's native `Volume{Gcs{Bucket, MountOptions}}` in the revision template. **MountOptions are mandatory for runner workspaces** per BUG-944 — without them the GCS-Fuse default 5s negative-cache hides freshly-written files from sibling containers (cross-execution metadata staleness). Use `gcp-common.RunnerWorkspaceMountOptions()` = `[implicit-dirs, metadata-cache:ttl-secs=0, metadata-cache:negative-ttl-secs=0]`. **Architectural note**: GCP's GCS-Fuse is object-store-with-FUSE (eventual consistency + caching by default); AWS-side EFS is NFSv4 (strong consistency by default). The MountOptions opt-out is sockerless's mechanism to make GCP behave like AWS for cross-execution shared-FS patterns. Optional upgrade to **Filestore** later for POSIX semantics if `O_APPEND` / file locking is needed. | `VolumeCreate` → `storage.Buckets.Insert` with naming `sockerless-volume-<id>`, label `sockerless-managed=true`. `VolumeRemove` → `DeleteBucket` (requires empty; force=true uses `DeleteObjects` first). Bind / named mount → inject `RevisionTemplate.Volumes[].Gcs{Bucket, MountOptions}` + `Container.VolumeMounts` in the service spec. | `storage.buckets.create/delete/list`, `storage.objects.*` for prune/delete. Cloud Run service account needs `roles/storage.objectAdmin` on buckets it mounts. | `simulator-gcp/storage.go` already has GCS slice; extend with `Volume{Gcs}` honouring on the Cloud Run simulator path so the backing Docker container gets a real bind mount against the sim's bucket directory. |
| `aca` | **Azure Files share** in a sockerless-owned storage account, linked into the managed environment as an `ManagedEnvironments/storages` resource, then referenced from `ContainerApp.Properties.Template.Volumes[]` + `Container.VolumeMounts`. | `VolumeCreate` → (ensure storage account exists) + `FileShares.Create` + `ManagedEnvironmentsStorages.CreateOrUpdate` so the env knows about the share. `VolumeRemove` → `FileShares.Delete` + `ManagedEnvironmentsStorages.Delete`. Bind / named mount → inject `ContainerAppProperties.Template.Volumes` + `Container.VolumeMounts` into the app spec. | `Microsoft.Storage/storageAccounts/read,write,listKeys`, `Microsoft.Storage/storageAccounts/fileServices/shares/read,write,delete`, `Microsoft.App/managedEnvironments/storages/read,write,delete`. | `simulator-azure/storage.go` gains `fileServices/shares` sub-resource CRUD (the storage slice today is blob-only). `simulator-azure/containerappsenv.go` gains `storages` sub-resource. The sim's ACA container bind-mounts a host-side directory per share so containers see real files. |
| `cloudrun-functions` (gcf) | GCP Cloud Functions (Gen2 only — Gen1 deprecated, not supported). Gen2 = Cloud Run Service under the hood. **Two-step volume attach** (escape hatch): Cloud Functions Gen2 `ServiceConfig` has no first-class Volumes primitive — only SecretVolumes. Sockerless attaches via `Run.Services.GetService` + `UpdateService` on the function's underlying `Function.ServiceConfig.Service`. Same `MountOptions` requirement as cloudrun (BUG-944). **Idempotent attach** required for pool-reused functions: must compare `volumes[].name + bucket + mountOptions` and replace stale entries (matching name + stale opts must NOT short-circuit the UpdateService — that's the layer-3 BUG-944 trap). | Shared helper with `cloudrun` — same GCS-bucket-mount lifecycle. Plus `attachVolumesToFunctionService` post-CreateFunction. | Same as cloudrun + `run.services.get/update` on the function's backing service. | Shares `simulator-gcp/` GCS extensions. |
| `azure-functions` (azf) | Azure Functions on the current Flex Consumption / Premium plan with BYOS Azure Files mounts. | Provision Azure Files share (shared helper with ACA), then attach to the Function App via `sites/<fn>/config/azurestorageaccounts`. | Same Azure Files permissions as ACA + `Microsoft.Web/sites/config/write`. | Shares `simulator-azure/` Azure Files extensions. |
...
| Method | docker | ecs | lambda | cloudrun | gcf | aca | azf |
|--------|:------:|:---:|:------:|:--------:|:---:|:---:|:---:|
| PodCreate | ✓ | ✓ multi-container task-def | ✓ merged-rootfs pod Function | ✓ multi-container Service revision | ✓ merged-rootfs pod Function | ✓ multi-container App | ✓ multi-container pod site |
| PodStart | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ |
| PodStop | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ |
| PodKill | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ |
| PodRemove | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ |
...
- **The FaaS backends assemble the pod rather than rejecting it.** Neither Lambda, Cloud Run Functions, nor Azure Functions exposes a native
  multi-container-per-invocation primitive, so each composes one from the primitives it does have. Lambda and Cloud Run Functions collapse the
  pod's members into a single Function backed by a merged-rootfs overlay image (`backends/lambda/pod_materialize.go`,
  `backends/cloudrun-functions/pod_materialize.go`), materialized on the last member's start, whose bootstrap pre-warms the sidecars so members
  reach each other on shared loopback. Azure Functions assembles the pod as one App Service site carrying every member
  (`backends/azure-functions/pod_site.go`). In all three the throwaway per-member resources created at `ContainerCreate` are deleted during
  materialization, so the pod resource is the only cloud-side object mapping back to any member.
- `BaseServer`'s generic multi-container rejection therefore never fires on a cloud backend: every one of the seven overrides `ContainerStart`
  with its own assembly path.
...
|------|------------|--------|-----------|
| EFS `AccessPoint` CRUD | ecs, lambda | closed | `simulator-aws/efs.go` |
| Azure Files share slice (`fileServices/shares`) | aca, azf | closed | `simulator-azure/files.go` |
| Managed-environment `storages` sub-resource | aca | closed | `simulator-azure/containerappsenv.go` |
| GCS bucket-mount honouring on Cloud Run spec-builder path | cloudrun, gcf | closed | `simulator-gcp/cloudrun.go` + `simulator-gcp/storage.go` |
| ACA console exec proto | aca | closed | `simulator-azure/containerapps.go::handleACAJobExec` |
...
| Lambda reverse-agent (agent-as-handler) | lambda | closed | `simulator-aws/lambda_runtime.go` exposes the per-invocation Runtime API; reverse-agent works end-to-end against the sim |
| AWS Session Manager agent-side ack validation | ecs | closed | `simulator-aws/ssm_proto.go` mirrors `SerializeClientMessageWithAcknowledgeContent` |
...
| `github-runner-dispatcher-gcp` (Phase 122) | Cloud Run Jobs (`cloud.google.com/go/run/apiv2`) | `Jobs.CreateJob` (one-shot, `MaxRetries: 0`) + `Jobs.RunJob`; tags via Job `Labels` (`sockerless-dispatcher-managed-by` etc.) | `Jobs.ListJobs` filtered by managed-by label | `Jobs.DeleteJob` keyed off the latest EXECUTION's state (`EXECUTION_SUCCEEDED` / `EXECUTION_FAILED`; `NO_EXECUTION` past a 15-min grace) — never `TerminalCondition.State`, which tracks Job-DEFINITION reconciliation and reads ready while the execution still runs |
| `github-runner-dispatcher-azure` (Phase 122b) | Container Apps Jobs (`armappcontainers`) | `Jobs.BeginCreateOrUpdate` + `Jobs.BeginStart` (Manual trigger, `Parallelism: 1`, `ReplicaRetryLimit: 0`); tags via ARM `Tags` (`sockerless-dispatcher-managed-by` etc.) | `Jobs.NewListByResourceGroupPager` filtered by managed-by tag | `Jobs.BeginDelete` keyed off the latest JobExecution's `Status` (`Succeeded` → terminal; `Failed`/`Stopped`/`Degraded` → terminal-failed; `NO_EXECUTION` past a 15-min grace) — never `Properties.ProvisioningState`, which tracks ARM resource provisioning and reads `Succeeded` right after create while the execution still runs |

All three share the same:
- Poll loop (`pkg/poller`): `GET /repos/{r}/actions/runs?status=queued` every 60 s with a 5-min seen-set and proactive back-off when rate-limit headers near depletion.
