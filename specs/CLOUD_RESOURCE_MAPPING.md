...
| Pod | **Assembled as ONE App Service site with sitecontainers** (`Microsoft.Web/sites/{name}/sitecontainers` — the native Azure multi-container primitive: one `isMain` container + N sidecars sharing a network namespace). A pod of containers on a user-defined network materializes a single Function App site whose IsMain container runs the reverse-agent overlay and whose sidecars run their RAW service images; members share one loopback (`localhost:<port>`) and the main bootstrap writes `SOCKERLESS_HOST_ALIASES` to `/etc/hosts` so siblings resolve by name. The Azure analog of an ECS multi-container task / Cloud Run multi-container revision. See `backends/azure-functions/pod_site.go::materializePodSite` + `network_pod.go`. | site name `skls-<mainID[:12]>` + sitecontainer names (`main`, sidecar names) | + tag `sockerless-pod=net-<networkID>`, `sockerless-pod-members=<base64 manifest>` |
| Image | ACR | `<acrName>.azurecr.io/<repo>:<tag>` | (registry-managed) |
