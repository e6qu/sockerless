...
Surface: `simulators/azure/blob.go`, `simulators/azure/storage_dataplane.go` (dispatch, shares, queues, tables), `simulators/azure/files_dataplane.go` + `simulators/azure/files_entries.go` (Files shares, directories, files, ranges and links), `simulators/azure/files_ranges_*.go` + `simulators/azure/files_punch_*.go` (the filesystem extent map List Ranges and Clear Range act on) and `simulators/azure/queue_dataplane.go` (queue access policies, message updates, Queue service configuration).
...
Every operation of the vendored `storage-dataplane-blob-2026-04-06` Swagger document is served; the coverage gate in `simulators/azure/azure_coverage_test.go` locks the count at 69/69.
