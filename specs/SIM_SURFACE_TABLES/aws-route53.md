...
Surface registered in `simulators/aws/route53.go`. Rows below are the Route 53 REST XML ops the AWS simulator currently registers.
...
|---|---|---|---|---|---|
| `POST /2013-04-01/hostedzone` | ✓ `simulators/aws/route53.go:405::handleR53CreateHostedZone` | ✓ (direct; see coverage matrix) | ✓ (direct; see coverage matrix) | n/a | |
| `GET /2013-04-01/hostedzone` | ✓ `simulators/aws/route53.go:514::handleR53ListHostedZones` | ✓ (direct; see coverage matrix) | ✓ (direct; see coverage matrix) | n/a | |
| `GET /2013-04-01/hostedzonesbyname` | ✓ `simulators/aws/route53.go:534::handleR53ListHostedZonesByName` | ✓ `simulators/aws/sdk-tests/route53_test.go::TestRoute53ListHostedZonesByName` | n/a | ✓ SDK test covers `MaxItems`, `NextDNSName`, and `NextHostedZoneId` | terraform-provider-aws v6.47.0 uses `ListHostedZones` for current `aws_route53_zone`/`aws_route53_zones` lookup flows, so this exact op has no current provider resource path. |
| `GET /2013-04-01/hostedzone/{id}` | ✓ `simulators/aws/route53.go:475::handleR53GetHostedZone` | ✓ (direct; see coverage matrix) | ✓ (direct; see coverage matrix) | n/a | |
