...
Surface: `simulators/aws/s3_subresources.go`. Object-level multipart upload family. Every operation listed dispatches via query-string subresource (`?uploads`, `?uploadId`, `?uploadId&partNumber`) on the `{bucket}/{key...}` route, gated by known-bucket (BUG-1150).
