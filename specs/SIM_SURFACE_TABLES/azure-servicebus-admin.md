...
Surface: `simulators/azure/servicebus_admin.go` via the `{namespace}.servicebus.<sim-host>` subdomain wrapper in `servicebus_dataplane.go`.

This is the namespace-level ATOM XML admin protocol used by `github.com/Azure/azure-sdk-for-go/sdk/messaging/azservicebus/admin`. It is separate from the ARM management surface in `simulators/azure/servicebus.go` and from the REST message data plane in `simulators/azure/servicebus_dataplane.go`.
