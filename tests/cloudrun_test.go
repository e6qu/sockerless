...
// cloudRunClient returns a Docker client configured for the Cloud Run backend, or skips the test.
func cloudRunClient(t *testing.T) *client.Client {
	t.Helper()
	socket := os.Getenv("SOCKERLESS_CLOUDRUN_SOCKET")
	if socket == "" {
		t.Skip("SOCKERLESS_CLOUDRUN_SOCKET not set, skipping Cloud Run integration test")
	}
	...
	if err != nil {
		t.Fatalf("failed to create Cloud Run docker client: %v", err)
	}
	return c
}
