...
// gcfClient returns a Docker client configured for the Cloud Run Functions backend, or skips the test.
func gcfClient(t *testing.T) *client.Client {
	t.Helper()
	socket := os.Getenv("SOCKERLESS_GCF_SOCKET")
	if socket == "" {
		t.Skip("SOCKERLESS_GCF_SOCKET not set, skipping Cloud Run Functions integration test")
	}
	...
	if err != nil {
		t.Fatalf("failed to create GCF docker client: %v", err)
	}
	return c
}
...
func TestGCFNetworkOperations(t *testing.T) {
	...
	netResp, err := c.NetworkCreate(ctx, "gcf-net-"+testID, network.CreateOptions{})
	if err != nil {
...
func TestGCFVolumeOperations(t *testing.T) {
	...
	vol, err := c.VolumeCreate(ctx, volume.CreateOptions{Name: "gcf-vol-" + testID})
	if err != nil {
