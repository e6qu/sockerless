...
var simBackends = map[string]simBackendInfo{
	"ecs": {
		...
		ExtraEnv: map[string]string{
			...
			// Architecture is mandatory; no default.
...
func startSimulator(binaryPath string, cloud string) (*simProcess, string, error) {
	// Both ports come from one reservation, so the DNS listener cannot be
	// handed the port just chosen for the API. The AWS simulator's Route 53
	// endpoint serves one coordinate over both TCP and UDP; its default 5353
	// collides with mDNS listeners on developer hosts, so the harness always
	// assigns an operating-system-selected port.
	ports := newPortReservation()
	port := ports.tcp()
	dnsPort := ports.tcpAndUDP()
	ports.release()
