...
`feat/ecs-gitlab-attach-stdin-1798` (Arc 3 Phase 3; PR pending). **BUG-1798 fixed — the ECS attach-stdin gate is closed.** The harness now runs the gitlab-runner helper stages and delivers the step script into the build container (no more hang). **Next gate — BUG-1800** (the build job's `/builds` volume is not writable). BUG-1797/1799 merged in #575; Phase 1 sim in #574.

### Arc 3 remaining work

1. **BUG-1800 — the next Phase-3 gate (sim EFS access-point perms).** After BUG-1798, the build container's `step_script` fails immediately: `mkdir: can't create directory '/builds/project-1.tmp': Permission denied`. The ECS backend maps the gitlab-runner `/builds` docker volume to an EFS access point created with `CreationInfo{OwnerUid:1000, OwnerGid:1000, Permissions:"0777"}` (`aws-common/volumes.go`); the **aws sim** materializes the access-point host dir with default `0755 root` perms instead of applying `CreationInfo` (verified: `…/sockerless/v/<hash>` is `drwxr-xr-x 0 0`). Fix is sim-side: when creating/serving an EFS access point, mkdir the root dir with `CreationInfo.Permissions` + chown to `OwnerUid/OwnerGid` (and enforce `PosixUser` when set), as real EFS does — no backend coupling. Repro: `make bleeplab-runner-docker-test-ecs` (now fails at the build `step_script`, not a hang); `BLEEPLAB_HOLD=1` to inspect, then `find /tmp/sockerless-bleeplab-data -path '*sockerless/v*' -type d | xargs ls -land` inside the harness container shows the `0755 root` dir.
