...
`audit-round20` — the 3 round-19 deferrals + a fresh fidelity/fuzz round, **all in one PR (no deferrals)**. 14 fixes (BUG-2112..2125): the gcp-family docker-label **single-source refactor** (2112), EventBridge InputPath/InputTransformer engine (2113), default-on-invalid → error (2114), 3 fuzz-found crashes (2115 SSM frame overflow, 2116 AMQP OOM, 2117 CW Insights recursion), and 8 fidelity fixes (2118 Key Vault PATCH clobber, 2119 EFS read-back, 2120 RDS, 2121 ElastiCache, 2122 Kinesis, 2123 GCP Logging, 2124 Cloud DNS, 2125 SSM). SDK + unit + fuzz tests; all green; extended fuzz over 19 targets = zero crashers.

**Validation gate for BUG-2112:** it edits the stateless reconstruction the cloudrun/gcf runner cells validate. Round-trip unit tests cover the carrier on both backends; before merge, run `make bleeplab-runner-docker-test-{cloudrun,gcf}` as the final gate (the docker harnesses can't run in the agent environment). See [[project_reliable_metadata_reconstruction]].

**Next candidates:** the live-cloud track (BUG-1075); a fresh fidelity/anti-pattern/fuzz round (the parallel-agent + un-fuzzed-surface-map method keeps finding real bugs — round 20 found 3 new crashes + ~8 fidelity gaps).
