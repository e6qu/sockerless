...
`fix/dynamodb-set-rhs-parens` — **consumer #648 + a cross-cloud NoSQL "silent-wrong evaluator" sweep** (BUG-2149..2166). Origin: #648 — `ddbEvalSetRHS` never stripped a fully-enclosing `(...)` so `(if_not_exists(c,:0)-:v)` / `(:z)` stored null (ElectroDB always parenthesizes `.subtract()`); fixed with `ddbStripParens` (2149). Then audited every cloud sim's DynamoDB-like service for the same class:
- **AWS DynamoDB (2150–2153):** nested-path `if_not_exists`/copy via `ddbResolvePath`; number `=`/`<>`/`IN` canonicalization; exact `big.Rat` SET arithmetic + comparison; non-numeric operand → ValidationException.
- **GCP Firestore (2154–2157):** field transforms (increment/arrayUnion/serverTimestamp) applied; `currentDocument` preconditions enforced; typed value-equality; cursors + `select`. (+2 fidelity bugs from driving `firestore.NewRESTClient`: enum-as-number decode, transform-only updateMask collapse.)
- **Azure Cosmos+Table (2160–2166):** real Cosmos SQL parser (`cosmos_sql.go`) replacing the equality-only matcher; Cosmos upsert+409, Cosmos PATCH; Table MERGE-vs-PUT, `$filter` typed literals, `$batch`, `$select`.

Done via 2 parallel subagents (GCP, Azure — disjoint modules) + AWS by hand; SDK tests for every fix; each module build/lint(0)/test/fuzz green.

**Staged (filed in BUGS.md, NOT in this PR):** GCP Firestore transactions (2158 — needs an isolation model; hard-fail not silent), GCP Bigtable data plane (2159 — entirely absent; nothing in the repo issues those RPCs yet).

**Next candidates:** the staged GCP items (2158/2159) if a consumer needs them; the live-cloud track (BUG-1075); another fidelity/fuzz round. Open GitHub issues: only #394 (azuread upstream-blocked).
