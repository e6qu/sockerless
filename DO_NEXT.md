...
- The compiled Admin and all three compiled simulator dashboards passed a real PostgreSQL, Ory Hydra, Shauth, and Chromium matrix covering direct entry, catalog entry, shared sign-on, identity, logout initiated by every relying party, global cross-application revocation, exact app-local signed-out destinations, signed-out reload, and fail-closed re-entry. The matrix used the exact CI-green Shauth no-CDN revision.
