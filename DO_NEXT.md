...
`fix/cloudtrail-management-events` — **CloudTrail data-plane→control-plane leak, fixed systemically** (consumer #650/#651, BUG-2167/2168) + a **start on the #652 prevention architecture** (2169 staged). The central recorder logged every op as a CloudTrail management event with no data-vs-management classification, so high-volume data events from ~all services (DynamoDB items, S3 objects, Lambda Invoke, SQS messages, SNS Publish, Kinesis records) leaked into `LookupEvents` (AWS guarantees management-only), and the unauthenticated container healthcheck (`GET /` → S3 `ListBuckets`) generated phantom events.

- **#650/2167:** `cloudTrailShouldRecord` skips unauthenticated requests (no `Credential=`). Service-initiated events use a separate `invokedBy` path, unaffected.
- **#651/2168 (systemic):** *registration-time* data/management classification — each service's register fn calls `cloudTrailDeclareDataEvents(source, ops…)`, co-located with its handlers; consulted centrally; data events excluded on BOTH the client path and the service-initiated (scheduler-fired) path. Rewrote the scheduler-fired-SQS test to verify firing via the queue. Cross-service SDK conformance guard added (#652 lever 5).
- **#652 architecture (user chose "start the bigger architecture now"):** adopted lever 4 (model the cross-cutting concern at registration) + lever 5 (conformance guard). **Staged as 2169:** lever 1 (spec→struct/validation codegen + a completeness test from the vendored smithy models), lever 2 (fail-loud-by-default for unimplemented constructs), lever 3 (replace remaining string-munging Condition/Filter evaluators with lexer→AST), lever 5b (differential testing vs DynamoDB Local).

aws sim build/lint(0)/unit + CloudTrail SDK+CLI green.

**Next candidates:** continue the #652 levers (2169) — the highest-leverage is lever 2 (fail-loud) + lever 1 (spec-completeness test); the staged GCP items (2158/2159); live-cloud (1075). Open GitHub issues: #652 (meta, this PR starts it), #394 (azuread upstream-blocked).
