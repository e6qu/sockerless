...
`feat/bleephub-repository-license-graphql` — Bleephub repository metadata now uses real persisted repository, git, Pages, and viewer-access state for license metadata, feature flags, merge-method settings, pushed timestamps, archive timestamps, template-generation provenance, Pages capability, and REST repository permissions instead of null/hardcoded compatibility shapes. CI keeps the same required AWS CLI simulator check names while moving RDS CLI coverage from the overloaded appdata shard to appdata2.
...
- **BUG-2397:** Generated repositories now persist the source template repository ID from `POST /repos/{template_owner}/{template_repo}/generate`, and GraphQL `Repository.templateRepository` resolves that persisted source repository instead of returning null for every repository.
- **BUG-2398:** REST repository JSON now derives `permissions.admin`, `permissions.push`, and `permissions.pull` from the authenticated viewer's real repository access. Owners see full permissions, pull-only collaborators see pull without push/admin, anonymous readers of public repositories see pull only, and private repositories no longer advertise fabricated access.
- **BUG-2399:** The AWS CLI simulator appdata shard no longer carries RDS coverage after hosted-runner variance canceled it mid-RDS. The existing appdata/appdata2 required check names remain unchanged, and RDS CLI tests run under appdata2 to balance the shard budgets.
...
- **BUG-2386:** Repository GraphQL metadata now reads real repository settings and implemented repository features instead of fixed compatibility values. `hasWikiEnabled`, `hasProjectsEnabled`, `hasDiscussionsEnabled`, `homepageUrl`, `deleteBranchOnMerge`, and `isTemplate` come from the repository row; `parent` and `forkCount` come from the real fork store; REST repository and migration shapes report `has_discussions: true` because Bleephub serves discussion categories and discussion mutations.
- **BUG-2387:** Pull requests now persist the real head repository and maintainer-edit flag. REST and GraphQL creation resolve GitHub's `owner:branch` head syntax to an actual fork repository, REST/GraphQL responses render `head`, `headRepository`, `headRepositoryOwner`, `isCrossRepository`, and `maintainerCanModify` from stored state, commits/files derive from the fork's git storage, pull request webhook payloads carry real head/base repository objects, synchronize events fire when the fork head branch changes, and merging a fork pull request copies real fork git objects into the base repository before materializing the merge commit.
- **BUG-2388:** Required all-binary build jobs now have ten-minute job and step budgets. The amd64 and arm64 build checks still compile the full binary matrix and remain required, but hosted-runner variance no longer cancels the job at the five-minute boundary before the check can prove the build.
- **BUG-2389:** Bleephub continuous-integration checks now use fully qualified GitHub-compatible service names instead of simulator shorthand: `Bleephub end-to-end`, `Bleephub GitHub software development kit`, `Bleephub GitHub command-line interface`, and `Bleephub GitHub Actions runner`.
- **BUG-2390:** Repository tag listing now peels annotated tag objects through real git storage before rendering the `commit` object. `GET /api/v3/repos/{owner}/{repo}/tags` reports the target commit SHA/URL for annotated and lightweight tags, and the regression creates both tag kinds through the public Git Data tag/ref endpoints instead of accepting an empty list.
...
**Validation**
- `GOCACHE=/private/tmp/sockerless-go-cache go test ./bleephub -run 'TestListTags|TestListRefs_TagsNamespaceEmpty|TestGitData' -count=1` passed with sandbox escalation for loopback listeners after the repository-tag peeling fix.
- `GOCACHE=/private/tmp/sockerless-go-cache go test ./bleephub -count=1` passed with sandbox escalation for loopback listeners after the repository-tag peeling fix.
...
- `GOCACHE=/private/tmp/sockerless-go-cache go test ./bleephub -run TestForkPullRequestRESTAndGraphQL -count=1` passed with sandbox escalation for loopback listeners after fork pull requests gained real head-repository storage.
- `GOCACHE=/private/tmp/sockerless-go-cache go test ./bleephub -run 'Test(CreatePullRequestREST|ForkPullRequestRESTAndGraphQL|GraphQLCreatePullRequest|PullRequest|ListRequestedReviewersREST|PRGraphQL)' -count=1` passed with sandbox escalation for loopback listeners after the pull-request head-repository cleanup.
- `GOCACHE=/private/tmp/sockerless-go-cache go test ./bleephub -count=1` passed with sandbox escalation for loopback listeners after the pull-request head-repository cleanup.
- `GOCACHE=/private/tmp/sockerless-go-cache GOWORK=off go test ./... -run TestPull -count=1` passed in `bleephub/sdk-tests` with sandbox escalation for loopback listeners after the pull-request head-repository cleanup.
...
- `GOCACHE=/private/tmp/sockerless-go-cache go test -run 'TestRepoGraphQL_(GitHubRepoQuery|ViewJSONStaticFields|ForkParentAndCount)|TestDiscussionsGraphQL_Lifecycle' -count=1 ./bleephub` and `GOCACHE=/private/tmp/sockerless-go-cache go test -count=1 ./bleephub` passed after the repository metadata GraphQL cleanup.
- `GOCACHE=/private/tmp/sockerless-go-cache go test ./bleephub -run 'TestConformanceRepoPermissions|TestUnitRepoJSONPermissionsFollowViewerAccess|TestUnitRepoJSONFields' -count=1` and `GOCACHE=/private/tmp/sockerless-go-cache go test ./bleephub -count=1` passed after the REST repository permissions cleanup.
- `pre-commit run --files .github/workflows/ci.yml BUGS.md DO_NEXT.md PLAN.md STATUS.md WHAT_WE_DID.md simulators/aws/cli-tests/rds_complete_test.go simulators/aws/cli-tests/rds_extended_test.go simulators/aws/cli-tests/rds_restore_extras_test.go simulators/aws/cli-tests/rds_state_test.go` passed after the AWS CLI appdata/appdata2 shard rebalance.
