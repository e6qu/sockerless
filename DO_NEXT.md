...
`fix/dynamodb-completeness-safety` — **4 consumer-reported DynamoDB bugs + a wide DynamoDB completeness/safety sweep** (consumer #641–#644 → BUG-2126..2129; sweep 2130..2140; staged 2141/2142). Method: read the (code-pointed) consumer reports, ran 2 read-only audit agents over the DynamoDB slice (completeness vs the vendored smithy spec; safety/robustness), then made **complete fixes** (per the user's directive — no workarounds / quick-fixes-ultimately-wrong) with SDK tests for each.

- **Consumer:** TransactWriteItems `Update` honoured + condition evaluated (2126); per-item `CancellationReasons` + service-prefixed `__type` on cancel (2127); `if_not_exists()` arithmetic computes — split top-level `+`/`-` first, recursive operand eval (2128); DeleteTable purges items (2129).
- **Sweep:** ScanIndexForward + Select=COUNT (2130/2131); exact `big.Rat` numeric-key canonicalization so `01`==`1` (2132); nested UpdateExpression paths with M/L container creation on SET/REMOVE/ADD/DELETE (2133); 32-level write-side nesting limit (2134); parallel-Scan partition (2135); contains() on S/N/B sets (2136); list_append() (2137); Batch/Transact 25/100 limits (2138); ReturnValuesOnConditionCheckFailure+Item (2139); ResourceNotFoundException consistency (2140). +`FuzzDDBItemMarshalRoundtrip`/`FuzzDDBProjectItem`.

**Validation:** aws sim build + `golangci-lint` (0 issues) + unit + the full DynamoDB SDK suite green; the new + migrated-update-expr fuzz clean. **Watch:** the numeric-key fix changed the store-key format (type-tagged + canonical) — touches every item op; all DynamoDB tests pass.

**Staged (filed, deliberately not rushed):** ConsumedCapacity on ~10 ops (2141); DynamoDB PartiQL `ExecuteStatement`/`ExecuteTransaction`/`BatchExecuteStatement` (2142 — own grammar, a follow-on slice).

**Next candidates:** PartiQL (2142) as a dedicated slice; ConsumedCapacity (2141); the live-cloud track (BUG-1075); another fidelity/fuzz round.
