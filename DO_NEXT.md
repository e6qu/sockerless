...
`feat/bleephub-object-store-real-client-tests` — Bleephub object-store test fidelity, Actions byte-object storage and fail-loud log downloads, GraphQL sub-issues, issue-field values, Projects v2 field values/project connections, issue-comment pin state, issue-type assignment, OAuth fail-loud web-flow authorization, GitHub App Manifest creation and browser installation, workflow/runner user-facing path cleanup, user-owned issue sidebar routing, Pages build metadata fidelity, Actions rerun workflow-file identity, fail-loud event ref resolution, paginated/searchable organization audit logs, go-github software development kit Actions dispatch coverage, SQLite-only metadata persistence, continuous-integration setup hardening, cloud-backend lint sharding, and AWS S3 CopyObject list visibility.

**Next**
- Keep tightening Bleephub toward real-service behavior by replacing remaining fake test boundaries, shallow GraphQL resolvers, and shape-only endpoints with real store/object-storage/git-backed behavior. After this branch, BUG-2363, BUG-2362, BUG-1345, and BUG-1075 remained open unless new boyscout bugs were found.
...
- **BUG-2362 partial:** GitHub App creation in the Bleephub user interface and `bleephub/sdk-tests` now uses GitHub's App Manifest flow (`POST /settings/apps/new` followed by `POST /api/v3/app-manifests/{code}/conversions`) instead of `/internal/apps`, and GitHub App installation setup now uses the signed-in browser app-slug installation flow (`POST /apps/{slug}/installations/new`) in the Go server tests, `go-github` compatibility tests, and gh parity script. The remaining open debt is retiring or replacing the legacy operator-only `/internal/apps*` setup surface and assembling OAuth App browser/settings flows for the `/internal/oauth-apps` setup surface.
- **BUG-2363 partial:** The Bleephub dashboard no longer points users at `/internal/exec/workflow` or generic Bleephub runner instructions; workflow text points to GitHub Actions workflow dispatch and the runners page points to GitHub Actions runner endpoint configuration. The remaining open debt is removing Bleephub-specific internal workflow/job submission from runner control-plane harnesses where the official GitHub Actions runner protocol should be sufficient.
