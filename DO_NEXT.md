...
**Current state**: Cells 5/6/7/8 all in flight live with multiple iterations. Closed live-only bugs BUG-907..916. Cells 7+8 currently failing on **BUG-917 OPEN**: cloudrun backend's `/containers/<id>/start` returns NotFoundError "No such container: <id>" (gitlab-runner re-formats as "No such job") for gitlab-runner's short-lived permission containers (chown the cache volume mount). gitlab-runner does `docker create --name=...` → `docker start <id>` very quickly. Dispatcher's cleanup loop filters by managed-by-label so it shouldn't touch arbitrary containers — but PendingCreates lookup STILL fails for the just-created container. **Visibility blocker**: bootstrap.sh redirected sockerless-backend-cloudrun output to `/tmp/sockerless-backend.log` which never surfaced in Cloud Logging — undiagnosable. **gitlab-runner Cloud Run revs 00008+ failing startup (2026-05-03 v4)**: New gitlab-runner-{cloudrun,gcf} revisions fail "user-provided container failed to start and listen on PORT=8080" and traffic stays on rev 00007/00008 (prior working rev WITHOUT helper_image wedge). Earlier diagnosis: STARTUP TCP probe DID succeed (visible in revision logs `Default STARTUP TCP probe succeeded after 1 attempt`). Hypothesis: liveness probe fails later, or `sed -i '/\\[runners.docker\\]/a\\helper_image = ...'` exits non-zero (no match) → `set -euo pipefail` in bootstrap.sh kills script. To debug: `gcloud logging read 'resource.type=cloud_run_revision resource.labels.revision_name=gitlab-runner-cloudrun-00009-5lf' --limit=50 --order=asc --freshness=1h` and look for the cat config.toml output BEFORE `exec gitlab-runner run` — if not visible, sed failed. Fix: change sed to use addressing like `/\\[runners.docker\\]/{p;a\\helper_image = ...\\n;d}` or insert via a different mechanism (heredoc rewrite of full config.toml, or use --docker-helper-image on register if v17 supports with --token). **Cells 7+8 unblock** depends on getting helper_image wedge live.

**Cell 8 also failed same path (2026-05-03 v6)**: Both cells 7+8 failed identically with "No such job: <container-id>" after the helper_image wedge. Confirmed: docker-image-by-ID resolution from `Store.ResolveImage` is failing. Investigation pointer: `backends/core/image_manager.go::Pull` line 168 calls `m.Base.ImagePullWithMetadata` which SHOULD populate Store. Trace whether it actually does for cloudrun backend (might be that BaseServer.ImagePullWithMetadata writes to Store but cloudrun's image store is a different instance, OR the backend's Pull path doesn't go through ImageManager).

**BUG-918 + BUG-919 closed → BUG-920 surfaced (2026-05-03 v7)**: After resolver rewrites `registry.gitlab.com/...` → AR remote-proxy `gitlab-registry/...` and replaces bare digest refs with RepoTag, gitlab-runner CAN now create permission containers via cloudrun/gcf backend. But mid-pipeline (after 1-2 successful creates), docker connection to `tcp://localhost:3375|3376` fails ("Cannot connect to docker daemon"). Backend was healthy earlier (logged `/_ping=200`, `Cloud Build succeeded`, `containers/create=201` — but each create took 130-200s due to Cloud Build for permission container image overlay). Suspect: gitlab-runner's 120s docker connection timeout fires when next stage's container create call queues behind a long-running build, OR Cloud Run instance recycles mid-pipeline. **Investigate**: extend gitlab-runner config `[runners.docker]` `wait_for_services_timeout` + check Cloud Run Service uptime metrics. Bug pattern: cloudrun + gcf are fundamentally slow for per-container creates (Cloud Build + Cloud Run Jobs/Functions deploy). Cells 7+8 may need a different image strategy: pre-built helper images cached in AR (so no per-create Cloud Build), or pool reuse for permission containers (similar to BUG-117 lambda pool).

**Major progress 2026-05-03 v8** — BUG-921 closed (cloudrun /start no longer blocks on RunJob.Wait; uses operation Metadata for execution name). Cell 7 now reaches `prepare_script success → get_sources` stage before BUG-922 surfaces (cloudrun auto-removes container after Cloud Run Job execution completes; gitlab-runner expects container to persist across exec calls — Cloud Run Job (one-shot) vs Service architecture mismatch). Cell 8 still hits BUG-923 (gcf ContainerCreate blocks 150-200s on Cloud Build + CreateFunction.Wait > 120s gitlab-runner docker timeout). BUG-920 wedge (volumes=[]) reverted per user "no fallbacks" rule — /cache mounts properly via cloud resource mapping (Cloud Run Job Volume{Gcs{Bucket}}). Next-session real fixes:
- **BUG-922**: cloudrun ContainerStart should NOT delete the Cloud Run Job after execution; preserve container metadata so subsequent exec creates a new execution on the same Job. OR adopt UseService path which is long-lived.
- **BUG-923**: gcf ContainerCreate must return as soon as Cloud Build + CreateFunction are QUEUED, not after they complete. Move op.Wait to ContainerStart background OR restructure as eager-create-lazy-deploy pattern.

**BUG-918 wedge proven INSUFFICIENT (2026-05-03 v5)**: After gitlab-runner-{cloudrun,gcf} rev 00010+ deployed with helper_image=tag-form pinned via sed, cell 7 STILL failed with same "No such job" error. Trace: `Using helper image: registry.gitlab.com/.../gitlab-runner-helper:x86_64-v17.5.0 (overridden) | Using locally found image version due to "if-not-present" pull policy | Using docker image sha256:180e3... for <tag> with digest sha256:95db8...`. gitlab-runner uses the IMAGE ID (sha256:180e3...) returned by sockerless's image-pull/inspect for the docker create call. So the ref to `ContainerCreate` is `sha256:180e3...` — sockerless's `Store.ResolveImage` STILL fails to find it. **Root fix needed**: cloudrun backend's image Store MUST populate during ImagePull/ImageInspect/ImageList so that subsequent `ContainerCreate(image="sha256:<digest>")` resolves to a known image. Currently the store is in-memory + empty on Cloud Run boot. Investigate `backends/core/image_manager.go::Pull`/`Inspect` to ensure they call `Store.Images.Put(<image>)`.

**Cloud Run startup probe extension done (2026-05-03 v5)**: Both gitlab-runner-{cloudrun,gcf} now have `--cpu-boost --startup-probe="tcpSocket.port=8080,timeoutSeconds=10,periodSeconds=10,failureThreshold=60"` (10-min budget). Earlier failure was backend recovery scan (~4 min) hitting default startup probe timeout. Latest revs 00010-zmw + 00011-dfs are healthy + serving 100% traffic.

**Backend logs visible (2026-05-03 v3)**: bootstrap.sh now does `> >(tee /tmp/sockerless-backend.log >&2)` so backend logs ride stderr → Cloud Logging. Cell 7 retry surfaced the REAL root cause:
...
11. After cells 5+6 GREEN: build serverless gitlab-runner on Cloud Run, register against `e6qu/sockerless` GitLab project, push the cell-7/8 yamls via the existing `tests/runners/gitlab/harness_test.go::runCell` pattern. GitLab PAT at `~/.sockerless/gitlab-pat` (mode 600).
   ...
   **gitlab-runner serverless deployment plan** (resume here):
   - Existing assets: `tests/runners/gitlab/dockerfile-{cloudrun,gcf}/{Dockerfile,bootstrap.sh,Makefile}` already build runner images that bake sockerless-backend-{cloudrun,gcf} + gitlab-runner v17.5 + docker CLI.
   - Gap: bootstrap.sh has no $PORT listener (Cloud Run requirement). Two options: (a) add `nc -l $PORT &` background; (b) change sockerless-backend-{cloudrun,gcf} `-addr :$PORT` so the backend's HTTP server doubles as the healthcheck (cleaner; gitlab-runner uses `DOCKER_HOST=tcp://localhost:$PORT`).
   - GitLab token: registration-token-based registration is deprecated in GitLab 16+. Need to mint a runner auth token via API: `POST /api/v4/runners` with `runner_type=project_type, projects=[<id>], tag_list=[sockerless-cloudrun|sockerless-gcf]` using the glpat from `~/.sockerless/gitlab-pat`. Result has `.token` which becomes `GITLAB_RUNNER_TOKEN`. Store in Secret Manager `gitlab-runner-token-{cloudrun,gcf}` with read by runner SA.
   - Add same SharedVolumes wire-up: bootstrap sets `SOCKERLESS_GCP_SHARED_VOLUMES=runner-workspace=/tmp/runner-work=<bucket>,runner-externals=/opt/runner/externals=<bucket>` from a `RUNNER_WORKSPACE_BUCKET` env passed by Cloud Run deploy. The deploy itself adds Volume{Gcs{Bucket}} + VolumeMount on `/tmp/runner-work` + `/opt/runner/externals`.
   - Build + push: `gcloud builds submit --config=tests/runners/gitlab/dockerfile-cloudrun/cloudbuild.yaml ...` (need to add cloudbuild.yaml; mirror github-runner-dispatcher-gcp shape). Same for gcf.
   - Deploy: `gcloud run deploy gitlab-runner-{cloudrun,gcf} --min-instances=1 --no-cpu-throttling --service-account=sockerless-runner@... --set-secrets=GITLAB_RUNNER_TOKEN=gitlab-runner-token-{cloudrun,gcf}:latest --set-env-vars=GITLAB_URL=https://gitlab.com,GITLAB_RUNNER_TAGS=sockerless-{cloudrun,gcf},SOCKERLESS_GCR_PROJECT=...,SOCKERLESS_GCR_REGION=us-central1,SOCKERLESS_GCP_BUILD_BUCKET=sockerless-live-46x3zg4imo-build,SOCKERLESS_GCP_SHARED_VOLUMES=runner-workspace=/tmp/runner-work=sockerless-live-46x3zg4imo-runner-workspace,runner-externals=/opt/runner/externals=sockerless-live-46x3zg4imo-runner-workspace ...`.
   - Pipeline trigger: `tests/runners/gitlab/cell-{7,8}-*.yml` already on disk on PR #123. Push branch to `origin-gitlab` to trigger pipeline. The runner picks up the job via tag match.
   - Cell GREEN gate: same three-URL capture as cells 5+6 (GL pipeline URL + Cloud Run Service revision URL + Cloud Logging URL).
