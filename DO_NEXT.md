...
- Verification so far: focused Azure SDK + CLI tests pass; focused GCP SDK + CLI tests pass; focused AWS SDK coverage-audit tests pass; Azure and GCP Terraform validate; Azure validate still emits the tracked AzureStack provider warning BUG-1584. CI exposed Spanner DDL route/metadata gaps, Logic Apps definition-shape bug, GCP/Azure helper-image remote-base dependency, and ACI response-shape bug, now fixed on the branch; Bigtable Terraform coverage is tracked as BUG-1585 because the provider bypasses the REST custom endpoint and attempts real Google auth.
...
- **#469** Azure ACR `/oauth2/exchange` + `/oauth2/token` (deterministic ACR-shaped tokens; fixed AAD-v1 middleware swallowing the bare `/oauth2/token`). **#470** EC2 launch-template provenance via `aws:ec2launchtemplate:*` **system tags** (SDK `Instance` has no LT field — provider reads tags; reverted a dead `<launchTemplate>` XML approach). **#471** route `NetworkInterfaceId`. **#472** SG egress `Ipv6Ranges`. **#473** ELBv2 listener `SslPolicy`. SDK+CLI each; #470–#473 added to the `idempotency-fidelity` TF stack (verified `plan -detailed-exitcode==0`). All 5 reproduced against running sims first.
- Merged: PR #474 (azf attach deadline), #468 (#465+#467), #466 (#457–#464), #463 (#453–#455+sweep). Only #394 (azuread upstream) open after this batch.
- **BUG-1505 fixed:** the flaky `test (azure backends)` 5-min timeout. Root-caused via the panic goroutine dump to `TestAZFGitLabRunnerAttachStdin` → buffered `attachStream.Read` strands on the invoke publish (600s POST timeout) when a contended CI pod expires before the instant workload runs; the test's 5m guard == global `-timeout 5m` → suite panic. Fix: bound `Read` with a deadline (`SOCKERLESS_AZF_ATTACH_TIMEOUT_SEC`, default = invoke Timeout, pure safety net) → EOF; integration backend runs 60s. Deterministic unit tests in `attach_stream_test.go`. **Note:** the dead-code "missing publish" hypothesis was investigated and reverted — `attachStream` only ever exists alongside a `stdinPipe`, so the no-stdin path can't strand a reader.
- Merged: PR #468 (#465 + #467), PR #466 (#457–#464), PR #463 (#453–#455 + sweep). All consumer issues #453–#467 fixed; only #394 (azuread upstream) open.
- **#465 (BUG-1504):** consumer reported ECR manifest HEAD returns 400 (go-containerregistry); **could not reproduce on current main** — built a throwaway g-c-r client, HEAD→404 across 1/2/3-segment repos and the full push works. Real defect fixed: only `GET /v2/` set `Docker-Distribution-Api-Version`; now set on **every** `/v2/` response in the shared `serve()` (all 3 cloud copies identical). Regression test `TestECR_OCIManifestHeadMissing`. Hardening, not a proven 400 repro. **Next: ask the consumer (via #465) for their exact commit SHA + g-c-r version + proxy/auth.**
- **#467 (BUG-1506):** ECS `DescribeTaskDefinition --include TAGS` returned no tags (sim leaked them inside `taskDefinition.tags`, dropped by the SDK model; provider reads response top-level `tags`). Fix: `ECSTaskDefinition.Tags`→`json:"-"`, emit top-level `tags` from Register (always) + Describe (include=TAGS only). SDK+CLI + `aws_ecs_task_definition` added to the `idempotency-fidelity` TF stack (simple container def stays idempotent). ListTagsForResource already worked.
- **BUG-1505 (open, P3):** flaky `test (azure backends)` 5-min integration timeout (reverse-agent FaaS dial-back, CI-only, not locally reproducible) — surfaced on #468 CI, causally isolated from the change. Needs a focused azf-integration session.
- Merged earlier today: PR #466 (#457–#464), PR #463 (#453–#455 + sweep). Consumer issues #453–#464 closed; #465/#467 fixed by PR #468; #394 upstream-blocked.
