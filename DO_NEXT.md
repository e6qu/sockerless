...
`feat/bleephub-object-store-real-client-tests` — Bleephub object-store test fidelity, Actions byte-object storage and fail-loud log downloads, GraphQL sub-issues and issue-field values, issue-type assignment, user-owned issue sidebar routing, Pages build metadata fidelity, Actions rerun workflow-file identity, fail-loud event ref resolution, paginated/searchable organization audit logs, go-github Actions dispatch coverage, SQLite-only metadata persistence, CI setup hardening, cloud-backend lint sharding, and AWS S3 CopyObject list visibility.
...
- **Docs/continuity:** Bleephub's README documents that S3 filesystem tests use the AWS simulator object-store slice rather than a local fake; it also documents Actions object-byte storage, no-github.com action resolution, rerun workflow-file identity, the go-github reference adaptor, default-branch workflow discovery, SQLite-only metadata persistence, and the now-real audit-log, marketplace, app-installation-request, webhook-config, runner-refresh, and org people-management surfaces. Continuity files reflect PR #779 as merged and this branch as active.
