...
`feat/iam-conformance-eventing` — **IAM policy-engine conformance system + completeness + service-initiated event delivery (BUG-2186/2187).**

- **Conformance system (2186).** `simulators/aws/iam_conformance_test.go` — an operator catalog, a condition-key registry (the non-conformity map), a golden-corpus evaluator (`testdata/iam_conformance_vectors.json`), and a **ratchet** that fails CI if the gap set drifts; plus `sdk-tests/iam_conformance_differential_test.go`, a coordinates-only `SimulateCustomPolicy` differential (sim by default, real AWS via `SOCKERLESS_IAM_ORACLE=aws`). Closed every gap → **26/26 operators, 24/24 condition keys**: `BinaryEquals`; the global keys + MFA (STS records MFA on sessions); the **Organizations slice** (`organizations.go`) for `aws:PrincipalOrgID`; `aws:ResourceTag/<k>` for 16 services beyond EC2; case-insensitive key names. Process doc: [docs/SERVICE_CONFORMANCE.md](docs/SERVICE_CONFORMANCE.md).
- **Service-initiated calls (2187).** Service-principal matching + `iamEvalServiceInitiated`/`iamAuthorizeServiceDelivery`, and **real event delivery**: SNS Publish → SQS/Lambda, EventBridge PutEvents → SQS/Lambda, S3 ObjectCreated/Removed → SQS/SNS/Lambda — each authorized against the target's resource policy with the service context (`aws:SourceArn`/`SourceAccount`/`CalledVia`/`ViaAWSService`). A delivery happens only when the target policy admits the source service; `aws:ViaAWSService=false` on direct calls.
- Tests: SDK + CLI prove the conformance gate, every closed key, and positive+negative delivery (no policy / wrong SourceArn → no delivery). aws sim/sdk/cli build/lint(0)/unit green; contract + cli-shard guards pass.

**Next candidates:** apply the conformance process (docs/SERVICE_CONFORMANCE.md) to the next slice. **BUG-2175 remaining** — Cosmos RU model / autoscale, stored procedures, change feed, consistency/session. Per-service resource-ARN derivation for the remaining AWS query/json services. Then live-cloud (1075). Open GitHub issues: #394 (azuread upstream-blocked).
