...
The AWS S3 bucket-subresource implementation follow-up from the stale surface-table cleanup is now closed. Issue #281 / BUG-1221 no longer remains as hidden table debt for the affected public-API fixes and selected coverage rows; remaining row-level SDK/CLI/Terraform coverage debt is tracked by issue #285 / BUG-1226.
...
Current phase: post-merge continuity, local harness, and simulator surface audit checks are closed. GitHub had no open issues when this pass started, so no issue-driven implementation branch was active after the Azure Key Vault data-plane PR merged.
...
The proactive simulator audit corrected the stale S3 bucket-subresource tracking reference by opening issue #285 / BUG-1226 for the remaining row-level SDK/CLI/Terraform coverage audit. Next implementation pass: start from the current GitHub open-issue list. If there are still no community-filed issues beyond #285, use BUG-1226 or BUG-1104 as the standing audit trigger, file any new concrete simulator gap before fixing it, and keep continuity docs in past tense in the same PR.
