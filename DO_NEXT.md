...
`feat/cloudrun-topology-cell` (PR pending) — **Cloud Run GitHub-topology cell bring-up (partial).** New `provision_cloudrun()` in `bleephub/test/run-integration.sh` (gcp sim on :4567 + gRPC :4577, GCS buckets, gcs-sync shared volumes, fake-SA-JSON auth, AR-endpoint coordinate, reverse-agent bootstrap, `SOCKERLESS_GCR_USE_SERVICE=1`), `bleephub-runner-docker-test-cloudrun` Make target (registry-port publish parameterized; registry-trust target generalized), Dockerfile bundles simulator-gcp + sockerless-backend-cloudrun + openssl, and a base-image pre-pull (ECR gallery) for the sim's docker-hub pull-through. **Six real bugs fixed** that take TEST 12 through the full pipeline (build→push→pull→deploy→materialize→reverse-agent→step exec):
- **BUG-1789** (gcp-common, two facets): `ResolveGCPImageURI` base-image rewrite + `ARAuthProvider.{IsCloudRegistry,RegistryEndpoint}` now honour the `SOCKERLESS_GCP_AR_ENDPOINT` coordinate (`OverlayRegistryHost` / `IsOverlayCoordinateRegistry`), so the overlay `FROM` and the metadata fetch route to the sim instead of the real AR host.
- **BUG-1790** (gcp sim): `hydrateOCIImageFromLocalDocker` served a mixed OCI-manifest/Docker-config image; now full-OCI.
- **BUG-1791** (cloudrun, two facets): materialize-on-exec for a deferred no-services GH job container, and `skipDefaultInvoke` so the exec-driven container's keepalive isn't run as a request (which SIGTERMed the pod).

### Remaining work

1. **BUG-1792 — gcs-sync workspace reachability (last TEST 12 gate).** The bootstrap's gcs-sync restore/save hardcodes `https://storage.googleapis.com`; against the sim every `docker exec` aborts at exit 255 (pre-exec restore fails). Fix: bootstrap honours `STORAGE_EMULATOR_HOST`; the workload reaches the sim's storage via the published-port + `host.docker.internal` coordinate the reverse-agent callback already uses (the in-container endpoint is not workload-reachable). See BUGS.md BUG-1792.
