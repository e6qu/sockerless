...
`feat/azf-clouddns-hardening` (ready for PR). The azure-cells PR **#587 merged** (`084b62dd`) — both aca + azf bleeplab GitLab cells GREEN; the full GitLab docker-executor arc is now green on every cloud backend. This branch hardens the merged cloud-dns work; **both tasks done**.

### This branch — tasks (DONE)

1. **azf `NetworkConnect` alias registration — DONE.** Under cloud-dns, `NetworkConnect` was a bare passthrough — a `docker network connect --network-alias X net container` after create/start registered no Private-DNS record (the cell only worked because gitlab-runner creates containers *with* the network, so `startCloudDNSSite` read the alias from `NetworkSettings` at start). Root: the `SyntheticNetworkDriver.Connect` only writes `Store.Containers`, which the stateless azf backend ignores. Fix (`cloudDNSNetworkConnect`, backends/azure-functions/network_clouddns.go, wired into `NetworkConnect` in backend_delegates.go behind the cloud-dns config): **connect-before-start** stamps the network+aliases onto the PendingCreate (so `startCloudDNSSite` VNet-integrates + registers them, exactly as create-with-network does); **live connect** VNet-integrates the running site into the network's subnet and writes the aliases as Private DNS CNAMEs now. Unit test `TestCloudDNSNetworkConnect_StampsPendingCreate`.
2. **Swift VNet-integration testing contract — DONE.** The `PUT/GET/DELETE .../sites/{name}/networkConfig/virtualNetwork` endpoint (SDK test `TestSDK_SiteVNetIntegration_RoundTrip` from #587) now also has: **CLI** — `TestSiteVNetIntegration_CLI_RoundTrip` (`az rest` PUT/GET/DELETE round-trip, mirroring the functions CLI tests' ARM-REST pattern); **Terraform** — `azurerm_app_service_virtual_network_swift_connection` + a `Microsoft.Web/serverFarms`-delegated subnet + an EP1 plan + function app, in the apply/idempotency/destroy stack (`apply_test.go` asserts the swift connection + delegated-subnet ARM paths). Surfaced + fixed two sim fidelity bugs: **BUG-1831** swift PUT force-started a container for any non-HTTP site → 500 on an imageless function app's VNet integration (now gated on `siteContainerImage != ""`; redis-with-image path unchanged); **BUG-1832** subnet delegation dropped `properties.delegations[].properties.actions` → azurerm idempotency drift (added `Actions []string` round-trip).

**Local validation (all green):** azure sim build + unit tests; SDK swift/functions/subnet; the new CLI swift round-trip; `terraform validate` against the real azurerm provider schema; azf backend full suite + the new `cloudDNSNetworkConnect` unit test. The TF apply/idempotency/destroy runs in the `tf (azure)` CI job (skips locally on macOS — no realexec network host).

**Next (own PR):** **B. GitHub `actions/runner` cells on aca + azf** — mirror the merged GitLab arc for true GitHub+GitLab parity on the Azure backends. Then **C. standing** — live-cloud pass (BUG-1075), releases (#363), fresh sim fidelity audits.
