...
`feat/gcp-grpc-dataplane` — **audit + coverage close-out of the landed Bigtable gRPC data plane (BUG-2237) + CloudBuild test-hang fix (BUG-2238) + AGENTS.md "No skip-if-absent tests" rule.** PR #656 already landed the Bigtable Data API v2 gRPC slice (`bigtable_data.go`, 740 lines) plus Bigtable admin gRPC, mounted in `main.go:startGRPCServer`. An audit (`grep` across sdk-tests/cli-tests/terraform-tests) found the existing SDK test exercised only 2 of 18 RowFilter types, 1.5 of 4 mutation kinds, 1 of 2 ReadModifyWrite rules, never `ApplyBulk`/`SampleRowKeys`; the CLI half was admin-only (`gcloud bigtable instances/tables create|list`) — the canonical data-plane CLI `cbt` was never invoked, so the data plane had no second-adaptor coverage at all.

**This branch closes those gaps:**

- **SDK** (`bigtable_data_test.go`): `TestBigtable_DataPlane_Filters` — 18 subtests, one per implemented RowFilter, each asserting real survival semantics (e.g. `ValueRangeFilter(["a1","a9")` keeps the three `f1:a` cells; `CellsPerRowOffsetFilter(2)` slices the flattened family/qualifier/ts order; `StripValueFilter` zeroes every value; `RowSampleFilter(1.0/0.0)` keeps/drops every row). Plus `TestBigtable_DataPlane_Mutations` (DeleteCellsInColumn/Family/TimestampRange), `TestBigtable_DataPlane_AppendValue` (the second ReadModifyWrite rule), `TestBigtable_DataPlane_ApplyBulk` (the `MutateRows` batch path, incl. a partial-failure case proving an unknown-family entry surfaces as a per-entry error while siblings stay OK), `TestBigtable_DataPlane_SampleRowKeys` (150 rows so the sampler's ~100-row cadence produces multiple samples). New tests confirmed the sim was already faithful (the one assertion bug they surfaced — `ValueRangeFilter` end-exclusive semantics — was in the test draft, not the sim).
- **CLI** (`spanner_dataflow_bigtable_test.go`): `TestBigtableCLI_DataPlane_cbt` drives `cbt createtable`/`createfamily`/`set`/`lookup`/`read`/`count` over gRPC through `BIGTABLE_EMULATOR_HOST` (the same coordinate the real client uses for the real emulator). `cbt` is built in TestMain via `go install cloud.google.com/go/bigtable/cmd/cbt@v1.13.0` into the test tmp dir so the test runs **unconditionally** (no skip-if-absent).
- **Boyscout (BUG-2238):** `TestCloudBuild_FaithfulBuildPush` + its setup helpers issued `docker pull/run/push` with no per-operation deadline, so a wedged container runtime (Podman-on-macOS gvproxy 500 state) hung the whole sdk-test suite for 8–10m. Bounded every `docker` call through a new `dockerCLIWithTimeout` helper (180s pull / 60s run/rmi/inspect) + the synchronous build POST through a 120s `context.WithTimeout`. A wedged runtime now fails in ~3min with a clear error instead of an 8m hang. No behavior change on healthy runs.
- **Rule (AGENTS.md):** added "## No skip-if-absent tests" section — codifies that tool-absent `t.Skip` tests are deceptive dead code; either install the dependency in `TestMain` or delete the test (the one narrow exception is a `runtime.GOOS`-gated skip for a kernel capability a foreign OS cannot provide).

**Next candidates:** GCP — more of Compute v1's 1994 (the next control-plane tranche), Spanner/SQL/BigQuery/Dataproc remainders, or extend gRPC to Pub/Sub (google.pubsub.v1) / Firestore (google.firestore.v1) native APIs. Azure — web-arm 161→ more of the 692, msgraph. Or the live-cloud track (BUG-1075). Open GitHub issues: only #394 (azuread, upstream-blocked).
