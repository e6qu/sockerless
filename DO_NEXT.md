...
`feat/ecs-gitlab-builds-volume-1801` (Arc 3 Phase 3; PR pending — diagnosis correction). The bleeplab GitLab ECS cell is **one gate from green**: helper stages run, script delivered, `/builds` writable, and the `/builds` volume IS shared across stages. **Final gate — BUG-1801** (re-diagnosed): the build job's project dir isn't created because bleeplab doesn't serve git. (BUG-1798 merged #576, BUG-1800 merged #577.)

### Arc 3 remaining work

1. **BUG-1801 — the final Phase-3 gate: bleeplab must serve the project as a git repo.** After BUG-1800 the build `step_script` writes to `/builds` fine but fails `sh: cd: can't cd to /builds/project-1: No such file or directory`. **Re-diagnosed (the original "volume not shared" was a stale-data-dir artifact):** a `DIAG resolveBindToVolume` log proved the `/builds` docker volume resolves to ONE shared EFS access point across all three stage tasks (`AccessPointForVolume` is idempotent). The real cause: gitlab-runner creates `CI_PROJECT_DIR` (`/builds/<project>`) by **cloning the project repo** in get_sources, then auto-`cd`s there; the harness uses `GIT_STRATEGY: none` (bleeplab serves no git), which skips both the clone and the dir creation. (`pre_get_sources_script` doesn't help on gitlab-runner 18.) **Fix:** add a git smart-HTTP slice to bleeplab — seed a real on-disk repo per project from the committed `.gitlab-ci.yml`, serve `GET /<project>.git/info/refs?service=git-upload-pack` + `POST /<project>.git/git-upload-pack` (shell out to `git upload-pack`), set the job's `git_info.repo_url` to that URL, and switch the harness jobs to the default `GIT_STRATEGY: clone`. The runner then clones the project, creating `CI_PROJECT_DIR`. Everything else in the pipeline works; this is a bleeplab feature, not a sockerless backend/sim bug.
