...
`fix/dynamodb-set-rhs-parens` — **consumer #648 (follow-up to #643): SET RHS doesn't strip enclosing parentheses** (BUG-2149). `(if_not_exists(c,:0) - :v)` and even `(:z)` stored null — `ddbEvalSetRHS` never stripped a fully-enclosing `(...)`, so the leading `(` hid the operator from the top-level `+`/`-` split. ElectroDB **always** wraps `.subtract()`/`.add()` RHS in parens, so every ORM counter decrement corrupted to null. Fixed with a new `ddbStripParens` (removes balanced fully-enclosing parens, repeatedly — but leaves `(a) - (b)` intact) called in both `ddbEvalSetRHS` and `ddbEvalSetOperand`. SDK test `TestDDBIssue648_ParenthesizedSetRHS` + the update-expr fuzz green.

**Next candidates:** the live-cloud track (BUG-1075); a fresh fidelity/fuzz round (another sim slice — S3, SQS, Step Functions). No other actionable DynamoDB items remain. Open GitHub issues: only #394 (azuread upstream-blocked); #641–#644 + #648 closed. Possible PartiQL extensions if a consumer needs them: nested-path WHERE predicates, SET arithmetic/list_append in PartiQL UPDATE, read/write-mix validation in batch/transaction (intentionally out of the PartiQL slice).
