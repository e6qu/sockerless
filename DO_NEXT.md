...
Phase 168 follow-up merged 2026-05-18 (PR #169, `0bd75902` on `origin/main`). Local `main` is synced to `origin/main`.

No active PR branch. Remaining tracked work is in `BUGS.md`: 1072, 1074, 1075, 1076. BUG-1075 needs real live-cloud credentials/setup; do not fake or mark it done without a live run.
...
| **P168.6** | ✅ | Bootstraps detect ENOSPC writes → exec envelope `exit_code=28` + operator-guidance message. Shared helper in `agent/enospc.go` (`DetectENOSPC` + `AnnotateENOSPC` + `ENOSPCExitCode=28`); wired into lambda + GCF + cloudrun bootstrap exec-result construction. AZF bootstrap was added later in P168.14; its ENOSPC-specific helper coverage remains part of BUG-1076's bootstrap test-pyramid gap. |
...
| **P168.9** | ✅ | E2E/readiness track merged across PR #168 (`3565e413`) and PR #169 (`0bd75902`). Cloud Run + GCF overlay-baked bootstrap exec pass real simulator backend tests (`ContainerStart` reverse-agent registration + `docker exec` over WebSocket), and full Cloud Run/GCF backend package simulator suites pass after scoping the exec e2e harness and fixing implicit-network cleanup (BUG-1081 / 1082). CI runs `26002912832` / `26003621756` / `26003955164` exposed harness/simulator regressions, fixed before merge: Cloud Run/ACA smoke declares a reachable callback URL, listens on all container interfaces, opens stdin for non-exec smoke containers, and preloads `alpine` into the simulator host Docker daemon; the GCP simulator overlay readiness probe now TCP-dials the bootstrap listener instead of invoking `/`; Cloud Run v2 Service EnvVar decoding accepts both real proto-JSON oneof shapes and SDK coverage asserts reverse-agent env round-trips; GCP overlay build platform is explicit in env and unified config, and simulator tests use the host platform; Cloud Run Service simulation keeps a real per-Service container instance alive until delete. CI run `26004958009` then exposed BUG-1087 in GCF one-shot lifecycle state, and full local GCF package validation exposed BUG-1088 in simulator cold-start invoke handling; both fixed. CI run `26011828766` exposed BUG-1089 and closed with BUG-1068: ACA Jobs no longer wait for a bootstrap on stock one-shot images, and Azure simulator ACA Apps start real App-replica containers with `ContainerAppName_s` logs. CI run `26012702755` exposed BUG-1090; Azure SDK monitor tests now resolve `Log_s` by column name. CI run `26013037364` exposed BUG-1091; GCF/Cloud Run harnesses no longer do a redundant ECR Public alpine pull. BUG-1069 fixed: ACA Apps now wrap non-overlay images through ACR Tasks with `SOCKERLESS_ACA_BOOTSTRAP`. Full ACA validation exposed and fixed BUG-1092: Azure simulator ACA Job execution container names no longer truncate to collision-prone 12-char prefixes. CI run `26013907371` exposed and fixed BUG-1093: GCF/Cloud Run now build local Docker Hub / AR alpine tags from the real ECR Public base instead of assuming BuildKit left the source base tag available to retag. CI run `26014253175` exposed and fixed BUG-1067/1071: AZF now has a real bootstrap, ACR overlay path, simulator HTTP invocation, reverse-agent registration, and SIGTERM lifetime-expired signaling. PR #169 green runs: `26025440106`, `26025985194`, `26027497573`, `26028982114`, `26029489706`. AZF/GCF/ACA all have verified GitLab-style attach-before-start stdin delivery. Remaining follow-ups moved to BUGS: 1072, 1074, 1075, 1076. |
