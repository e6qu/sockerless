...
`debug/cloudrun-reverse-agent-blocker-1794` (PR pending — docs + the bootstrap timeout only). Everything substantive is merged: #570 (the #569 process-mode managed-EBS fix + the full cloudrun gcs-sync data plane), #568 (gcs-sync prereqs), #567 (cell bring-up). TEST 12 still fails — root cause is now **BUG-1794** (below), filed after a fresh podman machine ruled out the degraded-engine theory.

### Remaining work

1. **BUG-1794 — the cloudrun Service bootstrap never registers its reverse-agent (THE live TEST 12 blocker).** The materialize-on-exec deploys the Service, the backend logs `service is Ready` + correctly skips default-invoke, but no reverse-agent registers → `materializeDeferredNetworkPodForExec` times out → the job hangs. The earlier "`gcsSyncPreExec` no-op / no `/sockerless-runner-ws/` storage" symptom is downstream of this — `gcsSyncPreExec` is simply never reached. **Confirmed:** overlay build→push→pull works (a freshly-created podman machine needs `podman machine stop && start` so buildah honors the sim-registry insecure drop-in — see the harness note + BUG-1794); `host.docker.internal:3375` IS reachable from a container during a live run; the Service (overlay) container is created and **removed within ~1s** so its bootstrap stderr can't be captured (`docker ps -a` never shows it; a `docker events`-triggered `docker logs -f` got nothing). So the bootstrap exits/crashes in <1s or `ContainerStart` fails. **Do next:** add a sim debug knob to NOT auto-remove a stopped Cloud Run Service container (`simulators/gcp/cloudrunservices.go` `stopCloudRunServiceInstance`/`ensureCloudRunServiceInstance`) — or persist the `StreamContainerLogs` sink — so the bootstrap's startup/crash is visible; OR run the built overlay's bootstrap standalone with the exact injected env (`SOCKERLESS_CALLBACK_URL`, `SOCKERLESS_CONTAINER_ID`, `SOCKERLESS_SYNC_MOUNTS`, `STORAGE_EMULATOR_HOST`). The failure first appeared after the BUG-1792 prerequisite bootstrap changes (run #6 registered; runs since don't) — check whether those regressed the bootstrap startup, but causation is unconfirmed. Run on a clean engine; fresh-podman quirks: `docker run` (attached) → "unable to upgrade to tcp, received 500"; mac `/tmp` not mounted into the VM.
2. **BUG-1792 validation** — once BUG-1794 is fixed, confirm the gcs-sync workspace round-trip (TEST 12 `proof.txt` assertion) and remove the `gcsSyncPreExec` Info diagnostic.
3. **TEST 13 (service container) + TEST 14 (dispatcher)**; then **GCF topology cell**; **FaaS multi-container pod assembly (BUG-1781)**; **Arc 3 GitLab parity.**
