...
`feat/bleephub-repository-license-graphql` — Bleephub repository metadata now uses real persisted repository, git, and Pages state for license metadata, feature flags, merge-method settings, pushed timestamps, archive timestamps, template-generation provenance, and Pages capability instead of null/hardcoded compatibility shapes.
