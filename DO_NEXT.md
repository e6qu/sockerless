...
**Staged for follow-up PRs (filed BUG-1909–1921 with fix-shapes, NOT rushed):** the larger state-machine / concurrency items — Batch lifecycle states (1909), Lambda async-Event log fabrication (1910, a synthetic-behavior rule violation — prioritize), ECS StopTask real exit code (1911), SFN Task/history (1912), `awsPage` snapshot-stable cursor (1913), S3 conditional writes (1914), S3 us-east-1 idempotency (1915), GCP compute-op store (1916), GCP Cloud Run v1 traffic (1917), GCP MIG fingerprint (1918), Azure ARM If-Match validation (1919) + Cosmos If-Match (1920), backend cleanup-swallow logging (1921).
