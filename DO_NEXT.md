...
`feat/bleeplab-azure-cells` → **PR #587 open**. **bleeplab GitLab cells on aca + azf, in one PR.** gcf (PR #586) merged. **Both aca + azf cells GREEN** (BUG-1828 closed). Next: ensure CI green on the push, then the PR is ready for review.

### State of this branch (updated)

- **aca cell — GREEN (all 4 tests, incl. the redis `services:` job).** Fixed BUG-1825 by routing runner stages through the bootstrap HTTP buffered-invoke to the App ingress (`runACAStageInvoke`), with the azure sim implementing **faithful ACA ingress** (`registerContainerAppsIngress`: Host=App-FQDN → reverse-proxy to the replica's `targetPort`; replaced an earlier sim-specific `/aca-app-invoke` smell — sims must mirror their clouds, differing only in endpoint/port coordinates). Backend WS keepalive also landed (`agent.ReverseAgentConn`), fixing a real all-FaaS infinite-hang (cloudrun cell re-verified green).
- **azf cell — GREEN (all 4 tests), BUG-1828 closed.** The full GitLab docker-executor flow runs on App Service: build (clone + gcc `6 x 7 = 42`), cross-stage `calc` artifact, redis `services:` (PING/SET/GET) reached by alias over the per-build network. The final cloud-dns service discovery is assembled from real Azure primitives (VNet + `Microsoft.Web/serverFarms`-delegated subnet + Private DNS zone linked to the VNet; App Service regional VNet integration via `WebApps` swift connection; `--network-alias` → Private DNS CNAME → site FQDN), the same backend code vs the sim and real Azure (no sim-awareness). The sim realizes a Web-delegated subnet as a Docker network (App Service container fabric, not netns), attaches the site on swift integration, and realizes a CNAME → site-FQDN as an embedded-DNS alias. Both aca + azf cells now GREEN; PR #587.
- **AWS sim fixes (this branch):** BUG-1827 — ECS now enforces advertised CPU/Memory on the launched container (cgroup `Memory`/`NanoCPUs` from the task/container def); process-mode managed-EBS hardened against the nil-Docker panic (#583/#569). SDK probes green.
