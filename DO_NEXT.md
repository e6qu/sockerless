...
`feat/faas-pod-polish-bleeplab-cells` (PR pending). **AZF pod polish + bleeplab artifact UI.** (1) AZF pod **shared-workspace volume** (members' binds → site-level Azure Files share via `UpdateAzureStorageAccounts` + per-sitecontainer `VolumeMounts`; sim realizes a shared Docker volume across members, torn down on site delete). (2) AZF **per-sidecar exec** (sidecars run the overlay in *sidecar mode* `SOCKERLESS_SIDECAR=1` → dial own reverse-agent + exec service, no HTTP-port bind → `docker exec <sidecar>` works; raw-image fallback). (3) **bleeplab artifact browse UI** (`jobView.artifact_filename` + unauthenticated `GET /internal/jobs/{id}/artifact` + JobDetail Artifacts panel). Proven by the extended `TestAZFMultiContainerPodSharesLocalhost` (shared `/shared` file + sidecar `docker exec`) + `TestArtifactFlow`. Merged previously: #582 (BUG-1781 FaaS pods), #581 (BUG-1804/1805), #580 (UI), #579 (artifacts), #578 (git + ECS binds).

### NEXT (own PR): bleeplab GitLab cells on cloudrun / gcf / aca

The ECS cell is green; extend the harness — it's additive (one Dockerfile + the `BLEEPLAB_BACKEND` case switch, mirroring bleephub's proven 4-backend harness):
1. **`bleeplab/Dockerfile`**: add `simulator-gcp` + `simulator-azure` builds; `sockerless-backend-{cloudrun,gcf,aca}` + `sockerless-{cloudrun,gcf}-bootstrap`; `openssl` (GCP fake-SA RSA key) to the apt install.
2. **`bleeplab/test/run-integration.sh`**: add `write_fake_sa_json` + `provision_cloudrun`/`provision_gcf`/`provision_aca` (copy bleephub `bleephub/test/run-integration.sh` `provision_*`, dropping the github-externals AP, staging `redis:7-alpine` for the overlay `FROM`); add the `case` arms. The control-plane section (project/CI/runner/pipeline) is backend-agnostic — unchanged.
3. **root `Makefile`**: add `bleeplab-runner-docker-test-{cloudrun,gcf,aca}` via the existing `run_bleeplab_harness` macro with `-p 5000:<sim-port>` + a `bleeplab-sim-registry-trust` dep (reuse `bleephub-sim-registry-trust`).
4. Validate each GREEN. **cloudrun first** (smallest lift, fully ✅ in the matrix). **gcf** watch **BUG-964** (default-invoke gating for the multi-container pod-Service — the `services:` redis job is exactly what trips it). **aca** adds the azure sim + ACR + `*.blob.localhost` hosts alias + `SIM_AZURE_ARM_EXTERNAL_DATA_PLANE_URLS_JSON`. Per-backend deltas: ECS has no overlay/registry; cloudrun/gcf use Cloud Build→AR + `SOCKERLESS_GCP_AR_ENDPOINT=127.0.0.1:5000` + `gcs-sync`; aca uses ACR Tasks + `SOCKERLESS_AZURE_ACR_ENDPOINT=127.0.0.1:5000` + Azure-Files. Reverse-agent callback `/v1/{cloudrun,gcf,aca}/reverse`. Registry-trust note: a fresh podman machine needs `podman machine stop && start` for the `:5000` insecure drop-in to load.
