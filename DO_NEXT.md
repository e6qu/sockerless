...
`feat/sim-efs-access-point-creationinfo-1800` (Arc 3 Phase 3; PR pending). **BUG-1800 fixed — the `/builds` write gate is closed** (BUG-1798 attach-stdin merged in #576). The harness now writes to `/builds` in the build container. **Next gate — BUG-1801** (the `/builds` volume isn't shared across the per-stage tasks).

### Arc 3 remaining work

1. **BUG-1801 — the next Phase-3 gate (gitlab `/builds` volume not shared across ECS stage tasks).** After BUG-1800 the build `step_script` writes fine but fails `sh: cd: can't cd to /builds/project-1: No such file or directory`. gitlab-runner shares ONE `/builds` volume across stages (get_sources clones into it, step_script runs there); on sockerless ECS each stage is a fresh Fargate task and the dir `get_sources` created isn't visible to the `step_script` task. Frozen-stack evidence: the build task's access-point host dir holds only its own `project-1.tmp`, and there are many separate `…/<fsID>/sockerless/v/<hash>` dirs — the same docker volume resolves to a **different EFS access point + filesystem per task**. Investigate `efs.AccessPointForVolume(volName)` (aws-common) idempotency (same volume name → same access point + filesystem across tasks) + whether the runner reuses one volume name across stages. Repro: `make bleeplab-runner-docker-test-ecs`; `BLEEPLAB_HOLD=1`, then inside the harness `find /tmp/sockerless-bleeplab-data -path '*sockerless/v/*' -type d` shows one access-point dir per stage.
