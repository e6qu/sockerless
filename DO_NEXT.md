...
- **azf cell — RED, BUG-1828 (open).** Overlay base-ref (BUG-1826) + azure-sim AZF-invoke-by-container-IP (BUG-1827) fixed, so cache-init runs; but job 4 then fails `No such container (exec.go:72)` — the azf FaaS invoke is ephemeral (container per `/api/function`, removed after) while the runner pattern needs a persistent workload container. **Fix shape:** run the App-Service site container persistently (provision pins an EP1/Premium = always-on plan) like an ACA App; route invoke/ingress/exec to that one long-lived container. This is the next chunk.
...
- **azf cell — SCAFFOLDED, untested.** `provision_azf` (run-integration.sh) + Dockerfile arms (`simulator-azure` present; `backends/azure-functions`→`sockerless-backend-azf`, `sockerless-azf-bootstrap`) + Makefile target `(azf,4568)` wired. Not yet run — start once aca is green (or in parallel).
