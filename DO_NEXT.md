...
`audit-fuzz-round17` — a **deeper/longer fuzzing + bug / weak-type / fidelity audit sweep** (5 parallel agents over the simulators, backends, shared libraries, agent, core, docker, bleephub + extended fuzz runs). **Result: the audited surface holds.** Every credible structural finding was verified at file:line as a false positive guarded by an existing invariant (16 prior hardening rounds did their job); shared-copy `state*/process/oci/router/db/otel` in sync; no stubs/TODOs; the full fuzz suite (32 targets, tens of millions of execs) found **zero crashers**. No production bug found — none fabricated. **Durable deliverable: +11 fuzz targets** for previously-uncovered untrusted-input parsers (regression guards). The lower-value "no-defaulted-behaviour on malformed input" fidelity nits (a handful of `strconv.Atoi(x), _` that silently 0-default an invalid/required field where real AWS returns a ValidationException) were left as-is — faithful clients never send invalid input, and the prior rounds judged them acceptable; a targeted batch could be done if desired.

**Next candidates:** the live-cloud track (BUG-1075); or harden a specific subsystem deeper.
