...
`feat/cw-org-ssm-completeness` — **CloudWatch/Organizations/SSM op completeness + conformance-ratchet expansion to 14 AWS services + S3 (BUG-2192).**

- **CloudWatch** 38/49 — alarm management (enable/disable actions, set-state, history, alarms-for-metric, composite alarms), metric streams, anomaly detectors, insight rules, GetMetricData, alarm mute rules, cross-resource tags — served on all three CloudWatch transports (rpc-v2-cbor for the Go SDK, awsJson for newer `aws` CLI, legacy query for older CLI). 11 niche gaps (datasets/KMS, OTel enrichment, managed insight rules, metric-widget-image) locked.
- **Organizations** 53/63 — rewrote the 2-op stub into a full stateful resource tree; a default org/root/management-account is materialized at startup so `aws:PrincipalOrgID` keeps resolving. Accounts, OUs, policies (attach/detach, policy types, effective policy), handshakes, delegated admins, service access, resource policy → IAM gate, tags. 10 niche gaps (GovCloud, responsibility-transfer, effective-policy-validation) locked.
- **SSM** 43/146 — versioned documents, maintenance windows + targets/tasks, patch baselines, service settings, resource data sync, on top of the existing Parameter Store. The 103 execution-subsystem ops (associations, ops items, sessions, automation, inventory, compliance, nodes, commands) locked.
- **Catalog:** `serviceConformanceCatalog` now covers 14 awsJson/query services + S3. The CloudWatch rpc-v2-cbor transport is a dynamic prefix dispatcher → its route is in `tests-exempt.txt` (the ops are covered via their awsJson registrations + tests).
- Tests: each op has SDK + CLI tests; runtime spec-shape validator 0 violations. aws sim/sdk/cli build/lint(0)/unit green; contract + cli-shard + ratchets pass. (Caught + fixed a real `_windows.go` GOOS-suffix build-blocker → `ssm_maintenance.go`.)

**Next candidates:** add the remaining un-ratcheted awsJson/query services to the catalog (Batch, Glue, RDS, ElastiCache, Route53/EC2/EFS need their own REST harness like S3). Implement more SSM/CloudWatch/Organizations ops if a consumer needs them. Then live-cloud (1075). Open GitHub issues: #394 (azuread upstream-blocked).
...
### (history) `feat/iam-resource-condition-keys` (MERGED as #662) — IAM enforcement: resource-scoped condition keys (#661, BUG-2185). `aws:ResourceTag/<k>` + `ec2:ResourceTag/<k>` + `ecs:cluster` + `aws:RequestTag`/`TagKeys` fed into the authz context before `iamEvalDecision`; SDK+CLI repro tests.
