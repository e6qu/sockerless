...
- **BUG-2368:** Amazon EC2 `RunInstances` control-plane state now converges to `running` before optional Firecracker host boot. A failed real host boot still settles the instance to `stopped`, but the public EC2 lifecycle no longer remains `pending` indefinitely while capable host setup is slow or blocked.
