...
`fix/sim-validate-invalid-numeric-input` — the **"no-defaulted-behaviour on invalid input" fidelity sweep** (the follow-on offered after round 17). **BUG-2093:** CloudWatch `PutMetricAlarm` validates EvaluationPeriods≥1 + single-metric Period≥1 across all 3 wire protocols (`cwValidateMetricAlarm`); BigQuery `tabledata.list` rejects a present-but-non-numeric/negative startIndex/maxResults (400 `INVALID_ARGUMENT`); absent params keep their defaults. Error codes verified against the cloudwatch Smithy model + the file's existing `MissingParameter` convention + BigQuery's `INVALID_ARGUMENT` — not guessed. **BUG-2094 (surfaced by the new validation test):** the CloudWatch rpc-v2-cbor handlers emitted awsJson-shaped errors (no `Smithy-Protocol` header), which the Go SDK rejects — a latent, untested bug; added `cwWriteCBORError`/`cwWriteCBORErrorf` (verified against aws-sdk-go-v2's `getProtocolErrorInfo`) and routed all 24 cbor error sites (alarms + dashboards) through them. SDK + CLI tests; build/lint clean.
