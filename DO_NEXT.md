...
`harden/systematic-fuzz-classes` — **systematic prevention of the bug classes fuzzing keeps finding**, instead of point-fixing each new instance. Delivered: shared safe primitives (`sim.ASCIIFold`/`CaseInsensitiveIndex` + bounded `sim.FrameReader`, 3 shared-lib copies) + migrated the CloudWatch/DynamoDB/Cosmos folds + SSM frame decoder onto them; `forcetypeassert` enabled + backlog fixed (fail-loud comma-ok everywhere); errcheck tightened (un-excluded json.Unmarshal/io.ReadAll/Decode/Sscanf + fixed every site); `scripts/check-casefold-slice.sh` pre-commit guard; nightly exploratory-fuzz CI (`scripts/run-fuzz.sh` + `fuzz-nightly.yml`). All modules lint-clean (0 forcetypeassert/errcheck); all touched tests green.
