...
1. **BUG-1792 — finish the gcs-sync exec data plane (last TEST 12 gate).** The data plane is now **wired** (this branch `feat/cloudrun-gcs-sync-dataplane-1792`): cloudrun `ExecStart`'s `gcsSyncPreExec` runs `GCSSyncDriver.PreExec` per gcs-sync volume + injects `SOCKERLESS_SYNC_VOLUMES` into the exec env + an `execPostHook` runs `PostExec` on stream `Close`; the shared agent gained `ExecHooks` threaded `ServeReverseAgentWithExecHooks` → `Router.handleExec` → `ExecSession.waitAndNotify`, and the cloudrun bootstrap passes gcs-sync restore/save (the **WS** exec path had no sync before — only HTTP `handleInvoke`). Compiles + agent unit tests green. **Two integration bugs to fix:** (a) `gcsSyncPreExec` is a runtime no-op — no `/sockerless-runner-ws/` storage request reaches the sim, so it finds **no gcs-sync volumes in `s.config.SharedVolumes`** at ExecStart despite the harness setting `SOCKERLESS_GCP_SHARED_VOLUMES`; add a debug log in `gcsSyncPreExec` (count/volumes) + check the config-load path the cmd takes (`ConfigFromEnv` vs `ConfigFromEnvironment`) and that `SharedVolumes` survives onto `s.config`. (b) the reverse-agent registration occasionally misses even a 240s deadline under local podman disk pressure — environmental; prune images + retry, or run on CI/clean engine.
