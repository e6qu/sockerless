...
`fix/dynamodb-partiql-and-tail` — **all remaining actionable DynamoDB items, one PR, no deferrals** (BUG-2141..2148).

- **PartiQL (2142):** new `dynamodb_partiql.go` — bounds-safe (`sim.Scanner`/`sim.ParseGuard`) parser + executor for `ExecuteStatement`/`BatchExecuteStatement`/`ExecuteTransaction`. SELECT/INSERT/UPDATE/DELETE translate onto the existing engine (WHERE → ConditionExpression via `ddbEvalExpr`; UPDATE → UpdateExpression via `ddbApplyUpdateExpression`; point-read vs scan; ORDER BY; base64 NextToken). Batch = per-statement errors (200); Transaction = all-or-nothing with `CancellationReasons`. Drafted by a subagent in a disjoint file, then reviewed + gated by SDK **and** CLI tests + `FuzzDDBPartiQLParse`.
- **ConsumedCapacity (2141):** decoded on Get/Put/Update/Delete/Query/Scan; faithful item-size→capacity-unit calc (`ddbAttrValueSize`/`ddbItemSizeBytes`, 4KB read / 1KB write blocks).
- **Audit tail:** CreateTable KeySchema-vs-AttributeDefinitions validation (2143); legacy `AttributeUpdates` ADD increments/unions (2144); nested `ProjectionExpression` returns only the sub-attr (2145); legacy `Expected`/`ConditionalOperator` evaluated (2146); `ConsistentRead` on a GSI rejected (2147); `DescribeLimits` (2148; `DescribeEndpoints` dropped — internal endpoint-discovery, untestable/speculative).

**Validation:** aws sim build + `golangci-lint` (0 issues) + unit + full DynamoDB SDK **and** CLI suites green; PartiQL fuzz clean; simulator testing-contract check passes (PartiQL/DescribeLimits covered by SDK+CLI; no Terraform resource exists for them).

**Next candidates:** the live-cloud track (BUG-1075); a fresh fidelity/fuzz round (e.g. another sim slice — S3, SQS, Step Functions). No actionable DynamoDB items remain. PartiQL extensions if a consumer needs them: nested-path WHERE predicates, SET arithmetic/list_append in PartiQL UPDATE, read/write-mix validation in batch/transaction (all intentionally out of this slice).
