...
# the first pull populates the cache. Required for ECS sub-task containers
# that reference any Docker Hub base image.
#
# Pull-through cache rules are singleton per (account, region, prefix).
# When both ecs + lambda modules are deployed in the same account+region,
