...
# The overlay repository is where the runtime Cloud Build pushes; the runner
# pushes to it and the Cloud Run service pulls from it.
resource "google_artifact_registry_repository_iam_member" "runner_overlay_writer" {
  project    = var.project_id
  location   = google_artifact_registry_repository.sockerless_overlay.location
  repository = google_artifact_registry_repository.sockerless_overlay.repository_id
  role       = "roles/artifactregistry.writer"
  member     = "serviceAccount:${google_service_account.runner.email}"
}
