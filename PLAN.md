...
The merged #783/#785 baselines pushed many shallow GitHub-compatible areas toward real behavior: repository metadata and permissions, pull request status rollups including count-by-state and Actions workflow-run links, durable GitHub Actions workflow-run and attempt history, durable gist/comment/star/fork state, release asset upload, GraphQL release immutable-state exposure, GitHub Pages deployment status routing, notification thread identity/URLs, user/organization/team/audit-log UI public-route usage, OAuth UI endpoint usage with explicit registered clients, registered OAuth client validation, stored-token browser authentication, persisted account suspension, checked credential entropy, Actions artifact and run/job repository scoping, workflow-dispatch ref resolution and organization provisioning for official `gh` command-line interface inputs, runtime enterprise coordinates for the Bleephub UI, Codespaces fail-loud semantics, GitHub App seed identity validation, Docker-backed local-image build loading, full local Docker-backed Bleephub hook coverage, public runner and metrics UI routing, route-level UI code-splitting, repository deletion durable-state cascades, object-backed Actions artifacts/caches/logs, and incidental AWS simulator CI hardening. The active follow-up branch extended object-backed durable service bytes to release assets, GitHub Packages files, GitHub Container Registry blobs, GitHub CodeQL database archives, and artifact attestation Sigstore bundles; made public package file, CodeQL database archive, and artifact attestation list/download paths read those object-backed bytes; made repository deletion fail loudly before metadata deletion when required git-storage, package-byte, CodeQL database-byte, or artifact-attestation-byte cleanup fails; moved go-github software development kit plus public REST organization setup to GitHub Enterprise Server's public admin organization API; made code scanning alert tests create state through GitHub's public SARIF upload path with persisted rule metadata and removed the obsolete internal code scanning alert seed route; made secret scanning alerts derive from committed repository content on contents commits and Git Database branch ref updates; removed the obsolete internal secret scanning alert seed route; made secret scanning push protection mint bypass placeholders from protected public contents and Git Database writes before mutating git state; made Dependabot alerts derive from public dependency graph snapshots plus published security advisories; corrected Git Database blob-create response shape; hardened the Bleephub UI typecheck hook against cleaned generated declarations; and fixed a CI-discovered AWS simulator SDK Amazon Elastic Container Service task-startup timing assumption, then continued from there by finding remaining places where Bleephub still:
