...
15. **GCP sim — Cloud Run jobs state machine.** Job `TerminalCondition.State = "CONDITION_SUCCEEDED"` on create. Real Cloud Run goes `CONDITION_INITIALIZING` → `BUILDING` → `DEPLOYING` → `SUCCEEDED`. Same shape as item 11.
