...
| Post-PR-#118 audit + Phase 104 skeleton + Phase 105 v2 (PR #120 — open) | Bug-audit sweep + phase plan + Phase 104 skeleton + Phase 105 second-wave shape tests. **18 bugs closed**: BUG-802 withdrawn; BUG-638/640/646/648 backfilled retroactively as closed by BUG-788; BUG-804/806 libpod-shape; BUG-820..831 fallback / synthetic-data findings (including docker NCPU/MemTotal hardcode and 0.0.0.0 placeholder leak). **Phase 104 skeleton**: 13 typed driver interfaces (`backends/core/drivers_phase104.go`), `DriverContext` envelope, per-cloud-per-dimension override resolver (`SOCKERLESS_<BACKEND>_<DIMENSION>=<impl>`), centralised `NotImplDriverError` formatter. **Phase 105 second wave**: golden shape tests for `info` / `containers/json` / `containers/{id}` (rm-report) — pinning the response shape so future regressions in the libpod dispatcher land at CI time. New phases queued: 106 (GitHub Actions runner), 107 (GitLab runner), 108 (cross-simulator feature parity audit). Project rule recorded: no defer, no fakes, no fallbacks. |
