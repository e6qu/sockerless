...
Merged #790 made GitHub Pages workflow deployment a real publication service. The active branch made legacy builds read their configured git source, publish `.nojekyll` content directly, run the real pinned GitHub Pages Jekyll toolchain for generated sites, and rebuild automatically through one committed-reference event path shared by smart HTTP, Contents API, and Git Database writes. Its dependency sweep also kept Bleephub's Markdown parser current.
