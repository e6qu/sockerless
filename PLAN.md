...
11. **Azure sim — Container Apps + Jobs `provisioningState` machine.** Real Azure: PUT returns 201 Created with `provisioningState=Creating` + `Azure-AsyncOperation` header; the SDK poller follows the header until `status=Succeeded`. Sim returns `Succeeded` directly. Proper fix needs a `/providers/Microsoft.App/locations/{loc}/operationStatuses/{id}` polling endpoint that returns InProgress→Succeeded. (Current state is the wider end of acceptable real-Azure shape — Azure does have a sync fast-path — but the proper async flow is on the to-do list.)
12. **GCP sim — Cloud Run jobs state machine.** Job `TerminalCondition.State = "CONDITION_SUCCEEDED"` on create. Real Cloud Run goes `CONDITION_INITIALIZING` → `BUILDING` → `DEPLOYING` → `SUCCEEDED`. Same shape as item 11.
13. **All sims — timestamps + state stamps respect-on-write.** Audit pass — most stamps are write-time as required, but a final sweep for re-stamping on read is on the to-do list.
