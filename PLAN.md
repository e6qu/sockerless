...
| in flight (`docs-streamline`) | 135 | Sim host model: workloads dispatch through Docker honouring explicit `Architecture` (sim's `linux/arm64` capacity); per-cloud-product host-metadata services (AWS IMDSv2 + ECS task v4; GCP `metadata.google.internal`; Azure IMDS instance metadata + identity token); static no-`os/exec`-of-workload check across all sims. Closes BUG-949 + BUG-975. See [specs/CLOUD_RESOURCE_MAPPING.md § Simulator host model](specs/CLOUD_RESOURCE_MAPPING.md#simulator-host-model-phase-135). | 949, 975 |
