...
Merged #790 made GitHub Pages workflow deployment a real publication service. The active branch then made manual legacy builds read their configured git source, publish `.nojekyll` content directly, and run the real pinned GitHub Pages Jekyll toolchain for generated sites. The remaining Pages priority was unified post-ref-update eventing and automatic source-branch builds.
...
- **BUG-2508 committed-ref eventing:** unify smart-HTTP, Contents API, and Git Database branch updates so push-derived behavior and automatic legacy Pages builds do not depend on the write surface.
