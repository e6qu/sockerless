...
Active focus: **live-cloud validation (BUG-1075)** — the sim-proven cells against real cloud infra. The pod model + GitHub/GitLab runner integration across all backends is **sim-complete**: GitHub `actions/runner` (container job + `services:` + dispatcher) AND GitLab docker-executor (build + artifact + `services:`) are both green on every container-capable backend (ECS, Cloud Run, GCF, ACA, AZF). No committed timeline; confirm before live spend.
