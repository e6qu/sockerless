...
Last completed phase: Firecracker-backed public VM lifecycle for issues #332/#333 plus the CI/ASG, Firecracker CI, and guest metadata follow-ups. AWS EC2, GCP Compute Engine, and Azure VM lifecycle paths now boot real Firecracker guests attached to the provider subnet TAP, transition public running state only after guest packet reachability, and power off/restart/delete the real guest on public lifecycle actions. Simulator SDK/CLI/Terraform CI jobs install Firecracker and the required rootfs tooling because VM APIs now require the real substrate, and the VM-backed simulator jobs run on pinned KVM-capable `ubuntu-24.04` x86_64 hosted runners with official Firecracker CI assets cached by Firecracker version and runner architecture. Simulator workload execution paths use real image/public-API architecture selection instead of stale arm64 hardcodes. Firecracker default launch workdirs are unique per launch, short enough for the API socket, and Firecracker kernel assets are selected and validated as ELF images instead of allowing `.config` sidecars. Guest rootfs images are sized from the actual copied rootfs contents with bounded headroom instead of a fixed 3 GiB image. Guest boot now copies the official rootfs contents directly into the per-VM rootfs directory, ensures the copied rootfs has a kernel-executable init path, masks/removes the official Firecracker CI rootfs `fcnet` MAC-derived networking path, installs deterministic systemd/netplan/ifupdown/resolver config for the assigned provider-private IP/gateway, and keeps TAP host endpoints separate from the Firecracker guest MAC. AWS Auto Scaling Groups now launch and terminate EC2 instances through the same real Firecracker guest lifecycle. Firecracker-backed AWS/GCP/Azure VM guests now reach provider-shaped metadata at `169.254.169.254`; GCP guests also resolve `metadata.google.internal` / `metadata` to that link-local address. Metadata handlers use the guest private source IP to return instance-specific fields. AWS ECS `StopTask` now initiates Docker graceful stop asynchronously so the public API path no longer consumes CI time waiting on the container grace period.

Next planned phase: continue BUG-1267 / #338 umbrella follow-through and re-audit the real-execution substrate for remaining cross-cloud parity gaps, unless a higher-priority issue appears. Versioned release/image publishing is intentionally deferred while the project is early. Number #337 is a merged PR, not an open issue.
...
1. **BUG-1267 / issues #332, #338.** Continue umbrella follow-through and re-audit the real-execution substrate for cross-cloud parity gaps.
...
- BUG-1267: compute/networking real-execution backlog. Issue #336's VPC/network/subnet/NIC/public-IP/NAT routing fabric landed on the real substrate. AWS/GCP/Azure firewall and load-balancer packet paths were migrated, Azure Event Grid no longer leaks simulator-local publish listeners from ARM, AWS/GCP/Azure public VM lifecycle now boots Firecracker guests, and guest metadata reachability landed for provider-shaped `169.254.169.254` / GCP metadata hostnames. Number #337 is a merged PR, not an open issue.
