...
| Post-PR-#118 audit + Phase 104 skel/lift1 + Phase 105 waves 2-3 + Phase 108 prep (PR #120 — open) | **18 bugs closed**: BUG-802 withdrawn; BUG-638/640/646/648 backfilled retroactively as closed by BUG-788; BUG-804/806 libpod-shape; BUG-820..831 fallback / synthetic-data findings. **Phase 104**: 13 typed driver interfaces (`backends/core/drivers_phase104.go`) + `DriverContext` envelope + per-cloud-per-dimension override resolver (`SOCKERLESS_<BACKEND>_<DIMENSION>=<impl>`) + centralised `NotImplDriverError` formatter + `WrapLegacyExec` adapter (first dimension lift, narrow→typed) + tests. **Phase 105 waves 2 + 3**: golden shape tests for libpod `info` / `containers/json` / rm-report / `images/pull` stream / `networks/json` / `volumes/json` / `system/df`. **Phase 108 prep**: `specs/SIM_PARITY_MATRIX.md` seeded with 77 cloud-API rows (33 AWS / 16 GCP / 28 Azure). New phases queued: 106 (GitHub Actions runner), 107 (GitLab runner), 108 (cross-simulator feature parity audit, tracker landed). Project rule recorded: no defer, no fakes, no fallbacks. |
...
Walks every cloud-API surface sockerless touches and verifies the AWS / GCP / Azure simulators implement it at the same fidelity. **Source of truth:** [specs/SIM_PARITY_MATRIX.md](../specs/SIM_PARITY_MATRIX.md) — seeded post-PR-#118 with 33 AWS / 16 GCP / 28 Azure calls inventoried by `grep` sweep across `backends/*/`. Each row starts as `tbd`, becomes ✓ / ⚠ / ✗ as Phase 108 audits the row against the corresponding sim handler. Every ⚠ / ✗ entry is filed as a BUG and fixed in-phase per the no-defer rule.

Closure rule: when all rows show ✓ the phase closes, and the existing "sim parity per commit" project rule is strengthened — any new SDK call added to a backend must update the matrix + add the sim handler in the same commit.
