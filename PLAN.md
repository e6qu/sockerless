...
Merged #790 made GitHub Pages workflow deployment a real publication service. The active branch then made manual `.nojekyll` legacy builds read their configured git source and publish through the same object-store transaction. The remaining Pages priority was the real GitHub Pages Jekyll runtime for legacy sources that require generation.
...
- **BUG-2507 GitHub Pages Jekyll execution:** run the real GitHub Pages Jekyll toolchain for legacy branch sources without `.nojekyll`, preserving real build output and errors.
