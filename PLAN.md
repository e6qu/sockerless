...
7. **Azure sim — Container Apps + Jobs `provisioningState` machine.** Real Azure: PUT returns 201 Created with `provisioningState=Creating` + `Azure-AsyncOperation` header; the SDK poller follows the header until `status=Succeeded`. Sim returns `Succeeded` directly. Proper fix needs a `/providers/Microsoft.App/locations/{loc}/operationStatuses/{id}` polling endpoint that returns InProgress→Succeeded. (Current state is the wider end of acceptable real-Azure shape — Azure does have a sync fast-path — but the proper async flow is on the to-do list.)
8. **GCP sim — Cloud Run jobs state machine.** Job `TerminalCondition.State = "CONDITION_SUCCEEDED"` on create. Real Cloud Run goes `CONDITION_INITIALIZING` → `BUILDING` → `DEPLOYING` → `SUCCEEDED`. Same shape as item 7.
9. **All sims — NAT / route table parity.** AWS `EC2NatGateway`/`EC2RouteTable` already round-trips. GCP `compute.routers` + Cloud NAT — missing. Azure `Microsoft.Network/natGateways` + `routeTables` — missing.
10. **All sims — DNS record-type breadth.** GCP Cloud DNS only has A + CNAME (no AAAA/MX/TXT). Azure Private DNS only has A (no CNAME/MX/TXT/SRV/VNetLinks). Add the remaining record types so terraform's `google_dns_record_set` and `azurerm_private_dns_*_record` round-trip.
...
13. **All sims — GCP/Azure NSG priority validation.** Real Azure rejects duplicate priorities within an NSG; sim accepts.
14. **All sims — timestamps + state stamps respect-on-write.** Several handlers re-set `CreatedAt = time.Now()` on every read; should stamp at write.
15. **No-fakes audit pass on test fixtures.** Repo-wide grep for `subnet-*`, `vpc-*`, `arn:aws:*`, `projects/test-*`, `00000000-...` style IDs in tests — every test must either use a sim-pre-registered ID or create the resource via the sim's CRUD API first.
