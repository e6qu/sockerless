...
The simulator operator surfaces used one first-party OpenID Connect authorization boundary for both their rendered interfaces and the `/sim/v1/*` dashboard data those interfaces consumed. Health probes and native AWS, Google Cloud, and Microsoft Azure protocol routes remained outside that browser-session boundary.
