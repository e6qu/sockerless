...
Last completed phase: AWS EC2/EBS, Auto Scaling, and CloudTrail coverage audit. BUG-1310 found stale coverage authority docs, not a missing implementation: Auto Scaling and CloudTrail already had real AWS SDK, AWS CLI, and Terraform provider coverage, and EC2 EBS lifecycle/snapshot operations were implemented and covered, but the canonical matrix/surface tables did not fully list them. `specs/SIM_TEST_COVERAGE_MATRIX.md` and `specs/SIM_SURFACE_TABLES/` now include `aws-autoscaling`, `aws-cloudtrail`, and the EC2 EBS operations with evidence tied to the existing public-client tests.
...
- BUG-1310 fixed stale AWS coverage authority docs found by the EC2/EBS, Auto Scaling, and CloudTrail audit. The matrix and surface tables now list `aws-autoscaling`, `aws-cloudtrail`, and the EC2 EBS volume/snapshot operations with real SDK, CLI, and Terraform evidence.
