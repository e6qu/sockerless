...
```
// simulators/{aws,gcp,azure}/go.mod
require github.com/sockerless/simulator-realexec v0.0.0
replace github.com/sockerless/simulator-realexec => ../realexec
```

Consumers: `simulators/aws` (`ec2_realexec.go`, `elbv2_dataplane.go`, NAT
tests), `simulators/gcp` (`compute_realexec.go`, `compute_loadbalancing.go`),
`simulators/azure` (compute/network). Simulator Docker builds `COPY
simulators/realexec/` alongside the sim source so the replace directive
resolves in-image.
...
```bash
make -C simulators/realexec build   # compile (library only, no binary)
make -C simulators/realexec test    # unit tests; Linux-only paths skip elsewhere
make -C simulators/realexec lint    # go vet + gofmt
```
...
- [`simulators/aws/README.md`](../aws/README.md), [`simulators/gcp/README.md`](../gcp/README.md), [`simulators/azure/README.md`](../azure/README.md) — the consumers.
- [`specs/CLOUD_RESOURCE_MAPPING.md`](../../specs/CLOUD_RESOURCE_MAPPING.md) — which cloud resources map onto which substrate primitive.
- [`docs/RUNNERS.md`](../../docs/RUNNERS.md) — runner hurdles where real execution (or its absence on a host) shaped the design.
