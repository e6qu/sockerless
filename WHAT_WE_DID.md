...
- **Phase 104 skeleton + lifts 1-4.** 13 typed driver interfaces (Exec / Attach / FSRead / FSWrite / FSDiff / FSExport / Commit / Build / Stats / ProcList / Logs / Signal / Registry) plus the `DriverContext` envelope, the `Driver.Describe()` composition rule, and the `SOCKERLESS_<BACKEND>_<DIMENSION>` override resolver. Lifts shipped: **Exec** (`WrapLegacyExec` in `driver_adapt_exec.go`), **Attach** (`WrapLegacyAttach` + `NewCloudLogsAttachDriver`), **Logs** (`WrapLegacyLogs` + `NewCloudLogsLogsDriver`), **Signal** (`WrapLegacyKill`). Typed framework subsequently renamed to drop the temporary `104` suffix: `ExecDriver104`/`AttachDriver104`/`DriverSet104` → `ExecDriver`/`AttachDriver`/`TypedDriverSet`; the narrow `core.ExecDriver` is now `LegacyExecDriver`. No behaviour change yet — backends keep their existing impls; opting in to `TypedDriverSet.{Exec,Attach,Logs,Signal}` is the next step.
