...
- **Phase 104 skeleton + first dimension lift.** 13 typed driver interfaces (`backends/core/drivers_phase104.go`) — Exec / Attach / FSRead / FSWrite / FSDiff / FSExport / Commit / Build / Stats / ProcList / Logs / Signal / Registry — plus the `DriverContext` envelope, the `Driver.Describe()` composition rule, and the `SOCKERLESS_<BACKEND>_<DIMENSION>` override resolver. **First dimension lift**: `WrapLegacyExec(narrow, backend, impl)` in `driver_adapt_exec.go` adapts the existing narrow `core.ExecDriver` to the new typed `ExecDriver104`. No behaviour change yet — backends keep their existing exec impl in `Drivers.Exec`; opting in to `DriverSet104.Exec` is the next step. Tests cover happy-path delegation, Describe() format, nil-narrow + non-Conn error paths.
...
- **Phase 108 prep.** [`specs/SIM_PARITY_MATRIX.md`](specs/SIM_PARITY_MATRIX.md) seeded with **77** cloud-API rows (33 AWS / 16 GCP / 28 Azure) inventoried by grep sweep across all 7 backends. Each `tbd` becomes ✓ / ⚠ / ✗ as Phase 108 audits. Every ⚠ / ✗ entry will be filed as a BUG and fixed in-phase per the no-defer rule.
