...
BUGS.md after this branch lands: **1160 filed · 1158 fixed · 2 open.** The proactive surface-table seed + the mux-overlap scanner + the paged-iterator rule + the state-machine skill are the load-bearing scaffolding that should keep similar reopen patterns from recurring — the next time the user finds a missing op, the table audit catches it before "fixed" is claimed.
