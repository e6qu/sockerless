...
## Post-PR-#118 audit + Phase 104 + 105 + 108 + Phase 106/107 prep (PR #120 — open)

PR #118 merged the round-8 + round-9 live-AWS sweep. PR #120 records every previously-open or "known-issue" bug as a real fix per the no-defer / no-fakes / no-fallbacks rule, and ships **Phase 104 framework migration + cloud-native typed drivers**, **Phase 105 waves 1-3** golden tests, **Phase 108 closure** (sim-parity matrix at 77/77 ✓), and **Phase 106/107 harness scaffolding**. ~50 commits ahead of main at the time of this update.

- **22 bug closures.** BUG-802 + 638/640/646/648 retro + 804/806 + 820..831 + 832..835. Full per-bug detail in [BUGS.md](BUGS.md).
- **Phase 104 framework migration complete.** 13 typed driver interfaces + `DriverContext` envelope + `Driver.Describe()` composition rule + `SOCKERLESS_<BACKEND>_<DIMENSION>` override resolver. Every dispatch site (Exec, Attach, Logs, Signal, ProcList, FSDiff, FSRead/Write/Export, Commit, Build, Registry) now flows through `TypedDriverSet`. Each dimension has a `WrapLegacy*` adapter for backends that keep going through `s.self.<api.Backend method>`; cloud-native typed drivers progressively replace those adapters. Cloud-native coverage as of this update — 44 of 91 cells in the per-backend matrix (excluding docker, where the local SDK passthrough *is* the cloud-native path):
  - **Logs + Attach** — 6 cloud backends use `NewCloudLogsLogsDriver` / `NewCloudLogsAttachDriver` going through CloudWatch / Cloud Logging / Azure Monitor via per-container fetcher factories.
  - **ECS** — additional typed drivers for ProcList / FSDiff / FS* / Signal via SSM ExecuteCommand (`backends/ecs/typed_drivers.go`).
  - **Lambda / GCF / AZF / Cloud Run / ACA** — typed Exec via `WrapLegacyExec` wrapping the existing `ReverseAgentExecDriver`; typed ProcList / FSDiff / FS* / Commit via shared `core.NewReverseAgent<X>Driver` helpers (`backends/core/driver_reverseagent_typed.go`).
  - The framework was renamed mid-stream to drop a temporary `104` suffix: `ExecDriver104`/`AttachDriver104`/`DriverSet104` → `ExecDriver`/`AttachDriver`/`TypedDriverSet`; the narrow `core.ExecDriver` became `LegacyExecDriver`.
  - **Type tightening underway.** New `core.ImageRef` domain object (`{Domain, Path, Tag, Digest}` + `ParseImageRef` + `String()`) lands at the typed `RegistryDriver.Push/Pull` boundary. Handlers parse once at dispatch; the typed driver receives a structured value. The remaining ad-hoc parse sites in `registry.go` / `image_manager.go` etc. still use the registry-resolution helper (`splitImageRefRegistry`) because they need docker-hub default rewrites — a parallel `ResolveImageReg(ImageRef)` helper is the follow-up.
  - Per-backend default-driver matrix: [specs/DRIVERS.md](specs/DRIVERS.md).
- **Phase 105 waves 1-3.** Golden shape tests for **8** libpod handlers: `pod inspect` (BUG-804) + `pod stop`/`kill` Errs serialization (BUG-806) + `info`, `containers/json`, rm-report, `images/pull` stream, `networks/json`, `volumes/json`, `system/df`.
- **Phase 108 closed (77/77 ✓).** [`specs/SIM_PARITY_MATRIX.md`](specs/SIM_PARITY_MATRIX.md) audit walked all 77 cloud-API rows (33 AWS / 16 GCP / 28 Azure). Closed BUG-832 (sim/aws ECS TagResource), BUG-833 (sim/gcp v2 Cloud Run Services), BUG-834 (sim/azure ContainerApps Apps surface), BUG-835 (sim/azure WebApps.UpdateAzureStorageAccounts). Every fix pinned with SDK + CLI tests at the wire-format the backend uses. Standing project rule strengthened (PLAN.md principle #10): any new SDK call added to a backend must update the matrix + add the sim handler in the same commit.
- **Phase 106/107 harnesses shipped.** [`tests/runners/github/harness_test.go`](tests/runners/github/harness_test.go) and [`tests/runners/gitlab/harness_test.go`](tests/runners/gitlab/harness_test.go) — build-tag-gated end-to-end harnesses that download the real `actions/runner` or `gitlab-runner` binary, register against a real CI service, dispatch a workflow / pipeline, poll until completion, and unregister cleanly. Architecture in PLAN.md: per-backend daemon (v1) → label-based dispatch via Phase 68 (v2). Sample workflows / pipelines in `tests/runners/{github,gitlab}/{workflows,pipelines}/`. Live-cloud runs against real repos still pending — gated on operator reactivating the AWS root-account key.

**Why these audit findings mattered.** Several were silent failures the existing test matrix couldn't see — backends used best-effort tagging (BUG-832), feature flags (`UseService`/`UseApp`) had no integration coverage against the sim (BUG-833/834), or the synthetic-data sweep surfaced fields that read as real-but-fabricated (BUG-820..831). Recording each as a bug + a real fix removes the temptation to revert to "it works against the sim" as a proxy for "it works."

**Repo-wide cleanup landed in this PR.**

- All `Phase NN` / `BUG-NNN` references stripped from Go source comments (95 files, ~212 occurrences) and from architecture / spec docs (specs/, docs/, top-level). The metadata stays in BUGS.md / git log / PR descriptions.
- Manual-tests directory ([`manual-tests/`](manual-tests/)) consolidates per-cloud canonical sweeps; replaces the scattered `PLAN_ECS_MANUAL_TESTING.md` + `_tasks/P86-AWS-manual-runbook.md` + `docs/manual-test-spec-crosswalk.md`.
- Three redundant simulator-parity docs deleted (specs/SIM_PARITY_MATRIX.md is the source of truth).
- 633 task-archive `.md` files dropped from `_tasks/done/` + `tasks/done/`.
