...
**All 6 backends' integration test harness restructured** (driven by user direction 2026-05-09: "no fallbacks, no skipping, all configs explicit, integration tests should be swappable sim↔cloud"):
- Every backend's `integration_test.go` (`backends/{aca, azure-functions, ecs, lambda, cloudrun, cloudrun-functions}`) — `TestMain` requires `SOCKERLESS_TEST_TARGET=sim|cloud` + `docker` + `go` on PATH. Sim path builds + starts the per-cloud simulator on a free port and pre-creates the fixed sim fixtures (storage account/managedEnvironment for ACA, storage account for AZF, ECS cluster for ECS, lambda-bootstrap test image + role for Lambda, GCS build bucket + SA JSON + gcf-bootstrap binary for GCF, baseline for Cloud Run); cloud path requires explicit `SOCKERLESS_ENDPOINT_URL` + per-backend ARM/IAM env vars. Test functions are target-agnostic — they hit the docker SDK regardless. The `//go:build integration` build tag is removed from cloudrun + cloudrun-functions; per-test `skipIfNoIntegration(t)` helpers and the legacy `SOCKERLESS_INTEGRATION` env-var gate are removed across the board. Replaces the prior shape where TestMain fell through to a nil-`dockerClient` panic on local-dev runs.
