...
| v15 | ContainerStart **ENTRY/resolved/NOT FOUND** logs at top of handler + resolvePodServiceFromCloud GetService follow-up for abbreviated annotations | **HUGE PROGRESS, new silent-hang failure mode**: ALL diagnostic logs fire correctly. ContainerStart→network-pod decision (netDefer=false netMembers=2)→marked running→materialize entry→exit in 13s. Both Services deployed (cache-permission + build/postgres pod). Bootstrap exec'd build's CMD `[/usr/bin/dumb-init /entrypoint gitlab-runner-build]` → exit=0 (expected — bootstrap stays up as HTTP server). gitlab-runner reaches "Preparing environment" then silently hangs with NO `ExecCreate`/`ExecStart`/`ContainerInspect` calls reaching sockerless backend for 30+ minutes. trace stuck at 1990 bytes. Next iteration (v16) needs frontend HTTP layer logging to prove whether gitlab-runner is making ANY docker calls that reach sockerless. |
