...
- **BUG-723 step 2 + BUG-724 step 1** — New `core.CloudImageLister` + `core.CloudPodLister` optional interfaces on `CloudStateProvider`. `BaseServer.ImageList` merges in-memory cache with cloud-registry entries (deduped by ID) when the backend implements `CloudImageLister`; `BaseServer.PodList` same-shape for pods. ECS reference implementation: `ecsCloudState.ListImages` iterates `DescribeRepositories` + `DescribeImages`, projecting each tagged image to `api.ImageSummary` with fully-qualified ECR `RepoTags`. After restart, `docker images` against ECS reflects live ECR. Five remaining backends (lambda / cloudrun / gcf / aca / azf) pending for `ListImages`; all backends pending for `ListPods`.
