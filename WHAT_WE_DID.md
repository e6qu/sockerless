...
86 phases, 757 tasks, 726 bugs tracked (725 fixed + 1 partially fixed — BUG-724). Phase 87 (Cloud Run Services) and Phase 88 (ACA Apps) both closed in code 2026-04-21 behind `SOCKERLESS_GCR_USE_SERVICE` / `SOCKERLESS_ACA_USE_APP` flags. See [STATUS.md](STATUS.md), [BUGS.md](BUGS.md), [specs/](specs/), [specs/CLOUD_RESOURCE_MAPPING.md](specs/CLOUD_RESOURCE_MAPPING.md).
...
Closes BUG-716 in code. The aca backend now has two parallel execution paths selected by `SOCKERLESS_ACA_USE_APP`:

- **Apps path**: `ContainerAppsClient.BeginCreateOrUpdate` with `Ingress.External=false` + managed environment + min/max replicas=1. Peers reach each other via Private DNS CNAME records pointing at `ContainerApp.LatestRevisionFqdn` (resolvable inside the environment's VNet). Logs filter switches to `ContainerAppName_s` in the same `ContainerAppConsoleLogs_CL` table.
- **Jobs path**: unchanged default; keeps `Jobs.BeginStart` + A-record skip-on-0.0.0.0 workaround.

`Config.Validate()` rejects `UseApp=true` without an existing Environment (no silent fallback). Five commits on `post-phase86-continuation`: appspec / cloud_state_apps / start_app / Stop-Kill-Remove branch / gate-open + CNAME discovery. 8 unit tests. Live-Azure validation pending.
...
- **`specs/CLOUD_RESOURCE_MAPPING.md`** — canonical cross-backend mapping for all 7 backends (ECS / Lambda / Cloud Run / ACA / GCF / AZF / Docker), state-derivation rules per backend, recovery contract, list of currently-violating in-memory state. Cross-linked from `specs/SOCKERLESS_SPEC.md` table-of-contents and `specs/BACKEND_STATE.md`.
...
- **BUG-725 Cloud Run** — `Server.resolveCloudRunState` + `cloudRunCloudState.resolveJobName` (list jobs, label filter on `sockerless_managed`/`sockerless_container_id`) + `resolveActiveExecution` (list executions, filter to non-terminal). Refactored `ContainerStop`, `ContainerKill`.
- **BUG-725 ACA** — `Server.resolveACAState` + `acaCloudState.resolveJobName` (list by resource group, tag filter) + `resolveActiveExecution` (Executions.NewListPager, filter to `Running`/`Processing`). Refactored `ContainerStop`, `ContainerKill`.
- **BUG-723 step 2 + BUG-724 step 1** — `core.CloudImageLister` + `core.CloudPodLister` optional interfaces on `CloudStateProvider`. `BaseServer.ImageList` / `BaseServer.PodList` merge in-memory cache with cloud-derived entries (deduped by ID). ECS + Lambda implement `ListImages` via ECR `DescribeRepositories` + `DescribeImages` → `api.ImageSummary`. ECS implements `ListPods` by regrouping sockerless-managed tasks via the `sockerless-pod` tag (earliest CreatedAt → Created; aggregate state from children). Cloudrun / GCF / ACA / AZF `ListImages` pending; cloudrun + ACA `ListPods` blocked on Phase 87/88 rewrites (Jobs path doesn't support pods).
