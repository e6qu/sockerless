...
| v15 | ContainerStart **ENTRY/resolved/NOT FOUND** logs at top of handler + resolvePodServiceFromCloud GetService follow-up for abbreviated annotations | All diagnostic logs fire correctly. ContainerStart→network-pod decision (netDefer=false netMembers=2)→marked running→materialize entry→exit in 13s. Both Services deployed. Bootstrap exec'd CMD → exit=0. gitlab-runner reaches "Preparing environment" then silently hangs. |
| v16 | ENTRY-level logs on ContainerInspect/ContainerAttach/ExecCreate/ExecStart delegates | **ROOT CAUSE PINNED**: gitlab-runner makes ZERO docker calls during silent window. **invokePodServiceMain** POSTs build's user CMD immediately on materialize completion; CMD exits 0 with no work; `PutInvocationResult` + WaitChs closed → docker reports container "exited" — but gitlab-runner expected the container to STAY ALIVE for `docker attach` script delivery. Cell 7 (cloudrun) GREEN works because `cloudrun/start_service.go::invokeServiceDefaultCmd` BLOCKS on `stdinPipe` (populated by ContainerAttach) before POSTing. Fix: port the `attach_stream.go` + `stdinPipe` + `publishAttachResponse` pattern from cloudrun to cloudrun-functions for the network-pod path. |
