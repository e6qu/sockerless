...
Full-package validation then caught two regressions that the focused exec tests alone would have missed. The exec e2e harness now self-invokes in a subprocess with an explicit env flag, so unrelated Cloud Run/GCF simulator tests keep their original direct path instead of inheriting overlay/service mode. Cloud Run and GCF `ContainerRemove` also now distinguish Docker's implicit `default` / `bridge` networks from explicit managed networks; implicit networks have no Cloud DNS/network object to disconnect, while explicit networks still clean up through the driver. Verified with full `SOCKERLESS_TEST_TARGET=sim go test -count=1` in both `backends/cloudrun` and `backends/cloudrun-functions`.

CI run `26002912832` then found two follow-on regressions. First, the shared smoke harness still assumed the pre-Phase-168 world: Cloud Run/ACA backends started without `SOCKERLESS_CALLBACK_URL`, listened only on loopback inside the smoke container, and created non-exec smoke containers without Docker's stdin-open contract. The harness now exposes the backend on all container interfaces, points callback URLs at the smoke container's real Docker-bridge address, opens stdin for Cloud Run/ACA smoke containers, and preloads `alpine:latest` into the host Docker daemon because the simulators execute real workload containers there.

Second, the GCP simulator's overlay readiness check used `GET /` against the bootstrap. That route is not a health endpoint; it is the real invocation endpoint, so the probe could run the user's command before reverse-agent registration. The readiness check now TCP-dials the listener without sending an HTTP request. Focused Cloud Run/GCF overlay exec tests pass locally, and both Cloud Run and ACA smoke containers pass locally.

The next CI run kept one failure in `backends/cloudrun TestCloudRunContainerExec`: the overlay container served HTTP but never registered a reverse-agent, pointing at the runtime env not reaching the bootstrap in that CI serialization path. The Cloud Run v2 Service simulator now decodes both real EnvVar proto-JSON oneof shapes (`value` and nested `values.value`), and the SDK service test asserts `SOCKERLESS_CALLBACK_URL` plus `SOCKERLESS_CONTAINER_ID` survive Create/Get round-trip. Focused Cloud Run exec and the Cloud Run Services SDK env round-trip pass locally.

The following CI run showed the last failure only on `ubuntu-24.04-arm`: the overlay path still assumed `linux/amd64` in backend Cloud Build config and simulator invocation. The fix made `SOCKERLESS_GCP_BUILD_PLATFORM` explicit (defaulting to the production `linux/amd64`), added the matching `gcp.build_platform` unified-config field, set simulator integration tests to the host platform, and made the simulator inspect the local overlay image's real OS/architecture before starting it. That exposed a second simulator fidelity gap: Cloud Run Services were being treated like Cloud Functions per-invocation containers. The simulator now keeps a real per-Service container instance alive until `DeleteService`, and Cloud Run's invoke endpoint no longer translates the bootstrap's exit-code header into HTTP 500. GCF pod-service cloud state now treats a live reverse-agent as the running-state authority, so a transport-level invoke response cannot mark an actively connected container exited. Focused Cloud Run and GCF exec tests pass locally with the real overlay images on arm64.

CI run `26004958009` passed every job except the `test` job, where `TestGCFContainerLifecycle` caught the over-broad version of that state rule: a completed one-shot GCF invocation with a real `InvocationResult` was still reported as running because its reverse-agent socket was live. The rule is now narrower: a live reverse-agent overrides only transport-only invoke errors; real invocation results still surface `exited` so `docker wait`, `docker inspect`, and non-force `docker rm` preserve Docker lifecycle semantics.

The focused lifecycle test passes with that rule, but full GCF package validation then exposed BUG-1088: one-shot arithmetic/logging tests could return from `docker wait` after bootstrap startup but before the user command output was produced. Root cause was the GCP simulator converting a cold-start bootstrap transport race into a Cloud Run Service 500 before the bootstrap had handled the request. The simulator now preserves the request body and retries only pre-response transport errors during the cold-start window; once the bootstrap returns any HTTP response, that response is authoritative. Focused GCF arithmetic/lifecycle/exec tests and the full GCF backend simulator package pass locally.
