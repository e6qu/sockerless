...
// cloud.google.com/go/storage client. The Cloud Run backend injects this on
// the workload host (a coordinate) so the bootstrap reaches the simulator's
// storage exactly as a real client reaches GCS; on real Cloud Run it is unset
// and the real endpoint + ADC token are used. Differs only in the coordinate.
func gcsBase() string {
