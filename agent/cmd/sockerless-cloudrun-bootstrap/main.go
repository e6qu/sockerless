...
const (
	...
	// `search` line of /etc/resolv.conf at bootstrap startup. Sourced by
	// the cloudrun backend from the per-network DNS driver
	// (cloud-dns-zone). Empty → no /etc/resolv.conf write. Lets short
	// hostnames in the same network resolve to the per-name A records
	// the network-discovery driver registers under the same suffix.
...
func main() {
	...
	if err := writeDNSSearchDomain(os.Getenv(envDNSSearchDomain)); err != nil {
		fmt.Fprintf(os.Stderr, "sockerless-cloudrun-bootstrap: write dns search domain: %v\n", err)
	}
...
func sendLifetimeExpiredOnSIGTERM(conn *websocket.Conn, connMu *sync.Mutex) {
	...
	case err := <-done:
		if err != nil {
			fmt.Fprintf(os.Stderr, "sockerless-cloudrun-bootstrap: send lifetime_expired on SIGTERM failed: %v\n", err)
		} else {
			fmt.Fprintln(os.Stderr, "sockerless-cloudrun-bootstrap: sent lifetime_expired on SIGTERM")
		}
	case <-time.After(2 * time.Second):
		fmt.Fprintln(os.Stderr, "sockerless-cloudrun-bootstrap: timed out sending lifetime_expired on SIGTERM")
	}
	os.Exit(0)
}
...
func writeDNSSearchDomain(raw string) error {
	...
	fmt.Fprintf(os.Stderr, "sockerless-cloudrun-bootstrap: /etc/resolv.conf search += %q\n", suffix)
	return nil
}
