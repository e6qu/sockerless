		...
		execHooks := agent.ExecHooks{
			...
			PostExec: func(env []string) {
				...
				if perr != nil {
					...
					fmt.Fprintf(os.Stderr, "sockerless-gcf-bootstrap: WS exec sync save SKIPPED — malformed sync volumes: %v\n", perr)
					return
				}
				if len(vols) == 0 {
					return
...
func parseUserArgv(key string) []string {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "sockerless-gcf-bootstrap: %s is not valid base64 — argv ignored: %v\n", key, err)
		return nil
	}
	var out []string
	if err := json.Unmarshal(decoded, &out); err != nil {
		fmt.Fprintf(os.Stderr, "sockerless-gcf-bootstrap: %s is not JSON argv — argv ignored: %v\n", key, err)
		return nil
	}
	return out
}
