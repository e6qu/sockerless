...
func parsePersistVolumes(env string) []persistVolume {
	...
	for _, entry := range strings.Split(env, ",") {
		...
		if len(parts) != 3 {
			fmt.Fprintf(os.Stderr, "sockerless-gcf-bootstrap: persist volume entry malformed (want name=path=bucket): %q\n", entry)
			continue
		}
		...
		if name == "" || mp == "" || bucket == "" {
			fmt.Fprintf(os.Stderr, "sockerless-gcf-bootstrap: persist volume entry has empty field: %q\n", entry)
			continue
		}
...
func restoreOne(ctx context.Context, v persistVolume) error {
	...
	if status == http.StatusNotFound {
		fmt.Fprintf(os.Stderr, "sockerless-gcf-bootstrap: persist restore %s: no prior object (first stage)\n", v.Name)
		return nil
	}
	...
	fmt.Fprintf(os.Stderr, "sockerless-gcf-bootstrap: persist restore %s: %d bytes -> %s\n", v.Name, len(body), v.MountPath)
	return nil
}
...
func saveOne(ctx context.Context, v persistVolume) error {
	...
	if err != nil {
		if errors.Is(err, os.ErrNotExist) {
			fmt.Fprintf(os.Stderr, "sockerless-gcf-bootstrap: persist save %s: mountpoint %s missing, skipping\n", v.Name, v.MountPath)
			return nil
		}
	...
	fmt.Fprintf(os.Stderr, "sockerless-gcf-bootstrap: persist save %s: %d bytes -> gs://%s/%s\n", v.Name, buf.Len(), v.Bucket, persistObjectName)
	return nil
}
