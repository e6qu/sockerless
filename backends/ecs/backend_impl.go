...
func (s *Server) ContainerCreate(req *api.ContainerCreateRequest) (*api.ContainerCreateResponse, error) {
	...
	for _, bind := range droppedBinds {
		s.Logger.Debug().Str("bind", bind).
			Msg("dropping bind mount; docker.sock has no Fargate analogue / parent shared volume already exposes the sub-path")
	}
	...
	// append a `search` line to /etc/resolv.conf and short-name lookups
	// within the network resolve. User's per-job override wins.
