...
// gcfCloudState implements core.CloudStateProvider for Cloud Functions.
// Queries GCF state store and PendingCreates for container state.
...
// allContainers merges PendingCreates and GCF state store entries
// to produce the full set of known containers.
func (p *gcfCloudState) allContainers() []api.Container {
	...
	// Containers from Store that have matching GCF state
	for _, id := range p.server.GCF.Keys() {
		if seen[id] {
			continue
		}
