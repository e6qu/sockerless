...
import (
	...
	backend "github.com/sockerless/backend-gcf"
)
...
func main() {
	...
	obs, err := core.InitObservability("sockerless-backend-gcf")
	if err != nil {
	...
	logger := zerolog.New(output).
		Level(level).
		With().Timestamp().Str("component", "backend-gcf").Logger()
	...
	logger.Info().Str("addr", *addr).Msg("starting Cloud Run Functions backend")
