...
func TestTranslateSharedVolumeBinds(t *testing.T) {
	...
	t.Run("mapped bind rewrites to named volume", func(t *testing.T) {
		translated, dropped, err := translateSharedVolumeBinds(cfg, []string{"/home/runner/_work:/__w:ro"})
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
		if len(translated) != 1 || translated[0] != "ws:/__w:ro" {
			t.Errorf("translated = %v, want [ws:/__w:ro]", translated)
		}
		if len(dropped) != 0 {
