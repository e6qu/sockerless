...
type Config struct {
	...
	// SharedVolumes are the Azure Files shares the runner Function App
	// already mounts (via the site's azurestorageaccounts config). When the
	...
	// share is shared with the runner app. Format:
	// azurecommon.SharedVolumeFormat, read from SOCKERLESS_AZF_SHARED_VOLUMES;
...
func ConfigFromEnvironment(env *core.Environment, sim *core.SimulatorConfig) Config {
	...
	c.NetworkDiscovery = core.NetworkDiscoveryFromEnv("SOCKERLESS_AZF_NETWORK_DISCOVERY", api.NetworkDiscoveryNATGatewayOnly)
	c.Access = core.AccessFromEnv("SOCKERLESS_AZF_ACCESS", api.AccessMechanismNoneInternal)
	c.AccessPrincipal = os.Getenv("SOCKERLESS_AZF_ACCESS_PRINCIPAL")
	c.BootstrapBinaryPath = os.Getenv("SOCKERLESS_AZF_BOOTSTRAP")
	c.SharedVolumes, c.sharedVolumesErr = core.ParseSharedVolumes(os.Getenv("SOCKERLESS_AZF_SHARED_VOLUMES"), azurecommon.SharedVolumeFormat)
	return c
}
