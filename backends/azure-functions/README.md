...
- **Multiple containers sharing one pod.** Assembled as ONE App Service site with **sitecontainers** (`Microsoft.Web/sites/{name}/sitecontainers` — the native Azure multi-container primitive: one `isMain` container + N sidecars sharing a network namespace). A pod of containers on a user-defined docker network materializes a single Function App site: the `isMain` container runs the reverse-agent overlay (the runner execs into it), so a sibling service is reachable from the main on `localhost:<port>`; the main bootstrap also writes the pod aliases to `/etc/hosts` so a sibling resolves by name. **Sidecars** run the overlay in *sidecar mode* (`SOCKERLESS_SIDECAR=1`): the bootstrap dials its own reverse-agent (so `docker exec <sidecar>` works) and execs the service without binding the function HTTP port; a raw image is the no-overlay fallback. **Shared workspace**: members' named-volume binds attach to the site once and each sitecontainer mounts it via `VolumeMounts`. This is the Azure analog of an ECS multi-container task / Cloud Run multi-container revision. GitHub Actions `services:` / sidecar `container:` jobs and GitLab service containers run on AZF. `docker network create` remains assembled via the Azure Private DNS zone (`cloud-dns` NetworkDiscovery driver) for cross-app discovery. See `pod_site.go::materializePodSite` + `network_pod.go`.
