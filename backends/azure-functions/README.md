...
- **Multiple containers sharing one pod.** Azure Functions is single-invocation: today a pod can hold at most one container, so `PodStart`/`ContainerStart` reject a multi-container pod up front (`multi-container pods are not supported by the azure-functions backend`). This is a *current limitation of the assembly*, not a missing cloud analog — it constrains containers sharing one invocation, **not** networking between single-container apps. `docker network create` is fully assembled on AZF (Azure Private DNS zone + per-container A/CNAME records via the `cloud-dns` NetworkDiscovery driver), so cross-app service discovery works. For GitHub Actions `services:` / sidecar `container:` jobs that need siblings in one pod today, run each container as its own single-container workload or use a container backend (ECS / Cloud Run / ACA); the project's direction is to compose multi-container pods on AZF from cloud primitives (sidecar containers) + the agent as the platform permits.
