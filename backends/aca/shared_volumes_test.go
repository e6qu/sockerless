...
func TestTranslateSharedVolumeBinds(t *testing.T) {
	...
	t.Run("rejection without any shared volumes still mentions the env var", func(t *testing.T) {
		_, _, err := translateSharedVolumeBinds(Config{}, []string{"/h:/c"})
		if err == nil || !strings.Contains(err.Error(), "SOCKERLESS_ACA_SHARED_VOLUMES") {
			t.Errorf("error %v does not mention SOCKERLESS_ACA_SHARED_VOLUMES", err)
		}
	})
