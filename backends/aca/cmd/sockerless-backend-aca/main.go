...
func main() {
	...
	obs, err := core.InitObservability("sockerless-backend-aca")
	if err != nil {
	...
	logger := zerolog.New(output).
		Level(level).
		With().Timestamp().Str("component", "backend-aca").Logger()
	...
	logger.Info().Str("addr", *addr).Str("rg", config.ResourceGroup).Msg("starting ACA backend")
