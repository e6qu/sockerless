...
// Named-volume binds (`volName:/mnt`) pass through and map to Cloud Run
// `Volume{Gcs{Bucket}}` on the sockerless-owned project. Host-path
// binds (`/h:/c`) translate via SharedVolumes (config-driven map from
// caller-side mount path → sockerless-managed named-volume + GCS
// bucket). Mirrors the ECS + Lambda translators:
//
...
//   - `/var/run/docker.sock` → drop (no docker socket on Cloud Run;
//     the github-runner adds this unconditionally);
//   - anything else → reject loudly (no host filesystem on Cloud Run).
//
