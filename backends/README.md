...
Per-cloud code that is shared within one cloud family lives in `aws-common`, `gcp-common`, and `azure-common` (SDK client configuration, the cloud's volume storage manager, image resolution against the cloud's registry, the shared-volume tuple format, the DNS record operations behind the shared skeleton, log readers). `gcp-common/registrytest` is the test-support package the Google harnesses provision Artifact Registry with — repositories through the Artifact Registry API and a `docker login` with an access token minted from a service-account key — the way an operator does against the real service. These three modules never import each other; see [AGENTS.md](../AGENTS.md) § "Shared code has three homes". The exec-envelope wire contract the backends and the bootstrap binaries both speak is `agent/envelope`.
