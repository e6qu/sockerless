...
Agent drivers route operations to forward or reverse agent connections. Operations return errors when no agent is connected.
...
| No agent | No agent connected | Operations return errors |
...
| Aspect | Vanilla Docker | Core Default |
|--------|---------------|--------------|
| What happens | Starts process via runc | Sets state to running (cloud backends override to create cloud resources) |
...
Calls `Store.ForceStopContainer(id, 0)`.

### `POST /containers/{id}/kill` — Kill Container

SIGKILL → exit code 137; other signals → exit code 0. Updates container state.

### `DELETE /containers/{id}` — Remove Container

Force-stops if running, deletes all associated state.

### `POST /containers/{id}/restart` — Restart Container

Stops, re-creates exit channel, re-starts. Increments `RestartCount`.
...
| Aspect | Vanilla Docker | Core Default |
|--------|---------------|--------------|
| Source | Docker daemon | `Drivers.Stream.LogBytes(id)` from driver |
...
Creates `ExecInstance` with command, env, working directory. Container must be running.
...
| Dispatch | nsenter into container | `Drivers.Exec.Exec()` through driver |
| Env | Exec-specific env | Container env + exec env merged (exec overrides) |
| Working dir | Exec workdir or container default | Same |
| Auto-stop | N/A | N/A (cloud backends manage container lifecycle) |
...
| Operation | Vanilla Docker | Core Default |
|-----------|---------------|--------------|
| Create | Real volume (local driver) | In-memory; creates temp directory in `VolumeDirs` |
...
| Prune (overridable) | Removes exited containers | Deletes exited/dead containers |
...
| `docker start` | Set running state (cloud backends override) |
| `docker stop` | Force-stop container |
| `docker kill` | Set exit code based on signal |
| `docker rm` | Remove all state |
| `docker restart` | Stop + re-start |
