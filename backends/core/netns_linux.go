...
func (m *NetnsManager) CreateVethPair(networkID, containerID, containerIP string) error {
	...
	// Roll back the veth pair on the first failure (the previous
	// behaviour was silent on error: if any of these three steps
	// failed the container's veth was half-configured — e.g. no IP,
	// link down — but the function returned success and the operator
	// only noticed when network reach broke).
