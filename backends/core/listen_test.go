...
func newTestBaseServer() *BaseServer {
	logger := zerolog.Nop()
	store := NewStore()
	s := NewBaseServer(store, BackendDescriptor{
		ID:   "test",
		Name: "test",
	}, logger)
	return s
...
func TestTLSListenAndServe(t *testing.T) {
	tmpDir := t.TempDir()
	...
	go func() { _ = s.ListenAndServe(addr, certFile, keyFile) }()
...
func TestNonTLSListenAndServe(t *testing.T) {
	s := newTestBaseServer()
	ln, err := net.Listen("tcp", "127.0.0.1:0")
	if err != nil {
		t.Fatal(err)
...
func TestUnixSocketListenAndServe(t *testing.T) {
	tmpDir := t.TempDir()
	certFile, keyFile := genSelfSignedCert(t, tmpDir)
	sockPath := filepath.Join(tmpDir, "docker.sock")

	s := newTestBaseServer()
	go func() { _ = s.ListenAndServe(sockPath, certFile, keyFile) }()
	time.Sleep(100 * time.Millisecond)
