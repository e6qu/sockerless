...
- Adding a new `mux.HandleFunc(...)` or `srv.HandleFunc(...)` registration in `simulators/<cloud>/*.go`.
...
```
simulators/aws/s3.go:121     POST /{bucket}/{key...}      handleS3PostObjectDispatch
simulators/aws/apigatewayv2.go:42  POST /v2/apis/{id}/deployments    handleAPIGwV2CreateDeployment
...
```
simulators/aws/s3.go::POST /{bucket}/{key...}  simulators/aws/apigatewayv2.go::POST /v2/apis/{id}/deployments  S3 gates by known-bucket; apigatewayv2 wins for /v2/* paths
```
