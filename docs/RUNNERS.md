...
Architectural shape: the runner *is* the workspace. For `container:` jobs it does `docker create -v /home/runner/_work:/__w …` — host bind mounts that assume a shared filesystem with the spawned container. **The sockerless-side bind-mount → shared-volume translation has landed across all six container backends** (`SOCKERLESS_{ECS,LAMBDA,GCP,ACA,AZF}_SHARED_VOLUMES`: binds whose source matches a configured path translate to the shared cloud volume — EFS access points / GCS buckets / Azure Files — with sub-path binds dropped because the parent mount already exposes them; unmapped binds still reject loudly). **The full runner-as-cloud-task topology is now SIM-PROVEN end-to-end** by the bleephub official-runner harness (`make bleephub-runner-docker-test`, tests 12-14): the runner's workspace lives on a sim-EFS access point, `container:` jobs and `services:` dispatch through sockerless-backend-ecs as sim-ECS tasks on the host engine sharing that workspace (a file written inside the job container is asserted on the runner's EFS workspace), and the github-runner dispatcher closes the control plane — it polls bleephub for the queued job, spawns an ephemeral runner, and the job completes on it. What remains for cells 1 + 2 is the LIVE pass: the same assembly against real ECS/Lambda (live-cloud track).
...
| GH-2 | Bind-mount `/var/run/docker.sock` for nested-docker support | Cloud backends must either provide a real socket contract or reject it clearly; nested `docker run` needs a separate dispatch path back to a sockerless instance | BUG-850 |
| GH-3 | `docker exec` issued immediately after task RUNNING, before SSM `ExecuteCommandAgent` is up (5-30 s lag on Fargate) | The ECS exec path waits for `ManagedAgents[ExecuteCommandAgent].LastStatus == RUNNING` before invoking ECS `ExecuteCommand`; same wait reused from `RunCommandViaSSM` | BUG-853 |
| GH-4 | Per-job docker network created via `docker network create --label <jobnum> github_network_<hex>` then sub-task attached | ECS maps Docker networks to VPC security groups + Cloud Map instead of Linux netns; the runner gets Docker-shaped network membership while the cloud resources carry the real isolation/discovery | BUG-851 |
...
| GH-6 | `workflow_dispatch` only fires on workflows present on the default branch | Harness commits the per-cell YAML to a throwaway branch at the path of an *existing* main-branch workflow file (e.g. `live-tests-ecs.yml`'s slot) and dispatches with `ref=<branch>` so the throwaway-branch content runs | (n/a — harness pattern) |
...
| GH-16 | AZF cell `container:` job `ContainerCreate` 500 — the Azure sim's ACR-Tasks overlay build hardcoded the buildx-only `--load`, which the legacy `docker.io` builder (no buildx plugin) rejects | Sim probes `docker buildx`; uses `docker buildx build --load` when present (loads to the daemon store for every driver), else plain `docker build` (legacy, store-native); chosen path logged. Irreducible host-capability detection (no single invocation spans legacy / buildx-docker / buildx-docker-container) — both arms run in real environments, errors propagate loudly | BUG-1834 |
| GH-17 | AZF cell mis-deployed the GitHub container-job topology: `startCloudDNSSite` keyed overlay-vs-raw on `OpenStdin` (gitlab-only), so the exec-driven `container:` job (not OpenStdin) ran raw → no reverse-agent → step `docker exec` exit 126, and a `services:` container (image-default entrypoint) ran as the overlay → workload never started | Derive `serviceLike` (no client entrypoint/cmd override AND not OpenStdin) from the ORIGINAL client create request, recorded into `labelServiceLike` BEFORE azf merges the image's default entrypoint/cmd (post-merge labels can't tell client-override from image-default; aca computes it pre-merge too). serviceLike → raw image started on the VNet by swift integration; else → overlay + invoke that blocks for the in-site reverse-agent | BUG-1835 |
...
This shape is **SIM-PROVEN end-to-end** by the [Bleeplab](https://github.com/e6qu/bleeplab) control-plane simulator: a real `gitlab-runner` registers against Bleeplab and dispatches jobs through a Sockerless backend, exercising the full runner-as-cloud-task data plane against the cloud simulators. The GL-12/13/14 hurdles below were all found and fixed on the Cloud Run cell of that harness. The per-runner × per-backend sim-proven set (both runners across the container backends) is tracked in [`runner-capability-matrix.md`](./runner-capability-matrix.md); the live-cloud subset is the four-cell matrix above.
...
| GL-12 | **gitlab-runner on cloudrun:** the runner picks its helper image arch from the DOCKER_HOST's reported `docker /version` arch; the cloudrun backend hardcoded `amd64`, so on arm64 it pulled the `x86_64` helper | Derive the backend's reported arch from `config.BuildPlatform` (`archFromPlatform`), like ECS reports `SOCKERLESS_ECS_CPU_ARCHITECTURE` | BUG-1808 |
| GL-13 | **gitlab-runner on cloudrun:** the `gitlab-runner-helper` image (overlay-built on Cloud Run) failed to pull — the gcp sim's AR pull-through only hydrated `docker-hub`, but the backend rewrites `registry.gitlab.com/<path>` → `<AR>/gitlab-registry/<path>` | gcp sim `hydrateOCIImageFromLocalDocker` maps `/gitlab-registry/` → local `registry.gitlab.com/<path>`; harness stages the arch-matched `gitlab-runner-helper:<arch>-v<ver>` | BUG-1809 |
| GL-14 | **gitlab-runner on cloudrun:** the cache-volume permission container (a one-shot Cloud Run **Service** default-cmd invoke) failed `bootstrap not ready: timeout after 30s` — the sim dialed `127.0.0.1:hostPort`, unreachable when the sim runs inside the harness container (host-published port ≠ sim container's loopback). bleephub never hit it (github-runner is exec-driven, not one-shot Service invokes) | gcp sim reaches the workload by its **bridge container IP:8080** (routable container-to-container), falls back to `127.0.0.1:hostPort` for a host-run sim; `start_service.go` now logs a failing default-cmd's output tail | BUG-1810 |
...
| D-8 | Dispatchers hardcoded api.github.com (GHES/bleephub impossible) and scopes.Verify required the X-OAuth-Scopes header that fine-grained PATs / app tokens never send | `--api-base` threaded through poller + scopes + token mint; header-less tokens verify by CAPABILITY (mint a registration token — the dispatcher's one load-bearing permission) | BUG-1763 / BUG-1764 |
| D-9 | The official runner deserializes `jobContainer`/`jobServiceContainers` as TemplateTokens — plain JSON maps fail job-start validation ("The template is not valid. Unexpected value ''"); object-form `container:` lost env/ports/volumes/options | bleephub emits mapping/sequence tokens (`{"type":2,"map":[{Key,Value}]}`, runner's `env` key not `environment`) for services and object containers | BUG-1765 / BUG-1766 |
| D-10 | `config.sh --ephemeral` aborts when the registration response drops the `ephemeral` property; completed ephemeral runners lingered as zombie registrations | bleephub round-trips the flag and deregisters ephemeral agents after their job completes | BUG-1769 |
| D-11 | Runs whose jobs are all still queued reported `in_progress` — invisible to `?status=queued` pollers (dispatchers, ARC-style autoscalers); job messages baked the SUBMITTER's request-Host as server URL, so off-host runners ran the job but could never report completion (Worker dialed 127.0.0.1) | run status derives from job states (queued until a job STARTS); `BLEEPHUB_EXTERNAL_URL` (the GHES external-URL knob) overrides request-Host URL derivation | BUG-1768 / BUG-1770 |
| D-12 | Spawned runner containers must resolve host.docker.internal to dial host-published services — absent on Linux Docker unless `--add-host …:host-gateway`, which Podman 5 rejects (GH-8's pair) | docker-shape Spawn detects the engine (docker version components) and adds the flag only where needed and legal — the sims' runtime-detection pattern | BUG-1771 |
