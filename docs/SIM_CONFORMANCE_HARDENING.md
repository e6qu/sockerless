...
- **Next — Batch G3 (pagination):** compute lists read only `pageSize` not `maxResults` (canonical compute param; existing test green-but-blind — fix `paginateList` to also read maxResults); compute-LB lists + DNS rrsets + eventarc + dataflow + logging entries:list + apigateway lists emit no nextPageToken; bigquery/spanner/bigtable/memorystore lists no token; firestore list/runQuery ignore pageSize/limit/orderBy + only do EQ filters. Then **Batch G4 (missing ops):** GCS bucket+object PATCH, Spanner Instances.Patch, KMS CreateCryptoKeyVersion(+enable/disable/restore), CloudFunctions v2 UpdateFunction+generateUploadUrl, CloudBuild ListBuilds, Bigtable modifyColumnFamilies+instance/cluster update, memorystore/SQL updateMask merge. Then update this doc AFTER the stage and move to Stage 3 (Azure).
