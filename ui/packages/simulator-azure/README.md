...
`make embed` in `simulators/azure/` copies this package's `dist/` to `simulators/azure/dist/` (see `make/go-app.mk`), which the binary bundles via `//go:embed all:dist` (`simulators/azure/ui_embed.go`) and serves at `/ui/`. A `-tags noui` build skips it.
