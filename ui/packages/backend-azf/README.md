# @sockerless/ui-backend-azf

Azure Functions Backend UI. The entire app is `<BackendApp title="Azure Functions Backend" />` from `@sockerless/ui-core`, plus an `index.html` that loads the design-system fonts and this app's `index.css`.
...
`make embed` in `backends/azure-functions/` copies this package's `dist/` to `backends/azure-functions/dist/` (see `make/go-app.mk`), which the binary bundles via `//go:embed all:dist` (`backends/azure-functions/ui_embed.go`) and serves at `/ui/`. A `-tags noui` build skips it.
