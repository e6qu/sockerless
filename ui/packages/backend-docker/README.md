# @sockerless/ui-backend-docker

Docker Backend UI. The entire app is `<BackendApp title="Docker Backend" />` from `@sockerless/ui-core`, plus an `index.html` that loads the design-system fonts and this app's `index.css`.
...
`make embed` in `backends/docker/` copies this package's `dist/` to `backends/docker/dist/` (see `make/go-app.mk`), which the binary bundles via `//go:embed all:dist` (`backends/docker/ui_embed.go`) and serves at `/ui/`. A `-tags noui` build skips it.
