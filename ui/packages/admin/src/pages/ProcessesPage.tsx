    ...
    <div className="space-y-6">
      ...
      {processes.length === 0 ? (
        <p className="text-sm text-gray-500 dark:text-gray-400">
          No managed processes configured. Add process definitions to admin.json.
        </p>
      ) : (
          ...
          <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
            <thead>
              <tr>
                ...
                <th className="px-4 py-3 text-left text-xs font-medium uppercase text-gray-500 dark:text-gray-400">Binary</th>
                <th className="px-4 py-3 text-left text-xs font-medium uppercase text-gray-500 dark:text-gray-400">Status</th>
                <th className="px-4 py-3 text-left text-xs font-medium uppercase text-gray-500 dark:text-gray-400">PID</th>
                <th className="px-4 py-3 text-left text-xs font-medium uppercase text-gray-500 dark:text-gray-400">Address</th>
                <th className="px-4 py-3 text-left text-xs font-medium uppercase text-gray-500 dark:text-gray-400">Uptime</th>
              ...
              {processes.map((proc: ProcessInfo) => (
                <tr key={proc.name}>
                  <td className="whitespace-nowrap px-4 py-3 text-sm font-medium">
                    <Link to={`/ui/processes/${encodeURIComponent(proc.name)}`} className="text-blue-600 hover:underline dark:text-blue-400">
                      {proc.name}
                    </Link>
                  </td>
                  <td className="whitespace-nowrap px-4 py-3 text-sm text-gray-500 dark:text-gray-400">{proc.binary}</td>
                  <td className="whitespace-nowrap px-4 py-3 text-sm">
                    <StatusBadge status={statusLabel(proc.status)} />
                  </td>
                  <td className="whitespace-nowrap px-4 py-3 text-sm text-gray-500 dark:text-gray-400">{proc.pid || "-"}</td>
                  <td className="whitespace-nowrap px-4 py-3 text-sm text-gray-500 dark:text-gray-400">{proc.addr || "-"}</td>
