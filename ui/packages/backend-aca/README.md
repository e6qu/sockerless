# @sockerless/ui-backend-aca

Container Apps Backend UI. The entire app is `<BackendApp title="Container Apps Backend" />` from `@sockerless/ui-core`, plus an `index.html` that loads the design-system fonts and this app's `index.css`.
...
`make embed` in `backends/aca/` copies this package's `dist/` to `backends/aca/dist/` (see `make/go-app.mk`), which the binary bundles via `//go:embed all:dist` (`backends/aca/ui_embed.go`) and serves at `/ui/`. A `-tags noui` build skips it.
