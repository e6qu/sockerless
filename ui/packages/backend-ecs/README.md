# @sockerless/ui-backend-ecs

ECS Backend UI. The entire app is `<BackendApp title="ECS Backend" />` from `@sockerless/ui-core`, plus an `index.html` that loads the design-system fonts and this app's `index.css`.
...
`make embed` in `backends/ecs/` copies this package's `dist/` to `backends/ecs/dist/` (see `make/go-app.mk`), which the binary bundles via `//go:embed all:dist` (`backends/ecs/ui_embed.go`) and serves at `/ui/`. A `-tags noui` build skips it.
