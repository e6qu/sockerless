# @sockerless/ui-backend-cloudrun

Cloud Run Backend UI. The entire app is `<BackendApp title="Cloud Run Backend" />` from `@sockerless/ui-core`, plus an `index.html` that loads the design-system fonts and this app's `index.css`.
...
`make embed` in `backends/cloudrun/` copies this package's `dist/` to `backends/cloudrun/dist/` (see `make/go-app.mk`), which the binary bundles via `//go:embed all:dist` (`backends/cloudrun/ui_embed.go`) and serves at `/ui/`. A `-tags noui` build skips it.
