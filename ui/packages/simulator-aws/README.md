...
`make embed` in `simulators/aws/` copies this package's `dist/` to
`simulators/aws/dist/` (see `make/go-app.mk`), which the binary bundles via
`//go:embed all:dist` (`simulators/aws/ui_embed.go`) and serves at `/ui/`. A
